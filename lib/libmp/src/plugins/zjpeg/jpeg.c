...
/*
 * Zephyr does not provide CMSIS "cmsis_gcc.h" on all architectures (e.g. native_sim).
 * The SIMD-accelerated path is optional; build without it on Zephyr.
 */
#define NO_SIMD
#endif
...
/* Zephyr only uses the RAM interface (libMP provides input data). */
int JPEG_openRAM(JPEGIMAGE *pJPEG, uint8_t *pData, int iDataSize, JPEG_DRAW_CALLBACK *pfnDraw)
{
    memset(pJPEG, 0, sizeof(JPEGIMAGE));
    pJPEG->ucMemType = JPEG_MEM_RAM;
