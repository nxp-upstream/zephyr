...
BUILD_ASSERT(!IS_ENABLED(CONFIG_THREAD_RUNTIME_STATS_USE_TIMING_FUNCTIONS),
	     "CPU load windows require CPU cycle runtime statistics");
...
static uint8_t window_count[CONFIG_MP_MAX_NUM_CPUS];
...
static uint8_t cpu_load_confidence(int cpu_id)
{
	uint32_t confidence = CPU_LOAD_CONFIDENCE_BASE +
		(window_count[cpu_id] * CPU_LOAD_CONFIDENCE_STEP);
...
int cpu_load_window_get(int cpu_id, struct cpu_load_window *window)
{
	...
	if ((window == NULL) || (cpu_id < 0) || (cpu_id >= arch_num_cpus())) {
		return -EINVAL;
	}
	...
	if (window_count[cpu_id] < UINT8_MAX) {
		window_count[cpu_id]++;
	}

	window_us = k_cyc_to_us_floor64(execution_cycles);

	window->non_idle_cycles = total_cycles;
	window->window_us = (uint32_t)MIN(window_us, (uint64_t)UINT32_MAX);
	window->load = (uint8_t)((100U * total_cycles) / execution_cycles);
	window->confidence = cpu_load_confidence(cpu_id);
	window->source_mask = CPU_LOAD_WINDOW_SOURCE_RUNTIME_HISTORY;
