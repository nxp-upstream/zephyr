...
typedef int (*mux_control_set_lock_fn)(const struct device *dev,
				       const struct mux_control *control,
				       bool lock);
...
__subsystem struct mux_control_driver_api {
	...
	/** @driver_ops_optional @copybrief mux_control_set_lock */
	mux_control_set_lock_fn set_lock;
};
...
/**
 * @brief Lock a MUX control line against further state changes.
 *
 * Some MUX hardware exposes a register-lock bit that freezes a route until
 * reset (e.g. NXP TRGMUX). This is NOT a concurrency lock: the subsystem does
 * no mutual exclusion between consumers. Drivers that support the freeze
 * implement the @c set_lock op; others return @c -ENOSYS.
 *
 * @param dev     MUX controller device.
 * @param control Addressing of the control line.
 * @param lock    @c true to lock, @c false to unlock (where supported).
 *
 * @retval 0 On success.
 * @retval -ENOSYS If the driver does not implement this operation.
 ...
 * @retval -errno Other negative errno on failure.
 */
__syscall int mux_control_set_lock(const struct device *dev,
				   const struct mux_control *control,
				   bool lock);

static inline int z_impl_mux_control_set_lock(const struct device *dev,
					      const struct mux_control *control,
					      bool lock)
{
	const struct mux_control_driver_api *api = DEVICE_API_GET(mux_control, dev);

	if (api->set_lock == NULL) {
		return -ENOSYS;
	}

	return api->set_lock(dev, control, lock);
}
