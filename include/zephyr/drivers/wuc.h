...
__subsystem struct wuc_driver_api {
	...
	wuc_api_record_wakeup_source_triggered record;
};
/** @endcond */
 ...
 * @retval -ENOSYS if the interface is not implemented.
 */
static inline int wuc_record_wakeup_source_triggered(const struct device *dev, uint32_t id)
{
	if (DEVICE_API_GET(wuc, dev)->record == NULL) {
		return -ENOSYS;
	}
	return DEVICE_API_GET(wuc, dev)->record(dev, id);
}
