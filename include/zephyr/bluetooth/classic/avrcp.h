...
struct bt_avrcp_item_hdr {
	uint8_t  item_type;  /**< 0x01=Media Player, 0x02=Folder, 0x03=Media Element */
...
struct bt_avrcp_media_element_item {
    ...
    uint8_t  media_type;          /**< bt_avrcp_media_type_t */
    uint16_t charset_id;          /**< Character set for the name */
    uint8_t  name_attr[];         /**< Flexible array:
...
/** @brief Callback function type for AVRCP event notifications for changed.
 *
...
struct bt_avrcp_ct_cb {
	 ...
	 *            codeis an AV/C REJECTED response, and buf is NULL.
	 *            Note that all multi-octet fields are encoded in big-endian format.
	 */
	void (*get_caps_rsp)(struct bt_avrcp_ct *ct, uint8_t tid, uint8_t status,
			     struct net_buf *buf);
	 ...
	 *             big-endian format.
	 */
	void (*set_browsed_player_rsp)(struct bt_avrcp_ct *ct, uint8_t tid, struct net_buf *buf);
	 ...
	 */
	void (*get_folder_items_rsp)(struct bt_avrcp_ct *ct, uint8_t tid, struct net_buf *buf);
	 ...
	 *  @param ct  AVRCP CT connection object.
	 *  @param tid The transaction label of the response.
	 *  @param buf The response buffer containing the Change Path response parameters
	 *             as defined by AVRCP PDU CHANGE_PATH (e.g. Status, NumberOfItems).
	 *             The application should parse this payload per the AVRCP specification.
	 *             Note that the data is encoded in big-endian format.
	 */
	void (*change_path_rsp)(struct bt_avrcp_ct *ct, uint8_t tid, uint8_t status,
			       uint32_t num_items);
	 ...
	 *             Note that the data is encoded in big-endian format.
	 */
	void (*get_item_attrs_rsp)(struct bt_avrcp_ct *ct, uint8_t tid, struct net_buf *buf);
	 ...
	 */
	void (*get_total_number_of_items_rsp)(struct bt_avrcp_ct *ct, uint8_t tid, struct net_buf *buf);
	 ...
	 *             specification. Note that the data is encoded in big-endian format.
	 */
	void (*search_rsp)(struct bt_avrcp_ct *ct, uint8_t tid, struct net_buf *buf);
	 ...
	 *  @param status status.
	 */
	void (*browsing_general_reject_rsp)(struct bt_avrcp_ct *ct, uint8_t tid, uint8_t status);
	 ...
	 */
	void (*set_absolute_volume_rsp)(struct bt_avrcp_ct *ct, uint8_t tid, uint8_t status,
					uint8_t absolute_volume);
	 ...
	 *                 @c BT_AVRCP_STATUS_SUCCESS for an interim notification.
	 *                 Error codes may be returned for invalid parameters or
	 *                 unsupported events.
	 * @param event_id AVRCP event identifier (BT_AVRCP_EVENT_*).
	 * @param data     Pointer to @ref bt_avrcp_event_data containing the
	 *                 event-specific payload. The active union member in
	 *                 @ref bt_avrcp_event_data depends on @p event_id.
	 *
	 * @note This callback is only invoked for interim notifications and error
	 *       statuses from the TG. For changed events, register
	 *       @ref bt_avrcp_notification_cb_t.
	 */
	 ...
	 *               or BT_AVRCP_STATUS_INVALID_PARAMETER.
	 */
	void (*inform_batt_status_of_ct_rsp)(struct bt_avrcp_ct *ct, uint8_t tid, uint8_t status);
	 ...
	 *            Note that all multi-octet fields are encoded in big-endian format.
	 */
	void (*get_element_attrs_rsp)(struct bt_avrcp_ct *ct, uint8_t tid, uint8_t status,
				      struct net_buf *buf);
	 ...
	 *            Note that all multi-octet fields are encoded in big-endian format.
	 */
	void (*get_play_status_rsp)(struct bt_avrcp_ct *ct, uint8_t tid, uint8_t status,
				    struct net_buf *buf);
	 ...
	 *               or BT_AVRCP_STATUS_INVALID_PARAMETER.
	 */
	void (*set_addressed_player_rsp)(struct bt_avrcp_ct *ct, uint8_t tid, uint8_t status);
	 ...
	 *               or BT_AVRCP_STATUS_INVALID_PARAMETER.
	 */
	void (*play_item_rsp)(struct bt_avrcp_ct *ct, uint8_t tid, uint8_t status);
	 ...
	 *               or BT_AVRCP_STATUS_INVALID_PARAMETER.
	 */
	void (*add_to_now_playing_rsp)(struct bt_avrcp_ct *ct, uint8_t tid, uint8_t status);
};
...
/** @brief Play Item.
 *
 *  This function sends AVRCP PDU PLAY_ITEM to the remote device.
 *
 *  @param ct  The AVRCP CT instance.
 *  @param tid The transaction label of the response, valid from 0 to 15.
 *  @param buf The command buffer containing the Play Item command parameters
 *             (Scope, UID, UIDCounter) encoded in big-endian format as defined by
...
struct bt_avrcp_tg_cb {
	 ...
	 *             encoded in big-endian format as defined by the AVRCP specification.
	 */
	void (*get_folder_items_req)(struct bt_avrcp_tg *tg, uint8_t tid, struct net_buf *buf);
	 ...
	 *             defined by the AVRCP specification.
	 */
	void (*change_path_req)(struct bt_avrcp_tg *tg, uint8_t tid, struct net_buf *buf);
	 ...
	 *             encoded in big-endian format as defined by the AVRCP specification.
	 */
	void (*get_item_attrs_req)(struct bt_avrcp_tg *tg, uint8_t tid, struct net_buf *buf);
	 ...
	 *  @param tg  AVRCP TG connection object.
	 *  @param tid The transaction label of the request.
	 *  @param buf The request buffer containing the Get Total Number Of Items
	 *             command parameters (Scope) encoded in big-endian format as defined
	 *             by the AVRCP specification.
	 */
	void (*get_total_number_of_items_req)(struct bt_avrcp_tg *tg, uint8_t tid,  uint8_t scope);
	 ...
	 *             defined by the AVRCP specification.
	 */
	void (*search_req)(struct bt_avrcp_tg *tg, uint8_t tid, struct net_buf *buf);
	 ...
	 *  @param player_id The player ID to be set as addressed player.
	 */
	void (*set_addressed_player_req)(struct bt_avrcp_tg *tg, uint8_t tid, uint16_t player_id);
 ...
 *  @return 0 on success or error code.
 */
int bt_avrcp_tg_send_get_play_status_rsp(struct bt_avrcp_tg *tg, uint8_t tid, uint8_t status,
					 struct net_buf *buf);
 ...
 *  @return 0 on success or error code.
 */
int bt_avrcp_tg_send_add_to_now_playing_rsp(struct bt_avrcp_tg *tg, uint8_t tid, uint8_t status);
 ...
 *  @param tid The transaction label of the response, valid from 0 to 15.
 *  @param buf The response buffer containing the Change Path response parameters
 *             (Status, NumberOfItems) encoded in big-endian format as defined by
 *             the AVRCP specification.
 *
 *  @return 0 in case of success or error code in case of error.
 */
 ...
 *  @return 0 in case of success or error code in case of error.
 */
int bt_avrcp_tg_send_get_total_number_of_items_rsp(struct bt_avrcp_tg *tg, uint8_t tid, struct net_buf *buf);
 ...
 */
int bt_avrcp_tg_send_browsing_general_reject_rsp(struct bt_avrcp_tg *tg, uint8_t tid, uint8_t status);
#ifdef __cplusplus
}
#endif
