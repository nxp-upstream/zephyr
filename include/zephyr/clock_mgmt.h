...
#include <zephyr/clock_mgmt/clock_driver.h>
#include <errno.h>
 ...
 */
#define Z_CLOCK_MGMT_DEFINE_OUTPUT(node_id)                                    \
	struct clock_mgmt_clk_api _CONCAT(node_id, _clk_api);                  \
	CLOCK_DT_DEFINE(node_id, NULL, NULL, &_CONCAT(node_id, _clk_api));
...
#define CLOCK_MGMT_DEFINE(node_id)					       \
	...
	IF_ENABLED(DT_CLOCK_USED(node_id), Z_CLOCK_MGMT_DEFINE_OUTPUT(node_id))
