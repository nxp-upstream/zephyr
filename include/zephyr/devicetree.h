...
/**
 * @brief Invoke @p fn for each direct dependency of a given node
 *
 * This macro expands to an invocation of @p fn for each direct dependency
 * of the node with @p node_id. Each invocation of @p fn is separated with a
 * space.
 * Example devicetree fragment:
 *	node_a: node-a {
 *              vnd,prop = <0>;
 ...
 *
 * @code{.c}
 *     #define GET_PROP(node) DT_PROP(node, vnd_prop)
 *     DT_FOREACH_SUPPORTED_NODE(DT_NODELABEL(node_a), GET_PROP)
 * @endcode
 ...
 *
 * @param node_id node identifier
 * @param fn macro to invoke on each direct dependent node
 */
#define DT_FOREACH_SUPPORTED_NODE(node_id, fn) \
	DT_CAT(node_id, _FOREACH_SUPPORTED_NODE)(fn)

/**
 * @brief Invoke @p fn for each node that depends directly on the instance @p inst
 * Equivalent to `DT_FOREACH_SUPPORTED_NODE(DT_DRV_INST(inst), fn)`
 *
 * @param inst instance number
 * @param fn macro to invoke on each direct dependent node
 */
#define DT_INST_FOREACH_SUPPORTED_NODE(inst, fn) \
	DT_FOREACH_SUPPORTED_NODE(DT_DRV_INST(inst), fn)

/**
 * @brief Invoke @p fn for each node @p node_id depends on
 *
 * This macro expands to an invocation of @p fn for each node that @p node_id
 * directly depends on. Each invocation of @p fn is separated with a
 * space.
 * Example devicetree fragment:
 *	node_a: node-a {
 *              vnd,prop = <0>;
 ...
 * @code{.c}
 *     #define GET_PROP(node) DT_PROP(node, vnd_prop)
 *     DT_FOREACH_REQUIRED_NODE(DT_NODELABEL(node_c), GET_PROP)
 * @endcode
 * This would expand to the following:
 * @code{.c}
 *     1 3
 ...
 *
 * @param node_id node identifier
 * @param fn macro to invoke on each node this node depends on directly
 */
#define DT_FOREACH_REQUIRED_NODE(node_id, fn) \
	DT_CAT(node_id, _FOREACH_REQUIRED_NODE)(fn)

/**
 * @brief Invoke @p fn for each node that instance @p inst depends on directly
 * Equivalent to `DT_FOREACH_REQUIRED_NODE(DT_DRV_INST(inst), fn)`
 *
 * @param inst instance number
 * @param fn macro to invoke on each node this instance depends on directly
 */
#define DT_INST_FOREACH_REQUIRED_NODE(inst, fn) \
	DT_FOREACH_REQUIRED_NODE(DT_DRV_INST(inst), fn)
