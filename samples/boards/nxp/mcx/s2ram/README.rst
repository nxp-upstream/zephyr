.. zephyr:code-sample:: nxp_mcx_s2ram
   ...
   Suspend an NXP MCXA/MCXN SoC to RAM and resume after a timed WUU wakeup.
...
blocks waiting for its next event, with no knowledge of Deep Power Down. Whenever
it (and every other thread) is blocked, the system PM policy suspends the SoC to
RAM; the wakeup resumes the thread transparently, right where it blocked. A
counter incremented before every suspend survives the cycle, so the printed value
demonstrates the retained RAM.

The worker thread waits for a console key press before each suspend. Until a key
is pressed the SoC stays awake and debuggable, so a freshly programmed board
never bricks its debug port by entering Deep Power Down on its own before it can
be re-flashed.

The wakeup source is configured through the :ref:`WUC (Wakeup Controller)
<wuc_api>` subsystem and is selectable at build time:

* ``CONFIG_S2RAM_WAKEUP_TIMER`` (default) arms LPTMR0 through the counter alarm
...
* ``CONFIG_S2RAM_WAKEUP_BUTTON`` resumes on a WUU external pin transition (the
...
.. zephyr-app-commands::
   :zephyr-app: samples/boards/nxp/mcx/s2ram
   :board: frdm_mcxa156
   :goals: build flash
   :compact:
...
.. code-block:: console
   ...
   Press any key to enter suspend-to-RAM (cycle 1)
   Entering suspend-to-RAM; wake in 3 s
   Resumed from suspend-to-RAM; retained counter is 1
   Press any key to enter suspend-to-RAM (cycle 2)
   Entering suspend-to-RAM; wake in 3 s
   Resumed from suspend-to-RAM; retained counter is 2
   ...

With ``CONFIG_S2RAM_WAKEUP_BUTTON`` each cycle instead waits for an SW2 press:
...
.. code-block:: console
   ...
   Retained S2RAM cycle counter: 0
   Press any key to enter suspend-to-RAM (cycle 1)
   Entering suspend-to-RAM; press SW2 to wake
   Resumed from suspend-to-RAM; retained counter is 1
   Press any key to enter suspend-to-RAM (cycle 2)
   Entering suspend-to-RAM; press SW2 to wake
   Resumed from suspend-to-RAM; retained counter is 2
