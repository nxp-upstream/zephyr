					...
					const char *arguments, const char *execution_token)
{
	...
	if (action && strcmp(action, "on") == 0) {
		...
		snprintf(response_buffer, sizeof(response_buffer), "LED turned ON");
	} else if (action && strcmp(action, "off") == 0) {
		ret = gpio_pin_set_dt(&led, 0);
		snprintf(response_buffer, sizeof(response_buffer), "LED turned OFF");
	} else if (action && strcmp(action, "toggle") == 0) {
		ret = gpio_pin_toggle_dt(&led);
		snprintf(response_buffer, sizeof(response_buffer), "LED toggled");
	} else {
