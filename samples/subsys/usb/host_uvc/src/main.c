...
static int wait_for_video_connection(const struct device *uvc_dev, struct video_format *fmt,
				 ...
				 bool *display_configured, struct video_format *fmt)
{
	...
	/* Get capabilities */
	caps.type = type;
	if (video_get_caps(uvc_dev, &caps)) {
	...
	if (video_set_selection(uvc_dev, &sel)) {
	...
	sel.target = VIDEO_SEL_TGT_CROP;
	err = video_get_selection(uvc_dev, &sel);
	if (err < 0 && err != -ENOSYS) {
	...
	if (err == 0 && (sel.rect.width != fmt->width || sel.rect.height != fmt->height)) {
		...
		err = video_set_selection(uvc_dev, &sel);
		if (err < 0 && err != -ENOSYS) {
	...
	if (video_set_format(uvc_dev, fmt)) {
		LOG_ERR("Unable to set format");
		return -1;
	}

	if (!video_get_frmival(uvc_dev, &frmival)) {
	...
	frmival.numerator = 1;
	if (!video_set_frmival(uvc_dev, &frmival)) {
		if (!video_get_frmival(uvc_dev, &frmival)) {
	...
	if (video_stream_start(uvc_dev, type)) {
...
static void cleanup_video_streaming(const struct device *uvc_dev,
				    struct video_buffer *allocated_vbufs[], int *allocated_count,
				    enum video_buf_type type)
{
	LOG_INF("Cleaning up video streaming resources...");

	video_stream_stop(uvc_dev, type);
...
int main(void)
{
	...
	if (!device_is_ready(uvc_dev)) {
		LOG_ERR("%s: USB host is not ready", uvc_dev->name);
		return 0;
	}
	LOG_INF("USB host: %s", uvc_dev->name);
	...
	while (true) {
		/* Wait for video device connection */
		err = wait_for_video_connection(uvc_dev, &fmt, type);
		if (err != 0) {
			continue;
		}

		/* Setup and start streaming */
		err = setup_video_streaming(uvc_dev, allocated_vbufs, &allocated_count,
		...
		cleanup_video_streaming(uvc_dev, allocated_vbufs, &allocated_count, type);
