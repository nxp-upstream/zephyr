...
int main(void)
{
	...
	/* Set caps filter to constrain negotiation with JPEG format + resolution */
	{
		struct mp_caps *caps = mp_caps_new(
			MP_MEDIA_VIDEO, MP_CAPS_PIXEL_FORMAT, MP_INT(VIDEO_PIX_FMT_JPEG),
			MP_CAPS_IMAGE_WIDTH, MP_INT(CONFIG_JPEG_IMAGE_WIDTH),
			MP_CAPS_IMAGE_HEIGHT, MP_INT(CONFIG_JPEG_IMAGE_HEIGHT), MP_CAPS_END);
