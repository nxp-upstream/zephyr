...
void draw_cube(vg_lite_buffer_t *rt)
{
	...
	if (nz0321 > 0.0f) {
		draw_path(&image, 0xFFFF0000, tiger_path, tiger_color_data, tiger_path_count, 90.0f,
	...
	if (nz4567 > 0.0f) {
		...
		if (draw_path(&image, 0xFFFFFFFF, nxp_path, nxp_color_data, nxp_path_count, 25.0f,
	...
	if (nz5126 > 0.0f) {
		draw_path(&image, 0xFFE7BFC8, tiger_path, tiger_color_data, tiger_path_count, 90.0f,
	...
	if (nz7623 > 0.0f) {
		draw_path(&image, 0xFFEAD999, tiger_path, tiger_color_data, tiger_path_count, 90.0f,
