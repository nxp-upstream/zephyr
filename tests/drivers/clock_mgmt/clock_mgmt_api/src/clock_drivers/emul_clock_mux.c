...
static int emul_clock_mux_get_rate(const struct clk *clk)
{
	struct emul_clock_mux *data = clk->hw_data;

	return clock_get_rate(data->parents[data->src_sel]);
}

static int emul_clock_mux_configure(const struct clk *clk, const void *mux)
{
	struct emul_clock_mux *data = clk->hw_data;
	int ret;
	...
	ret = clock_notify_children(clk, clock_get_rate(data->parents[(uint32_t)mux]));
	if (ret < 0) {
		return ret;
	}
...
static int emul_clock_mux_notify(const struct clk *clk, const struct clk *parent,
...
static int emul_clock_mux_round_rate(const struct clk *clk, uint32_t rate)
{
	struct emul_clock_mux *data = clk->hw_data;
	int cand_rate;
...
static int emul_clock_mux_set_rate(const struct clk *clk, uint32_t rate)
{
	struct emul_clock_mux *data = clk->hw_data;
	...
	best_rate = clock_set_rate(data->parents[best_idx], rate, clk);
	if (best_rate < 0) {
		return best_rate;
	}
	...
	clock_unlock(data->parents[data->src_sel], clk);
