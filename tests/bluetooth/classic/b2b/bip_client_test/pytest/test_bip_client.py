...
"""Bluetooth Classic BIP client b2b tests: BIP client (DUT) against BIP server (harness device)."""
...
def test_BR_BIP_CLIENT_RFCOMM_CONNECT_SUCCESS(bip_client, bip_server):
    """Test BR BIP Client RFCOMM transport connection success."""
...
def test_BR_BIP_CLIENT_RFCOMM_CONNECT_FAILURE(bip_client, bip_server):
    """Test BR BIP Client RFCOMM transport connection failure with invalid channel."""
    bip_br_clear(bip_client, bip_server)
    bip_br_connect(bip_client, bip_server)
    invalid_channel = 8
...
def test_BR_BIP_CLIENT_L2CAP_CONNECT_SUCCESS(bip_client, bip_server):
    """Test BR BIP Client L2CAP transport connection success."""
    bip_br_clear(bip_client, bip_server)
    bip_br_connect(bip_client, bip_server)
...
def test_BR_BIP_CLIENT_L2CAP_CONNECT_FAILURE(bip_client, bip_server):
    """Test BR BIP Client L2CAP transport connection failure with invalid PSM."""
    bip_br_clear(bip_client, bip_server)
    bip_br_connect(bip_client, bip_server)
    invalid_psm = '1008'
...
def test_BR_BIP_CLIENT_RFCOMM_DISCONNECT_SUCCESS(bip_client, bip_server):
    """Test BR BIP Client RFCOMM transport disconnection success."""
    bip_br_clear(bip_client, bip_server)
    bip_br_connect(bip_client, bip_server)
    bip_rfcomm_transport_connection(bip_client, bip_server)
...
def test_BR_BIP_CLIENT_L2CAP_DISCONNECT_SUCCESS(bip_client, bip_server):
    """Test BR BIP Client L2CAP transport disconnection success."""
    bip_br_clear(bip_client, bip_server)
    bip_br_connect(bip_client, bip_server)
    bip_l2cap_transport_connection(bip_client, bip_server)
...
def test_BR_BIP_CLIENT_OBEX_CONNECT_SUCCESS(bip_client, bip_server):
    """Test BR BIP Client OBEX connection success."""
    bip_br_clear(bip_client, bip_server)
    bip_br_connect(bip_client, bip_server)
    bip_l2cap_transport_connection(bip_client, bip_server)
...
def test_BR_BIP_CLIENT_OBEX_CONNECT_ERROR(bip_client, bip_server):
    """Test BR BIP Client OBEX connection error - server rejects with Forbidden."""
    bip_br_clear(bip_client, bip_server)
    bip_br_connect(bip_client, bip_server)
    bip_l2cap_transport_connection(bip_client, bip_server)
    bip_client.iexpect('bip client set_feats_funcs', bip_client.shell.prompt)
...
def test_BR_BIP_CLIENT_OBEX_DISCONNECT_SUCCESS(bip_client, bip_server):
    """Test BR BIP Client OBEX disconnection success."""
    bip_br_clear(bip_client, bip_server)
    bip_br_connect(bip_client, bip_server)
    bip_l2cap_transport_connection(bip_client, bip_server)
    bip_obex_connect(bip_client, bip_server, conn_type=0)
...
def test_BR_BIP_CLIENT_OBEX_DISCONNECT_ERROR(bip_client, bip_server):
    """Test BR BIP Client OBEX disconnection error - server rejects."""
    bip_br_clear(bip_client, bip_server)
    bip_br_connect(bip_client, bip_server)
    bip_l2cap_transport_connection(bip_client, bip_server)
    bip_obex_connect(bip_client, bip_server, conn_type=0)
...
def test_BR_BIP_CLIENT_ABORT_SUCCESS(bip_client, bip_server):
    """Test BR BIP Client abort ongoing operation success."""
    bip_br_clear(bip_client, bip_server)
    bip_br_connect(bip_client, bip_server)
    bip_l2cap_transport_connection(bip_client, bip_server)
    bip_obex_connect(bip_client, bip_server, conn_type=0)
...
def test_BR_BIP_CLIENT_ABORT_ERROR(bip_client, bip_server):
    """Test BR BIP Client abort operation error - server rejects."""
    bip_br_clear(bip_client, bip_server)
    bip_br_connect(bip_client, bip_server)
    bip_l2cap_transport_connection(bip_client, bip_server)
    bip_obex_connect(bip_client, bip_server, conn_type=0)
    ...
    for line in lines:
        if re.search(r'get_caps rsp.*Continue', line):
            while True:
                ...
                for l in lines2:
                    if re.search(r'get_caps rsp.*Success', l):
                        done = True
                        break
                if done:
                    break
...
def test_BR_BIP_CLIENT_GET_CAPS_SUCCESS(bip_client, bip_server):
    """Test BR BIP Client GetCapabilities success."""
    bip_br_clear(bip_client, bip_server)
    bip_br_connect(bip_client, bip_server)
    bip_l2cap_transport_connection(bip_client, bip_server)
    bip_obex_connect(bip_client, bip_server, conn_type=0)
...
def test_BR_BIP_CLIENT_GET_CAPS_CONTINUE(bip_client, bip_server):
    """Test BR BIP Client GetCapabilities with multi-packet Continue responses."""
    bip_br_clear(bip_client, bip_server)
    bip_br_connect(bip_client, bip_server)
    bip_l2cap_transport_connection(bip_client, bip_server)
    bip_obex_connect(bip_client, bip_server, conn_type=0)
...
def test_BR_BIP_CLIENT_GET_CAPS_ERROR(bip_client, bip_server):
    """Test BR BIP Client GetCapabilities error - server rejects."""
    bip_br_clear(bip_client, bip_server)
    bip_br_connect(bip_client, bip_server)
    bip_l2cap_transport_connection(bip_client, bip_server)
    bip_obex_connect(bip_client, bip_server, conn_type=0)
...
def test_BR_BIP_CLIENT_GET_CAPS_SRM(bip_client, bip_server):
    """Test BR BIP Client GetCapabilities with SRM over L2CAP."""
...
def test_BR_BIP_CLIENT_GET_CAPS_SRMP_SERVER(bip_client, bip_server):
    """Test BR BIP Client GetCapabilities with SRMP Wait from server."""
    bip_srm_test_setup(bip_client, bip_server)
    bip_srmp_server_operation(bip_client, bip_server, 'get_caps')
    bip_srm_test_teardown(bip_client, bip_server)


def test_BR_BIP_CLIENT_GET_CAPS_SRMP_CLIENT(bip_client, bip_server):
    """Test BR BIP Client GetCapabilities with SRMP Wait from client."""
    bip_srm_test_setup(bip_client, bip_server)
    bip_srmp_client_operation(bip_client, bip_server, 'get_caps')
    bip_srm_test_teardown(bip_client, bip_server)


def test_BR_BIP_CLIENT_GET_CAPS_ABORT(bip_client, bip_server):
    """Test BR BIP Client abort during GetCapabilities."""
    bip_br_clear(bip_client, bip_server)
    bip_br_connect(bip_client, bip_server)
    bip_l2cap_transport_connection(bip_client, bip_server)
    bip_obex_connect(bip_client, bip_server, conn_type=0)
...
def test_BR_BIP_CLIENT_PUT_IMAGE_SUCCESS(bip_client, bip_server):
    """Test BR BIP Client PutImage success."""
    bip_br_clear(bip_client, bip_server)
    bip_br_connect(bip_client, bip_server)
    bip_l2cap_transport_connection(bip_client, bip_server)
    bip_obex_connect(bip_client, bip_server, conn_type=0)
...
def test_BR_BIP_CLIENT_PUT_IMAGE_ERROR(bip_client, bip_server):
    """Test BR BIP Client PutImage error - server rejects."""
    bip_br_clear(bip_client, bip_server)
    bip_br_connect(bip_client, bip_server)
    bip_l2cap_transport_connection(bip_client, bip_server)
    bip_obex_connect(bip_client, bip_server, conn_type=0)
...
def bip_secondary_server_reg(bip_client, conn_type):
    """Register secondary server on DUT (INITIATOR) side."""
    bip_client.exec_command('bip_client_test sec_unreg')
    bip_client.iexpect(f'bip_client_test sec_reg {conn_type}', bip_client.shell.prompt)


def bip_secondary_server_unreg(bip_client):
    """Unregister secondary server on DUT side."""
    bip_client.exec_command('bip_client_test sec_unreg')


def bip_secondary_obex_connect(bip_client, bip_server, conn_type):
    """Secondary OBEX connect: Harness=secondary client, DUT=secondary server."""
    bip_server.iexpect('bip_client_test sec_set_feats_funcs 0x1ff 0x1ffff',
                       bip_server.shell.prompt)
    bip_server.iexpect('bip alloc-buf', bip_server.shell.prompt)
    bip_server.exec_command(f'bip_client_test sec_conn {conn_type}')
    bip_client._wait_for_shell_response(r'BIP server.*conn req', timeout=10)
    bip_client.iexpect('bip alloc-buf', bip_client.shell.prompt)
    bip_client.exec_command('bip_client_test sec_server_conn success')
...
def bip_partial_image_test_setup(bip_client, bip_server):
    ...
    Secondary roles: Harness=client, DUT=server (reversed from primary)."""
    bip_br_clear(bip_client, bip_server)
...
def bip_sec_operation(bip_client, bip_server, op):
    ...
    while True:
        bip_server.iexpect('bip alloc-buf', bip_server.shell.prompt)
        bip_server.exec_command(f'bip_client_test sec_{op}')
        bip_client._wait_for_shell_response(rf'BIP server.*{op} req', timeout=10)
        bip_client.iexpect('bip alloc-buf', bip_client.shell.prompt)
        bip_client.exec_command(f'bip_client_test sec_server_{op} noerror')
...
def bip_sec_operation_error(bip_client, bip_server, op,
                            error_code='c3', error_str='Forbidden'):
    """Perform secondary operation that server rejects with error."""
    bip_server.iexpect('bip alloc-buf', bip_server.shell.prompt)
    bip_server.exec_command(f'bip_client_test sec_{op}')
    bip_client._wait_for_shell_response(rf'BIP server.*{op} req', timeout=10)
    bip_client.iexpect('bip alloc-buf', bip_client.shell.prompt)
    bip_client.exec_command(f'bip_client_test sec_server_{op} error {error_code}')
...
def bip_sec_start_op_then_abort(bip_client, bip_server, op):
    ...
    bip_client._wait_for_shell_response(rf'BIP server.*{op} req', timeout=10)
    bip_client.iexpect('bip alloc-buf', bip_client.shell.prompt)
    bip_client.exec_command(f'bip_client_test sec_server_{op} error 90')
    bip_server._wait_for_shell_response(rf'{op} rsp.*Continue', timeout=10)
    bip_server.iexpect('bip alloc-buf', bip_server.shell.prompt)
    bip_server.exec_command('bip_client_test sec_abort')
    bip_client._wait_for_shell_response(r'BIP server.*abort req', timeout=10)
    bip_client.iexpect('bip alloc-buf', bip_client.shell.prompt)
    bip_client.exec_command('bip_client_test sec_server_abort success')
...
def test_BR_BIP_CLIENT_PUT_IMAGE_CONTINUE(bip_client, bip_server):
    """Test BR BIP Client PutImage with multi-packet Continue responses."""
...
def test_BR_BIP_CLIENT_PUT_IMAGE_SRM(bip_client, bip_server):
    """Test BR BIP Client PutImage with SRM over L2CAP."""
    bip_srm_test_setup(bip_client, bip_server)
    bip_srm_operation(bip_client, bip_server, 'put_image')
    bip_srm_test_teardown(bip_client, bip_server)


def test_BR_BIP_CLIENT_PUT_IMAGE_SRMP_SERVER(bip_client, bip_server):
    """Test BR BIP Client PutImage with SRMP Wait from server."""
    bip_srm_test_setup(bip_client, bip_server)
    bip_srmp_server_operation(bip_client, bip_server, 'put_image')
    bip_srm_test_teardown(bip_client, bip_server)


def test_BR_BIP_CLIENT_PUT_IMAGE_SRMP_CLIENT(bip_client, bip_server):
    """Test BR BIP Client PutImage with SRMP Wait from client."""
    bip_srm_test_setup(bip_client, bip_server)
    bip_srmp_client_operation(bip_client, bip_server, 'put_image')
    bip_srm_test_teardown(bip_client, bip_server)


def test_BR_BIP_CLIENT_PUT_LINKED_THUMBNAIL_SUCCESS(bip_client, bip_server):
    """Test BR BIP Client PutLinkedThumbnail success."""
    bip_test_setup(bip_client, bip_server)
    bip_put_image(bip_client, bip_server)
...
def test_BR_BIP_CLIENT_PUT_LINKED_THUMBNAIL_ERROR(bip_client, bip_server):
    """Test BR BIP Client PutLinkedThumbnail error."""
    bip_test_setup(bip_client, bip_server)
    bip_put_image(bip_client, bip_server)
...
def test_BR_BIP_CLIENT_PUT_LINKED_THUMBNAIL_CONTINUE(bip_client, bip_server):
    """Test BR BIP Client PutLinkedThumbnail with Continue."""
    bip_test_setup(bip_client, bip_server)
    bip_put_image(bip_client, bip_server)
    bip_operation(bip_client, bip_server, 'put_linked_thumbnail')
    bip_test_teardown(bip_client, bip_server)
...
def test_BR_BIP_CLIENT_PUT_LINKED_THUMBNAIL_SRM(bip_client, bip_server):
    """Test BR BIP Client PutLinkedThumbnail with SRM."""
    bip_srm_test_setup(bip_client, bip_server)
    bip_put_image(bip_client, bip_server)
...
def test_BR_BIP_CLIENT_PUT_LINKED_THUMBNAIL_SRMP_SERVER(bip_client, bip_server):
    """Test BR BIP Client PutLinkedThumbnail with SRMP Wait from server."""
    bip_srm_test_setup(bip_client, bip_server)
    bip_put_image(bip_client, bip_server)
...
def test_BR_BIP_CLIENT_PUT_LINKED_THUMBNAIL_SRMP_CLIENT(bip_client, bip_server):
    """Test BR BIP Client PutLinkedThumbnail with SRMP Wait from client."""
    bip_srm_test_setup(bip_client, bip_server)
    bip_put_image(bip_client, bip_server)
...
def test_BR_BIP_CLIENT_PUT_LINKED_ATTACHMENT_SUCCESS(bip_client, bip_server):
    """Test BR BIP Client PutLinkedAttachment success."""
    bip_test_setup(bip_client, bip_server)
    bip_put_image(bip_client, bip_server)
...
def test_BR_BIP_CLIENT_PUT_LINKED_ATTACHMENT_ERROR(bip_client, bip_server):
    """Test BR BIP Client PutLinkedAttachment error."""
    bip_test_setup(bip_client, bip_server)
    bip_put_image(bip_client, bip_server)
...
def test_BR_BIP_CLIENT_PUT_LINKED_ATTACHMENT_CONTINUE(bip_client, bip_server):
    """Test BR BIP Client PutLinkedAttachment with Continue."""
    bip_test_setup(bip_client, bip_server)
    bip_put_image(bip_client, bip_server)
    bip_operation(bip_client, bip_server, 'put_linked_attachment')
    bip_test_teardown(bip_client, bip_server)
...
def test_BR_BIP_CLIENT_PUT_LINKED_ATTACHMENT_SRM(bip_client, bip_server):
    """Test BR BIP Client PutLinkedAttachment with SRM."""
    bip_srm_test_setup(bip_client, bip_server)
    bip_put_image(bip_client, bip_server)
...
def test_BR_BIP_CLIENT_PUT_LINKED_ATTACHMENT_SRMP_SERVER(bip_client, bip_server):
    """Test BR BIP Client PutLinkedAttachment with SRMP Wait from server."""
    bip_srm_test_setup(bip_client, bip_server)
    bip_put_image(bip_client, bip_server)
...
def test_BR_BIP_CLIENT_PUT_LINKED_ATTACHMENT_SRMP_CLIENT(bip_client, bip_server):
    """Test BR BIP Client PutLinkedAttachment with SRMP Wait from client."""
    bip_srm_test_setup(bip_client, bip_server)
    bip_put_image(bip_client, bip_server)
...
def test_BR_BIP_CLIENT_GET_IMAGE_LIST_SUCCESS(bip_client, bip_server):
    """Test BR BIP Client GetImageList success."""
...
def test_BR_BIP_CLIENT_GET_IMAGE_LIST_ERROR(bip_client, bip_server):
    """Test BR BIP Client GetImageList error."""
    bip_test_setup(bip_client, bip_server, conn_type=1)
    bip_operation_error(bip_client, bip_server, 'get_image_list')
    bip_test_teardown(bip_client, bip_server, conn_type=1)


def test_BR_BIP_CLIENT_GET_IMAGE_LIST_CONTINUE(bip_client, bip_server):
    """Test BR BIP Client GetImageList with multi-packet Continue responses."""
    bip_test_setup(bip_client, bip_server, conn_type=1)
    bip_operation(bip_client, bip_server, 'get_image_list')
    bip_test_teardown(bip_client, bip_server, conn_type=1)


def test_BR_BIP_CLIENT_GET_IMAGE_LIST_SRM(bip_client, bip_server):
    """Test BR BIP Client GetImageList with SRM over L2CAP."""
...
def test_BR_BIP_CLIENT_GET_IMAGE_LIST_SRMP_SERVER(bip_client, bip_server):
    """Test BR BIP Client GetImageList with SRMP Wait from server."""
    bip_srm_test_setup(bip_client, bip_server, conn_type=1)
    bip_srmp_server_operation(bip_client, bip_server, 'get_image_list')
    bip_srm_test_teardown(bip_client, bip_server, conn_type=1)


def test_BR_BIP_CLIENT_GET_IMAGE_LIST_SRMP_CLIENT(bip_client, bip_server):
    """Test BR BIP Client GetImageList with SRMP Wait from client."""
    bip_srm_test_setup(bip_client, bip_server, conn_type=1)
    bip_srmp_client_operation(bip_client, bip_server, 'get_image_list')
    bip_srm_test_teardown(bip_client, bip_server, conn_type=1)


def test_BR_BIP_CLIENT_GET_IMAGE_LIST_ABORT(bip_client, bip_server):
    """Test BR BIP Client GetImageList abort during operation."""
    bip_test_setup(bip_client, bip_server, conn_type=1)
    bip_start_op_then_abort(bip_client, bip_server, 'get_image_list')
    bip_test_teardown(bip_client, bip_server, conn_type=1)


def test_BR_BIP_CLIENT_GET_IMAGE_PROPERTIES_SUCCESS(bip_client, bip_server):
    """Test BR BIP Client GetImageProperties success."""
    bip_test_setup(bip_client, bip_server, conn_type=1)
    bip_operation(bip_client, bip_server, 'get_image_properties')
    bip_test_teardown(bip_client, bip_server, conn_type=1)


def test_BR_BIP_CLIENT_GET_IMAGE_PROPERTIES_ERROR(bip_client, bip_server):
    """Test BR BIP Client GetImageProperties error."""
    bip_test_setup(bip_client, bip_server, conn_type=1)
    bip_operation_error(bip_client, bip_server, 'get_image_properties')
    bip_test_teardown(bip_client, bip_server, conn_type=1)


def test_BR_BIP_CLIENT_GET_IMAGE_PROPERTIES_CONTINUE(bip_client, bip_server):
    """Test BR BIP Client GetImageProperties with multi-packet Continue responses."""
    bip_test_setup(bip_client, bip_server, conn_type=1)
    bip_operation(bip_client, bip_server, 'get_image_properties')
    bip_test_teardown(bip_client, bip_server, conn_type=1)


def test_BR_BIP_CLIENT_GET_IMAGE_PROPERTIES_SRM(bip_client, bip_server):
    """Test BR BIP Client GetImageProperties with SRM over L2CAP."""
    bip_srm_test_setup(bip_client, bip_server, conn_type=1)
    bip_srm_operation(bip_client, bip_server, 'get_image_properties')
    bip_srm_test_teardown(bip_client, bip_server, conn_type=1)


def test_BR_BIP_CLIENT_GET_IMAGE_PROPERTIES_SRMP_SERVER(bip_client, bip_server):
    """Test BR BIP Client GetImageProperties with SRMP Wait from server."""
    bip_srm_test_setup(bip_client, bip_server, conn_type=1)
    bip_srmp_server_operation(bip_client, bip_server, 'get_image_properties')
    bip_srm_test_teardown(bip_client, bip_server, conn_type=1)


def test_BR_BIP_CLIENT_GET_IMAGE_PROPERTIES_SRMP_CLIENT(bip_client, bip_server):
    """Test BR BIP Client GetImageProperties with SRMP Wait from client."""
    bip_srm_test_setup(bip_client, bip_server, conn_type=1)
    bip_srmp_client_operation(bip_client, bip_server, 'get_image_properties')
    bip_srm_test_teardown(bip_client, bip_server, conn_type=1)


def test_BR_BIP_CLIENT_GET_IMAGE_PROPERTIES_ABORT(bip_client, bip_server):
    """Test BR BIP Client GetImageProperties abort during operation."""
    bip_test_setup(bip_client, bip_server, conn_type=1)
...
def test_BR_BIP_CLIENT_GET_IMAGE_SUCCESS(bip_client, bip_server):
    """Test BR BIP Client GetImage success."""
    bip_test_setup(bip_client, bip_server, conn_type=1)
    bip_operation(bip_client, bip_server, 'get_image')
    bip_test_teardown(bip_client, bip_server, conn_type=1)


def test_BR_BIP_CLIENT_GET_IMAGE_ERROR(bip_client, bip_server):
    """Test BR BIP Client GetImage error."""
    bip_test_setup(bip_client, bip_server, conn_type=1)
    bip_operation_error(bip_client, bip_server, 'get_image')
    bip_test_teardown(bip_client, bip_server, conn_type=1)


def test_BR_BIP_CLIENT_GET_IMAGE_CONTINUE(bip_client, bip_server):
    """Test BR BIP Client GetImage with multi-packet Continue responses."""
    bip_test_setup(bip_client, bip_server, conn_type=1)
    bip_operation(bip_client, bip_server, 'get_image')
    bip_test_teardown(bip_client, bip_server, conn_type=1)


def test_BR_BIP_CLIENT_GET_IMAGE_SRM(bip_client, bip_server):
    """Test BR BIP Client GetImage with SRM over L2CAP."""
    bip_srm_test_setup(bip_client, bip_server, conn_type=1)
    bip_srm_operation(bip_client, bip_server, 'get_image')
    bip_srm_test_teardown(bip_client, bip_server, conn_type=1)


def test_BR_BIP_CLIENT_GET_IMAGE_SRMP_SERVER(bip_client, bip_server):
    """Test BR BIP Client GetImage with SRMP Wait from server."""
    bip_srm_test_setup(bip_client, bip_server, conn_type=1)
    bip_srmp_server_operation(bip_client, bip_server, 'get_image')
    bip_srm_test_teardown(bip_client, bip_server, conn_type=1)


def test_BR_BIP_CLIENT_GET_IMAGE_SRMP_CLIENT(bip_client, bip_server):
    """Test BR BIP Client GetImage with SRMP Wait from client."""
    bip_srm_test_setup(bip_client, bip_server, conn_type=1)
    bip_srmp_client_operation(bip_client, bip_server, 'get_image')
    bip_srm_test_teardown(bip_client, bip_server, conn_type=1)


def test_BR_BIP_CLIENT_GET_IMAGE_ABORT(bip_client, bip_server):
    """Test BR BIP Client GetImage abort during operation."""
    bip_test_setup(bip_client, bip_server, conn_type=1)
...
def test_BR_BIP_CLIENT_GET_LINKED_THUMBNAIL_SUCCESS(bip_client, bip_server):
    """Test BR BIP Client GetLinkedThumbnail success."""
    bip_test_setup(bip_client, bip_server, conn_type=1)
    bip_operation(bip_client, bip_server, 'get_linked_thumbnail')
    bip_test_teardown(bip_client, bip_server, conn_type=1)


def test_BR_BIP_CLIENT_GET_LINKED_THUMBNAIL_ERROR(bip_client, bip_server):
    """Test BR BIP Client GetLinkedThumbnail error."""
    bip_test_setup(bip_client, bip_server, conn_type=1)
    bip_operation_error(bip_client, bip_server, 'get_linked_thumbnail')
    bip_test_teardown(bip_client, bip_server, conn_type=1)


def test_BR_BIP_CLIENT_GET_LINKED_THUMBNAIL_CONTINUE(bip_client, bip_server):
    """Test BR BIP Client GetLinkedThumbnail with multi-packet Continue responses."""
    bip_test_setup(bip_client, bip_server, conn_type=1)
    bip_operation(bip_client, bip_server, 'get_linked_thumbnail')
    bip_test_teardown(bip_client, bip_server, conn_type=1)


def test_BR_BIP_CLIENT_GET_LINKED_THUMBNAIL_SRM(bip_client, bip_server):
    """Test BR BIP Client GetLinkedThumbnail with SRM over L2CAP."""
    bip_srm_test_setup(bip_client, bip_server, conn_type=1)
    bip_srm_operation(bip_client, bip_server, 'get_linked_thumbnail')
    bip_srm_test_teardown(bip_client, bip_server, conn_type=1)


def test_BR_BIP_CLIENT_GET_LINKED_THUMBNAIL_SRMP_SERVER(bip_client, bip_server):
    """Test BR BIP Client GetLinkedThumbnail with SRMP Wait from server."""
    bip_srm_test_setup(bip_client, bip_server, conn_type=1)
    bip_srmp_server_operation(bip_client, bip_server, 'get_linked_thumbnail')
    bip_srm_test_teardown(bip_client, bip_server, conn_type=1)


def test_BR_BIP_CLIENT_GET_LINKED_THUMBNAIL_SRMP_CLIENT(bip_client, bip_server):
    """Test BR BIP Client GetLinkedThumbnail with SRMP Wait from client."""
    bip_srm_test_setup(bip_client, bip_server, conn_type=1)
    bip_srmp_client_operation(bip_client, bip_server, 'get_linked_thumbnail')
    bip_srm_test_teardown(bip_client, bip_server, conn_type=1)


def test_BR_BIP_CLIENT_GET_LINKED_THUMBNAIL_ABORT(bip_client, bip_server):
    """Test BR BIP Client GetLinkedThumbnail abort during operation."""
    bip_test_setup(bip_client, bip_server, conn_type=1)
...
def test_BR_BIP_CLIENT_GET_LINKED_ATTACHMENT_SUCCESS(bip_client, bip_server):
    """Test BR BIP Client GetLinkedAttachment success."""
    bip_test_setup(bip_client, bip_server, conn_type=1)
    bip_operation(bip_client, bip_server, 'get_linked_attachment')
    bip_test_teardown(bip_client, bip_server, conn_type=1)


def test_BR_BIP_CLIENT_GET_LINKED_ATTACHMENT_ERROR(bip_client, bip_server):
    """Test BR BIP Client GetLinkedAttachment error."""
    bip_test_setup(bip_client, bip_server, conn_type=1)
    bip_operation_error(bip_client, bip_server, 'get_linked_attachment')
    bip_test_teardown(bip_client, bip_server, conn_type=1)


def test_BR_BIP_CLIENT_GET_LINKED_ATTACHMENT_CONTINUE(bip_client, bip_server):
    """Test BR BIP Client GetLinkedAttachment with multi-packet Continue responses."""
    bip_test_setup(bip_client, bip_server, conn_type=1)
    bip_operation(bip_client, bip_server, 'get_linked_attachment')
    bip_test_teardown(bip_client, bip_server, conn_type=1)


def test_BR_BIP_CLIENT_GET_LINKED_ATTACHMENT_SRM(bip_client, bip_server):
    """Test BR BIP Client GetLinkedAttachment with SRM over L2CAP."""
    bip_srm_test_setup(bip_client, bip_server, conn_type=1)
    bip_srm_operation(bip_client, bip_server, 'get_linked_attachment')
    bip_srm_test_teardown(bip_client, bip_server, conn_type=1)


def test_BR_BIP_CLIENT_GET_LINKED_ATTACHMENT_SRMP_SERVER(bip_client, bip_server):
    """Test BR BIP Client GetLinkedAttachment with SRMP Wait from server."""
    bip_srm_test_setup(bip_client, bip_server, conn_type=1)
    bip_srmp_server_operation(bip_client, bip_server, 'get_linked_attachment')
    bip_srm_test_teardown(bip_client, bip_server, conn_type=1)


def test_BR_BIP_CLIENT_GET_LINKED_ATTACHMENT_SRMP_CLIENT(bip_client, bip_server):
    """Test BR BIP Client GetLinkedAttachment with SRMP Wait from client."""
    bip_srm_test_setup(bip_client, bip_server, conn_type=1)
    bip_srmp_client_operation(bip_client, bip_server, 'get_linked_attachment')
    bip_srm_test_teardown(bip_client, bip_server, conn_type=1)


def test_BR_BIP_CLIENT_GET_LINKED_ATTACHMENT_ABORT(bip_client, bip_server):
    """Test BR BIP Client GetLinkedAttachment abort during operation."""
    bip_test_setup(bip_client, bip_server, conn_type=1)
...
def test_BR_BIP_CLIENT_DELETE_IMAGE_SUCCESS(bip_client, bip_server):
    """Test BR BIP Client DeleteImage success."""
    bip_test_setup(bip_client, bip_server, conn_type=1)
    bip_operation(bip_client, bip_server, 'delete_image')
    bip_test_teardown(bip_client, bip_server, conn_type=1)


def test_BR_BIP_CLIENT_DELETE_IMAGE_ERROR(bip_client, bip_server):
    """Test BR BIP Client DeleteImage error."""
    bip_test_setup(bip_client, bip_server, conn_type=1)
...
def test_BR_BIP_CLIENT_REMOTE_DISPLAY_SUCCESS(bip_client, bip_server):
    """Test BR BIP Client RemoteDisplay success."""
...
def test_BR_BIP_CLIENT_REMOTE_DISPLAY_ERROR(bip_client, bip_server):
    """Test BR BIP Client RemoteDisplay error."""
    bip_test_setup(bip_client, bip_server, conn_type=5)
...
def test_BR_BIP_CLIENT_GET_MONITORING_IMAGE_SUCCESS(bip_client, bip_server):
    """Test BR BIP Client GetMonitoringImage success."""
...
def test_BR_BIP_CLIENT_GET_MONITORING_IMAGE_ERROR(bip_client, bip_server):
    """Test BR BIP Client GetMonitoringImage error."""
    bip_test_setup(bip_client, bip_server, conn_type=4)
    bip_operation_error(bip_client, bip_server, 'get_monitoring_image')
    bip_test_teardown(bip_client, bip_server, conn_type=4)


def test_BR_BIP_CLIENT_GET_MONITORING_IMAGE_CONTINUE(bip_client, bip_server):
    """Test BR BIP Client GetMonitoringImage with multi-packet Continue responses."""
    bip_test_setup(bip_client, bip_server, conn_type=4)
    bip_operation(bip_client, bip_server, 'get_monitoring_image')
    bip_test_teardown(bip_client, bip_server, conn_type=4)


def test_BR_BIP_CLIENT_GET_MONITORING_IMAGE_SRM(bip_client, bip_server):
    """Test BR BIP Client GetMonitoringImage with SRM over L2CAP."""
...
def test_BR_BIP_CLIENT_GET_MONITORING_IMAGE_SRMP_SERVER(bip_client, bip_server):
    """Test BR BIP Client GetMonitoringImage with SRMP Wait from server."""
    bip_srm_test_setup(bip_client, bip_server, conn_type=4)
    bip_srmp_server_operation(bip_client, bip_server, 'get_monitoring_image')
    bip_srm_test_teardown(bip_client, bip_server, conn_type=4)


def test_BR_BIP_CLIENT_GET_MONITORING_IMAGE_SRMP_CLIENT(bip_client, bip_server):
    """Test BR BIP Client GetMonitoringImage with SRMP Wait from client."""
    bip_srm_test_setup(bip_client, bip_server, conn_type=4)
    bip_srmp_client_operation(bip_client, bip_server, 'get_monitoring_image')
    bip_srm_test_teardown(bip_client, bip_server, conn_type=4)


def test_BR_BIP_CLIENT_GET_MONITORING_IMAGE_ABORT(bip_client, bip_server):
    """Test BR BIP Client GetMonitoringImage abort during operation."""
    bip_test_setup(bip_client, bip_server, conn_type=4)
...
def test_BR_BIP_CLIENT_GET_PARTIAL_IMAGE_SUCCESS(bip_client, bip_server):
    """Test BR BIP Client GetPartialImage success.
...
def test_BR_BIP_CLIENT_GET_PARTIAL_IMAGE_ERROR(bip_client, bip_server):
    """Test BR BIP Client GetPartialImage error."""
    bip_partial_image_test_setup(bip_client, bip_server)
    bip_sec_operation_error(bip_client, bip_server, 'get_partial_image')
    bip_partial_image_test_teardown(bip_client, bip_server)


def test_BR_BIP_CLIENT_GET_PARTIAL_IMAGE_CONTINUE(bip_client, bip_server):
    """Test BR BIP Client GetPartialImage with Continue."""
    bip_partial_image_test_setup(bip_client, bip_server)
    bip_sec_operation(bip_client, bip_server, 'get_partial_image')
    bip_partial_image_test_teardown(bip_client, bip_server)


def test_BR_BIP_CLIENT_GET_PARTIAL_IMAGE_SRM(bip_client, bip_server):
    """Test BR BIP Client GetPartialImage with SRM over L2CAP."""
    bip_partial_image_test_setup(bip_client, bip_server)
    bip_sec_srm_operation(bip_client, bip_server, 'get_partial_image')
    bip_partial_image_test_teardown(bip_client, bip_server)


def test_BR_BIP_CLIENT_GET_PARTIAL_IMAGE_SRMP_SERVER(bip_client, bip_server):
    """Test BR BIP Client GetPartialImage with SRMP Wait from server."""
    bip_partial_image_test_setup(bip_client, bip_server)
    bip_sec_srmp_server_operation(bip_client, bip_server, 'get_partial_image')
    bip_partial_image_test_teardown(bip_client, bip_server)


def test_BR_BIP_CLIENT_GET_PARTIAL_IMAGE_SRMP_CLIENT(bip_client, bip_server):
    """Test BR BIP Client GetPartialImage with SRMP Wait from client."""
    bip_partial_image_test_setup(bip_client, bip_server)
    bip_sec_srmp_client_operation(bip_client, bip_server, 'get_partial_image')
    bip_partial_image_test_teardown(bip_client, bip_server)


def test_BR_BIP_CLIENT_GET_PARTIAL_IMAGE_ABORT(bip_client, bip_server):
    """Test BR BIP Client GetPartialImage abort."""
    bip_partial_image_test_setup(bip_client, bip_server)
...
def test_BR_BIP_CLIENT_START_PRINT_SUCCESS(bip_client, bip_server):
    """Test BR BIP Client StartPrint success."""
...
def test_BR_BIP_CLIENT_START_PRINT_ERROR(bip_client, bip_server):
    """Test BR BIP Client StartPrint error."""
    bip_test_setup(bip_client, bip_server, conn_type=2)
...
def test_BR_BIP_CLIENT_START_ARCHIVE_SUCCESS(bip_client, bip_server):
    """Test BR BIP Client StartArchive success."""
...
def test_BR_BIP_CLIENT_START_ARCHIVE_ERROR(bip_client, bip_server):
    """Test BR BIP Client StartArchive error."""
    bip_test_setup(bip_client, bip_server, conn_type=3)
...
def test_BR_BIP_CLIENT_GET_STATUS_SUCCESS(bip_client, bip_server):
    """Test BR BIP Client GetStatus success."""
    bip_test_setup(bip_client, bip_server, conn_type=3)
    bip_operation(bip_client, bip_server, 'get_status')
    bip_test_teardown(bip_client, bip_server, conn_type=3)


def test_BR_BIP_CLIENT_GET_STATUS_ERROR(bip_client, bip_server):
    """Test BR BIP Client GetStatus error."""
    bip_test_setup(bip_client, bip_server, conn_type=3)
    bip_operation_error(bip_client, bip_server, 'get_status')
    bip_test_teardown(bip_client, bip_server, conn_type=3)


def test_BR_BIP_CLIENT_GET_STATUS_CONTINUE(bip_client, bip_server):
    """Test BR BIP Client GetStatus with Continue."""
    bip_test_setup(bip_client, bip_server, conn_type=3)
    bip_operation(bip_client, bip_server, 'get_status')
    bip_test_teardown(bip_client, bip_server, conn_type=3)


def test_BR_BIP_CLIENT_GET_STATUS_SRM(bip_client, bip_server):
    """Test BR BIP Client GetStatus with SRM."""
...
def test_BR_BIP_CLIENT_GET_STATUS_SRMP_SERVER(bip_client, bip_server):
    """Test BR BIP Client GetStatus with SRMP Wait from server."""
    bip_srm_test_setup(bip_client, bip_server, conn_type=3)
    bip_srmp_server_operation(bip_client, bip_server, 'get_status')
    bip_srm_test_teardown(bip_client, bip_server, conn_type=3)


def test_BR_BIP_CLIENT_GET_STATUS_SRMP_CLIENT(bip_client, bip_server):
    """Test BR BIP Client GetStatus with SRMP Wait from client."""
    bip_srm_test_setup(bip_client, bip_server, conn_type=3)
    bip_srmp_client_operation(bip_client, bip_server, 'get_status')
    bip_srm_test_teardown(bip_client, bip_server, conn_type=3)


def test_BR_BIP_CLIENT_GET_STATUS_ABORT(bip_client, bip_server):
    """Test BR BIP Client GetStatus abort."""
    bip_test_setup(bip_client, bip_server, conn_type=3)
