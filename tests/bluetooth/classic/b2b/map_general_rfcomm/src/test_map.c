/* map_mce_mas.c - Bluetooth classic MAP test */
...
#define USER_DEFINED_APP_PARAM_HDR 0xFF
#define USER_DEFINED_APP_PARAM                                                                     \
	"Application parameter for MAP Test. This parameter will be included "                     \
	"in the GET request to force multiple packets during transmission. The length of this"     \
	"must fit within MAP_MOPL to ensure proper fragmentation handling."
...
{
	if (tlv->type == USER_DEFINED_APP_PARAM_HDR) {
		bt_shell_print("T %02x L %d V %.*s", tlv->type, tlv->data_len, tlv->data_len,
			       tlv->data);
		return true;
	}
	...
	switch (tlv->type) {
	case BT_MAP_APPL_PARAM_TAG_ID_MAX_LIST_COUNT:
		if (tlv->data_len == 2) {
			bt_shell_print("  MaxListCount: %u", sys_get_be16(tlv->data));
		}
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_LIST_START_OFFSET:
		if (tlv->data_len == 2) {
			bt_shell_print("  ListStartOffset: %u", sys_get_be16(tlv->data));
		}
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_FILTER_MESSAGE_TYPE:
		if (tlv->data_len == 1) {
			uint8_t type = tlv->data[0];
			bt_shell_print("  FilterMessageType: 0x%02x", type);
			bt_shell_print("    SMS_GSM=%s, SMS_CDMA=%s, EMAIL=%s, MMS=%s, IM=%s",
				       (type & BIT(0)) ? "Y" : "N", (type & BIT(1)) ? "Y" : "N",
				       (type & BIT(2)) ? "Y" : "N", (type & BIT(3)) ? "Y" : "N",
				       (type & BIT(4)) ? "Y" : "N");
		}
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_FILTER_PERIOD_BEGIN:
		bt_shell_print("  FilterPeriodBegin: %.*s", tlv->data_len, tlv->data);
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_FILTER_PERIOD_END:
		bt_shell_print("  FilterPeriodEnd: %.*s", tlv->data_len, tlv->data);
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_FILTER_READ_STATUS:
		if (tlv->data_len == 1) {
			const char *status[] = {"no_filtering", "unread_only", "read_only"};
			uint8_t val = tlv->data[0];
			bt_shell_print("  FilterReadStatus: %u (%s)", val,
				       val <= 2 ? status[val] : "invalid");
		}
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_FILTER_RECIPIENT:
		bt_shell_print("  FilterRecipient: %.*s", tlv->data_len, tlv->data);
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_FILTER_ORIGINATOR:
		bt_shell_print("  FilterOriginator: %.*s", tlv->data_len, tlv->data);
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_FILTER_PRIORITY:
		if (tlv->data_len == 1) {
			const char *priority[] = {"no_filtering", "high_priority",
						  "non_high_priority"};
			uint8_t val = tlv->data[0];
			bt_shell_print("  FilterPriority: %u (%s)", val,
				       val <= 2 ? priority[val] : "invalid");
		}
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_ATTACHMENT:
		if (tlv->data_len == 1) {
			bt_shell_print("  Attachment: %s", tlv->data[0] ? "ON" : "OFF");
		}
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_TRANSPARENT:
		if (tlv->data_len == 1) {
			bt_shell_print("  Transparent: %s", tlv->data[0] ? "ON" : "OFF");
		}
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_RETRY:
		if (tlv->data_len == 1) {
			bt_shell_print("  Retry: %s", tlv->data[0] ? "ON" : "OFF");
		}
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_NEW_MESSAGE:
		if (tlv->data_len == 1) {
			bt_shell_print("  NewMessage: %s", tlv->data[0] ? "ON" : "OFF");
		}
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_NOTIFICATION_STATUS:
		if (tlv->data_len == 1) {
			bt_shell_print("  NotificationStatus: %s", tlv->data[0] ? "ON" : "OFF");
		}
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_MAS_INSTANCE_ID:
		if (tlv->data_len == 1) {
			bt_shell_print("  MASInstanceID: %u", tlv->data[0]);
		}
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_PARAMETER_MASK:
		if (tlv->data_len == 4) {
			bt_shell_print("  ParameterMask: 0x%08x", sys_get_be32(tlv->data));
		}
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_FOLDER_LISTING_SIZE:
		if (tlv->data_len == 2) {
			bt_shell_print("  FolderListingSize: %u", sys_get_be16(tlv->data));
		}
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_LISTING_SIZE:
		if (tlv->data_len == 2) {
			bt_shell_print("  ListingSize: %u", sys_get_be16(tlv->data));
		}
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_SUBJECT_LENGTH:
		if (tlv->data_len == 1) {
			bt_shell_print("  SubjectLength: %u", tlv->data[0]);
		}
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_CHARSET:
		if (tlv->data_len == 1) {
			bt_shell_print("  Charset: %s", tlv->data[0] ? "UTF-8" : "Native");
		}
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_FRACTION_REQUEST:
		if (tlv->data_len == 1) {
			bt_shell_print("  FractionRequest: %s", tlv->data[0] ? "Next" : "First");
		}
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_FRACTION_DELIVER:
		if (tlv->data_len == 1) {
			bt_shell_print("  FractionDeliver: %s", tlv->data[0] ? "Last" : "More");
		}
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_STATUS_INDICATOR:
		if (tlv->data_len == 1) {
			const char *status[] = {"ReadStatus", "DeletedStatus", "ExtendedData"};
			uint8_t val = tlv->data[0];
			bt_shell_print("  StatusIndicator: %u (%s)", val,
				       val <= 2 ? status[val] : "invalid");
		}
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_STATUS_VALUE:
		if (tlv->data_len == 1) {
			bt_shell_print("  StatusValue: %s", tlv->data[0] ? "Yes" : "No");
		}
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_MSE_TIME:
		bt_shell_print("  MSETime: %.*s", tlv->data_len, tlv->data);
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_DATABASE_IDENTIFIER:
		bt_shell_print("  DatabaseIdentifier: %.*s", tlv->data_len, tlv->data);
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_CONV_LISTING_VER_CNTR:
		bt_shell_print("  ConvoListingVersionCounter: %.*s", tlv->data_len, tlv->data);
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_PRESENCE_AVAILABILITY:
		if (tlv->data_len == 1) {
			const char *presence[] = {"Unknown",      "Offline", "Online",   "Away",
						  "DoNotDisturb", "Busy",    "InMeeting"};
			uint8_t val = tlv->data[0];
			bt_shell_print("  PresenceAvailability: %u (%s)", val,
				       val <= 6 ? presence[val] : "invalid");
		}
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_PRESENCE_TEXT:
		bt_shell_print("  PresenceText: %.*s", tlv->data_len, tlv->data);
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_LAST_ACTIVITY:
		bt_shell_print("  LastActivity: %.*s", tlv->data_len, tlv->data);
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_FILTER_LAST_ACTIVITY_BEGIN:
		bt_shell_print("  FilterLastActivityBegin: %.*s", tlv->data_len, tlv->data);
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_FILTER_LAST_ACTIVITY_END:
		bt_shell_print("  FilterLastActivityEnd: %.*s", tlv->data_len, tlv->data);
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_CHAT_STATE:
		if (tlv->data_len == 1) {
			const char *state[] = {"Unknown",   "Inactive",        "Active",
					       "Composing", "PausedComposing", "Gone"};
			uint8_t val = tlv->data[0];
			bt_shell_print("  ChatState: %u (%s)", val,
				       val <= 5 ? state[val] : "invalid");
		}
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_CONVERSATION_ID:
		bt_shell_print("  ConversationID: %.*s", tlv->data_len, tlv->data);
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_FOLDER_VER_CNTR:
		bt_shell_print("  FolderVersionCounter: %.*s", tlv->data_len, tlv->data);
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_FILTER_MSG_HANDLE:
		bt_shell_print("  FilterMsgHandle: %.*s", tlv->data_len, tlv->data);
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_NOTIFICATION_FILTER_MASK:
		if (tlv->data_len == 4) {
			bt_shell_print("  NotificationFilterMask: 0x%08x", sys_get_be32(tlv->data));
		}
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_CONV_PARAMETER_MASK:
		if (tlv->data_len == 4) {
			bt_shell_print("  ConvoParameterMask: 0x%08x", sys_get_be32(tlv->data));
		}
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_OWNER_UCI:
		bt_shell_print("  OwnerUCI: %.*s", tlv->data_len, tlv->data);
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_EXTENDED_DATA:
		bt_shell_print("  ExtendedData: %.*s", tlv->data_len, tlv->data);
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_MAP_SUPPORTED_FEATURES:
		if (tlv->data_len == 4) {
			bt_shell_print("  MAPSupportedFeatures: 0x%08x", sys_get_be32(tlv->data));
		}
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_MESSAGE_HANDLE:
		bt_shell_print("  MessageHandle: %.*s", tlv->data_len, tlv->data);
		break;
	case BT_MAP_APPL_PARAM_TAG_ID_MODIFY_TEXT:
		if (tlv->data_len == 1) {
			bt_shell_print("  ModifyText: %s", tlv->data[0] ? "PREPEND" : "REPLACE");
		}
		break;
	default:
		bt_shell_print("  Unknown parameter type");
		break;
	}
...
static int parse_chunked_req(const struct shell *sh, size_t argc, char *argv[], bool *enable_chunk)
{
	int arg_idx = 1;

	while (arg_idx < argc) {
		if (!strcmp(argv[arg_idx], "chunked_req")) {
			*enable_chunk = true;
			break;
		}
		arg_idx++;
...
static int cmd_mce_mas_get_folder_listing(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err = parse_chunked_req(sh, argc, argv, &enable_chunk);
	if (err != 0) {
		shell_help(sh);
		return SHELL_CMD_HELP_PRINTED;
	}
	...
	if (inst->chunked_req) {
		struct bt_obex_tlv tlv[1];

		tlv[0].type = USER_DEFINED_APP_PARAM_HDR;
		tlv[0].data_len = strlen(USER_DEFINED_APP_PARAM);
		tlv[0].data = (const uint8_t *)USER_DEFINED_APP_PARAM;
		err = bt_obex_add_header_app_param(buf, ARRAY_SIZE(tlv), tlv);
		if (err != 0) {
			shell_error(sh, "Failed to add application parameter header %d", err);
			goto failed;
		}

		inst->chunked_req = false;
		goto continue_req;
	}
...
continue_req:
	err = bt_map_mce_mas_get_folder_listing(default_mce_mas, !inst->chunked_req, buf);
	if (err != 0) {
...
continue_req:
	err = bt_map_mce_mas_get_msg_listing(default_mce_mas, !inst->chunked_req, buf);
	if (err != 0) {
		clear_local_srm(buf, &inst->srm);
		clear_local_srm_param(buf, &inst->srm);
...
static int cmd_mce_mas_get_msg(const struct shell *sh, size_t argc, char *argv[])
{
	...
	bool enable_srmp = false;
	bool enable_chunk = false;
	const char *name = "0000000000000001";
...
continue_req:
	err = bt_map_mce_mas_get_msg(default_mce_mas, !inst->chunked_req, buf);
	if (err != 0) {
		clear_local_srm(buf, &inst->srm);
		clear_local_srm_param(buf, &inst->srm);
...
static int cmd_mce_mas_set_msg_status(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (!bt_obex_has_header(buf, BT_OBEX_HEADER_ID_APP_PARAM)) {
		uint8_t status_indicator = BT_MAP_STATUS_INDICATOR_READ;
		uint8_t status_value = BT_MAP_STATUS_VALUE_NO;
		struct bt_obex_tlv appl_params[] = {
...
continue_req:
	err = bt_map_mce_mas_get_mas_inst_info(default_mce_mas, !inst->chunked_req, buf);
	if (err != 0) {
		clear_local_srm(buf, &inst->srm);
		clear_local_srm_param(buf, &inst->srm);
...
continue_req:
	err = bt_map_mce_mas_get_owner_status(default_mce_mas, !inst->chunked_req, buf);
	if (err != 0) {
		clear_local_srm(buf, &inst->srm);
		clear_local_srm_param(buf, &inst->srm);
...
continue_req:
	err = bt_map_mce_mas_get_convo_listing(default_mce_mas, !inst->chunked_req, buf);
	if (err != 0) {
		clear_local_srm(buf, &inst->srm);
		clear_local_srm_param(buf, &inst->srm);
...
static int cmd_mse_mas_rfcomm_register(const struct shell *sh, size_t argc, char *argv[])
{
	...
	shell_print(sh, "Register RFCOMM server done");
...
static int cmd_mse_mas_l2cap_register(const struct shell *sh, size_t argc, char *argv[])
{
	...
	shell_print(sh, "Register L2CAP server done");
...
static int cmd_mse_mas_get_msg_listing(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (!bt_obex_has_header(buf, BT_OBEX_HEADER_ID_APP_PARAM)) {
		uint8_t new_message = BT_MAP_NEW_MESSAGE_OFF;
...
static int cmd_app_param_message_handle(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err = add_tlv(sh, BT_MAP_APPL_PARAM_TAG_ID_MESSAGE_HANDLE, (const uint8_t *)handle_str,
		      len);
	if (err == 0) {
...
static int cmd_app_param_modify_text(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (!strcmp(argv[1], "prepend") || !strcmp(argv[1], "1")) {
		value = 1;
	} else if (!strcmp(argv[1], "replace") || !strcmp(argv[1], "0")) {
		value = 0;
	} else {
		shell_error(sh, "Invalid value (use: prepend/replace or 1/0)");
		return -EINVAL;
	}

	err = add_tlv(sh, BT_MAP_APPL_PARAM_TAG_ID_MODIFY_TEXT, &value, 1);
	if (err == 0) {
		shell_print(sh, "Added ModifyText: %s", value ? "PREPEND" : "REPLACE");
	}
	return err;
}
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(modify_text, NULL, "<prepend/replace or 1/0>", cmd_app_param_modify_text,
		      2, 0),
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(get_folder_listing, NULL, "[chunked_req] [srmp]",
		      cmd_mce_mas_get_folder_listing, 1, 2),
	SHELL_CMD_ARG(get_msg_listing, NULL, "[chunked_req] [srmp]", cmd_mce_mas_get_msg_listing, 1,
		      2),
	SHELL_CMD_ARG(get_msg, NULL, "[chunked_req] [srmp]", cmd_mce_mas_get_msg, 1, 2),
	SHELL_CMD_ARG(set_msg_status, NULL, HELP_NONE, cmd_mce_mas_set_msg_status, 1, 0),
	SHELL_CMD_ARG(push_msg, NULL, HELP_NONE, cmd_mce_mas_push_msg, 1, 0),
	SHELL_CMD_ARG(update_inbox, NULL, HELP_NONE, cmd_mce_mas_update_inbox, 1, 0),
	SHELL_CMD_ARG(get_mas_inst_info, NULL, "[chunked_req] [srmp]",
		      cmd_mce_mas_get_mas_inst_info, 1, 2),
	SHELL_CMD_ARG(set_owner_status, NULL, HELP_NONE, cmd_mce_mas_set_owner_status, 1, 0),
	SHELL_CMD_ARG(get_owner_status, NULL, "[chunked_req] [srmp]", cmd_mce_mas_get_owner_status,
		      1, 2),
	SHELL_CMD_ARG(get_convo_listing, NULL, "[chunked_req] [srmp]",
		      cmd_mce_mas_get_convo_listing, 1, 2),
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(send_event, NULL, "<rsp: noerror, error> [rsp_code] [srmp]",
		      cmd_mce_mns_send_event, 2, 2),
	SHELL_SUBCMD_SET_END);
