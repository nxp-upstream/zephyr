###default
### mps = BT_L2CAP_STD_CONTROL_SIZE(2) + BT_L2CAP_RT_FC_SDU_LEN_SIZE(2) + payload_size + BT_L2CAP_FCS_SIZE(2)
...
PAYLOAD_LEN = BT_L2CAP_MPS - BT_L2CAP_STD_CONTROL_SIZE - BT_L2CAP_RT_FC_SDU_LEN_SIZE - BT_L2CAP_FCS_SIZE
...
def test_l2cap_eret_mode_TC06(client, server):
    ...
    logger.info(f'TEST client send data to server')

    logger.info(f'client send data1, len < PAYLOAD_LEN')
    data1 = "this_is_data1"
    data_len = len(data1)
    ...
    logger.info(f'client send data2, PAYLOAD_LEN< len < 2*PAYLOAD_LEN')
    ...
    _ , lines = server._wait_for_shell_response(f"!@#")
    seg_count = 0
    server_recv_data = ""
    for line in lines:
        if "Incoming data:" in  line:
    ...
    logger.info(f'TEST server send data to client')

    logger.info(f'client send data3, len < PAYLOAD_LEN')
    ...
    logger.info(f'server send data4, PAYLOAD_LEN< len < 2*PAYLOAD_LEN')
...
def test_l2cap_stream_mode_TC05(client, server):
    ...
    logger.info(f'client send data2, PAYLOAD_LEN< len < BT_L2CAP_MPS < 2*PAYLOAD_LEN')
    data2 = "1234567890QWERTYUIOPASDFGHJKLZXCVBNM1234567!@#"
    data_len = len(data2)
    ...
    logger.info(f'server send data4, PAYLOAD_LEN< len < BT_L2CAP_MPS < 2*PAYLOAD_LEN')
    data4 = "1234567890QWERTYUIOPASDFGHJKLZXCVBNM1234567][]"
    data_len = len(data4)
    assert data_len > PAYLOAD_LEN and data_len < 2 * PAYLOAD_LEN and data_len < BT_L2CAP_MPS
