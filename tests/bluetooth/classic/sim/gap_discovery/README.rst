...
Twister uses the script harness defined in :file:`testcase.yaml` and runs:
...
* copies the generated ``zephyr.exe`` into ``${BT_CLASSIC_BIN}`` using
  ``bt_classic_${BOARD_SANITIZED}_bluetooth.classic.sim.gap.discovery.exe`` naming,
