...
start_bumble_controllers() {
    ...
    for port_spec in "${ports[@]}"; do
        ...
        local elapsed=0
        local ready=0
        while (( $(echo "$elapsed < $timeout_s" | bc -l) )); do
            # Check if process is still alive before polling
            if ! kill -0 "$controller_pid" 2>/dev/null; then
                echo "Bumble controllers process exited unexpectedly while waiting for port $port" >&2
                cat "$log_file"
                return 1
            fi
            # Try a non-blocking TCP connection via /dev/tcp
