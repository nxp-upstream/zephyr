...
extra_twister_out() {
    ...
    output_name="bt_classic_${BOARD_SANITIZED}_${test_name}.exe"

    # Remove stale output before copying new one
    if [[ -f "${BT_CLASSIC_BIN}/${output_name}" ]]; then
        rm -f "${BT_CLASSIC_BIN}/${output_name}"
    fi
    ...
    # Prefer zephyr.exe paths that include test_name (avoid copying unrelated builds)
    local matched_exe_files
    matched_exe_files=$(echo "${exe_files}" | grep -F -- "${test_name}" || true)
    ...
    # If multiple candidates match, copy only the newest one and warn
    local n_matches
    ...
    cp "${selected_exe}" "${BT_CLASSIC_BIN}/${output_name}"
}
