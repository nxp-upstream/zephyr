...
* copies the generated ``zephyr.exe`` into ``${BT_CLASSIC_BIN}`` using a naming
  scheme matching the Twister output binary,
* launches the test binary twice:

  * server instance (e.g. ``-test=sdp_server::...``)
  * client instance (e.g. ``-test=sdp_client::...``)
