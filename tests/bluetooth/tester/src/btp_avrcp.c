...
static void set_playing_item(void)
{
	...
	node = sys_slist_peek_head(&tg_now_playing_list);
	if (node != NULL) {
		vfs_node = CONTAINER_OF(node, struct vfs_node, node);
		tg_playing_item = (struct media_item *)(void *)vfs_node->item;
	} else {
					...
					uint16_t *rsp_len)
{
	...
	case BT_AVRCP_EVT_NOW_PLAYING_CONTENT_CHANGED:
		...
		if (removed_node != NULL) {
			...
			remove_from_list(&tg_now_playing_list, vfs_node);
		}
		break;
