...
static void rfcomm_connected(struct bt_rfcomm_dlc *dlc)
{
	...
	bt_addr_copy(&ev.address, &chan->dst);
...
static void rfcomm_sent(struct bt_rfcomm_dlc *dlc, int err)
{
	...
	tester_event(BTP_SERVICE_ID_RFCOMM, BTP_RFCOMM_EV_DATA_SENT,
		     &ev, sizeof(ev));
}
			  ...
			  void *rsp, uint16_t *rsp_len)
{
	...
	chan = find_channel(&cp->address, cp->channel);
	if (chan == NULL) {
