...
static uint8_t hfp_ag_read_supported_commands(const void *cmd, uint16_t cmd_len, void *rsp,
					      uint16_t *rsp_len)
{
...
static uint8_t hfp_ag_connect(const void *cmd, uint16_t cmd_len, void *rsp, uint16_t *rsp_len)
{
...
static uint8_t hfp_ag_disconnect(const void *cmd, uint16_t cmd_len, void *rsp, uint16_t *rsp_len)
{
...
static uint8_t hfp_ag_remote_incoming(const void *cmd, uint16_t cmd_len, void *rsp,
				      uint16_t *rsp_len)
{
	...
	if (cmd_len < sizeof(*cp) || cmd_len < (sizeof(*cp) + cp->number_len)) {
...
static uint8_t hfp_ag_outgoing(const void *cmd, uint16_t cmd_len, void *rsp, uint16_t *rsp_len)
{
...
static uint8_t hfp_ag_remote_ringing(const void *cmd, uint16_t cmd_len, void *rsp,
				     uint16_t *rsp_len)
{
...
static uint8_t hfp_ag_remote_accept(const void *cmd, uint16_t cmd_len, void *rsp, uint16_t *rsp_len)
{
...
static uint8_t hfp_ag_remote_reject(const void *cmd, uint16_t cmd_len, void *rsp, uint16_t *rsp_len)
{
...
static uint8_t hfp_ag_remote_terminate(const void *cmd, uint16_t cmd_len, void *rsp,
				       uint16_t *rsp_len)
{
...
static uint8_t hfp_ag_accept_call(const void *cmd, uint16_t cmd_len, void *rsp, uint16_t *rsp_len)
{
...
static uint8_t hfp_ag_reject_call(const void *cmd, uint16_t cmd_len, void *rsp, uint16_t *rsp_len)
{
...
static uint8_t hfp_ag_terminate_call(const void *cmd, uint16_t cmd_len, void *rsp,
				     uint16_t *rsp_len)
{
...
static uint8_t hfp_ag_hold_call(const void *cmd, uint16_t cmd_len, void *rsp, uint16_t *rsp_len)
{
...
static uint8_t hfp_ag_retrieve_call(const void *cmd, uint16_t cmd_len, void *rsp, uint16_t *rsp_len)
{
...
static uint8_t hfp_ag_hold_incoming(const void *cmd, uint16_t cmd_len, void *rsp, uint16_t *rsp_len)
{
...
#if defined(CONFIG_BT_HFP_AG_3WAY_CALL)
static uint8_t hfp_ag_explicit_call_transfer(const void *cmd, uint16_t cmd_len, void *rsp,
					     uint16_t *rsp_len)
{
...
static uint8_t hfp_ag_audio_connect(const void *cmd, uint16_t cmd_len, void *rsp, uint16_t *rsp_len)
{
...
static uint8_t hfp_ag_audio_disconnect(const void *cmd, uint16_t cmd_len, void *rsp,
				       uint16_t *rsp_len)
{
...
static uint8_t hfp_ag_set_vgm(const void *cmd, uint16_t cmd_len, void *rsp, uint16_t *rsp_len)
{
...
static uint8_t hfp_ag_set_vgs(const void *cmd, uint16_t cmd_len, void *rsp, uint16_t *rsp_len)
{
...
static uint8_t hfp_ag_set_operator(const void *cmd, uint16_t cmd_len, void *rsp, uint16_t *rsp_len)
{
...
static uint8_t hfp_ag_set_inband_ringtone(const void *cmd, uint16_t cmd_len, void *rsp,
					  uint16_t *rsp_len)
{
...
#if defined(CONFIG_BT_HFP_AG_VOICE_RECG)
static uint8_t hfp_ag_voice_recognition(const void *cmd, uint16_t cmd_len, void *rsp,
					uint16_t *rsp_len)
{
...
#if defined(CONFIG_BT_HFP_AG_ENH_VOICE_RECG)
static uint8_t hfp_ag_vre_state(const void *cmd, uint16_t cmd_len, void *rsp, uint16_t *rsp_len)
{
...
static uint8_t hfp_ag_vre_text(const void *cmd, uint16_t cmd_len, void *rsp, uint16_t *rsp_len)
{
	...
	if (cmd_len < sizeof(*cp) || cmd_len < (sizeof(*cp) + cp->text_len)) {
		LOG_ERR("Invalid command length");
		return BTP_STATUS_FAILED;
	}
	...
	err = bt_hfp_ag_vre_textual_representation(conn->ag, cp->state, text_id, cp->text_type,
						   cp->text_operation, text);
	if (err != 0) {
...
static uint8_t hfp_ag_set_signal_strength(const void *cmd, uint16_t cmd_len, void *rsp,
					  uint16_t *rsp_len)
{
...
static uint8_t hfp_ag_set_roaming_status(const void *cmd, uint16_t cmd_len, void *rsp,
					 uint16_t *rsp_len)
{
...
static uint8_t hfp_ag_set_battery_level(const void *cmd, uint16_t cmd_len, void *rsp,
					uint16_t *rsp_len)
{
...
static uint8_t hfp_ag_set_service_availability(const void *cmd, uint16_t cmd_len, void *rsp,
					       uint16_t *rsp_len)
{
...
#if defined(CONFIG_BT_HFP_AG_HF_INDICATORS)
static uint8_t hfp_ag_set_hf_indicator(const void *cmd, uint16_t cmd_len, void *rsp,
				       uint16_t *rsp_len)
{
...
static uint8_t hfp_ag_set_ongoing_calls(const void *cmd, uint16_t cmd_len, void *rsp,
					uint16_t *rsp_len)
{
...
static uint8_t hfp_ag_set_last_number(const void *cmd, uint16_t cmd_len, void *rsp,
				      uint16_t *rsp_len)
{
...
static uint8_t hfp_ag_set_default_indicator_value(const void *cmd, uint16_t cmd_len, void *rsp,
						  uint16_t *rsp_len)
{
...
static uint8_t hfp_ag_set_memory_dial_mapping(const void *cmd, uint16_t cmd_len, void *rsp,
					      uint16_t *rsp_len)
{
...
static uint8_t hfp_ag_set_subscriber_number(const void *cmd, uint16_t cmd_len, void *rsp,
					    uint16_t *rsp_len)
{
...
#if defined(CONFIG_BT_HFP_AG_VOICE_TAG)
static uint8_t hfp_ag_set_voice_tag_number(const void *cmd, uint16_t cmd_len, void *rsp,
					   uint16_t *rsp_len)
{
