...
static void ag_disconnected(struct bt_hfp_ag *ag)
{
	...
	bt_addr_copy(&ev.address, &conn->address);
			      ...
			      void *rsp, uint16_t *rsp_len)
{
	...
	conn = find_connection_by_address(&cp->address);
	if (conn != NULL) {
	...
	if (acl_conn == NULL) {
		acl_conn = bt_conn_create_br(&cp->address, BT_BR_CONN_PARAM_DEFAULT);
		if (acl_conn == NULL) {
...
static int add_subscriber_number(const char *number, uint8_t type, uint8_t service)
{
	...
	strncpy(subscriber_numbers[subscriber_numbers_count].number, number,
		CONFIG_BT_HFP_AG_PHONE_NUMBER_MAX_LEN);
	subscriber_numbers[subscriber_numbers_count].number[CONFIG_BT_HFP_AG_PHONE_NUMBER_MAX_LEN] = '\0';
	subscriber_numbers[subscriber_numbers_count].type = type;
	subscriber_numbers[subscriber_numbers_count].service = service;
	subscriber_numbers[subscriber_numbers_count].in_use = true;
