				     ...
				     uint16_t *rsp_len)
{
	...
	/*
	 * The secondary transport was opened here via second_connect_l2cap()/
	 * second_connect_rfcomm(), whose bt_bip_l2cap_connect()/bt_bip_rfcomm_connect()
	 * unconditionally set inst->second_bip.role to INITIATOR. The host
	 * bt_bip_client_connect() secondary branch rejects role INITIATOR with
	 * "Invalid role initiator". Reset the role to RESPONDER here, after the
	 * transport connect has completed and before the secondary OBEX client
	 * connect, so no later transport connect can overwrite it.
	 */
