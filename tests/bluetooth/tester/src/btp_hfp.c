...
static struct bt_conn *hfp_conn;
static struct bt_hfp_hf *hfp_hf_conn;
...
{
    LOG_DBG("HFP HF connected");
    hfp_conn = bt_conn_ref(conn);
    hfp_hf_conn = hf;
}

static void hf_disconnected(struct bt_hfp_hf *hf)
{
    LOG_DBG("HFP HF disconnected");
    if (hfp_conn) {
        bt_conn_unref(hfp_conn);
        hfp_conn = NULL;
    }
    hfp_hf_conn = NULL;
}
...
{
    LOG_DBG("HFP HF SCO connected");
}

static void hf_sco_disconnected(struct bt_conn *sco_conn, uint8_t reason)
{
    LOG_DBG("HFP HF SCO disconnected (reason %u)", reason);
}
		       ...
		       void *rsp, uint16_t *rsp_len)
{
    ...
    struct btp_hfp_ag_connect_rp *rp = rsp;
    struct bt_conn *conn;
    struct bt_hfp_ag *ag;
