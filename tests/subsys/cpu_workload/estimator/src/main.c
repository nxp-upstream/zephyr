...
{
	k_thread_activation_stats_t stats;
	int ret;
	...
	ret = k_thread_activation_stats_get(&profile_thread, &stats, false);
	zassert_ok(ret);
	zassert_true(stats.completed_count > 0U, "thread stats did not collect activations");
	zassert_true(stats.completed_cycles > 0U, "thread stats has no completed cycles");
}
