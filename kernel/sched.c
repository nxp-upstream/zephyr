...
{	
	/**
	 * By default, attribute wakeups from this function to synchronous arrivals,
	 * which include explicit wakeups (e.g. k_thread_wake()) and timeout wakeups
	 * (e.g. k_thread_sleep()). Other scheduler paths can call
	 * z_sched_wake_thread_locked_source() directly to attribute to other sources.
	 */
