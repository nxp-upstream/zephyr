...
* The :dtcompatible:`nxp,imxrt-arm-pll` binding now uses ``loop-div`` and
  ``post-div`` for ARM PLL configuration. For RT11xx compatibility, the
  legacy ``clock-mult`` and ``clock-div`` properties remain supported but are
  deprecated. Existing RT11xx overlays should be updated using the mapping
