...
.. code-block:: bash
   ...
   extra_twister_out "${ZEPHYR_BASE}/twister-out" "${test_case_name}"
   ...
   ARGS="--peer_bd_address=${BD_ADDR[1]} -test=gap_peripheral::test_01_gap_peripheral_general_discovery"
   ...
   ARGS="--peer_bd_address=${BD_ADDR[0]}  -test=gap_central::test_01_gap_central_general_discovery"
...
* Each test directory provides its own :file:`testcase.yaml` (for example
  :zephyr_file:`tests/bluetooth/classic/sim/gap_discovery/testcase.yaml`).
...
.. code-block:: none
   ...
   └── gap_discovery/                      # Example test: GAP discovery
       ...
       │   └── general_discovery.sh        # Script-harness entrypoint (alloc ports, start controllers, run exe)
