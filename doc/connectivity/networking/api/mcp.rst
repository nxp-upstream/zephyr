...
- Check interval: ``CONFIG_MCP_HEALTH_CHECK_INTERVAL_MS``
...
The HTTP transport supports both static and dynamic IP address assignment:

.. code-block:: kconfig

   CONFIG_MCP_HTTP_ADDRESS_STATIC
      Use static IP address configuration
      Type: bool
      Default: n
      Depends on: CONFIG_MCP_TRANSPORT_HTTP

   CONFIG_MCP_HTTP_LOG_ADDRESS
      Log HTTP server address on startup
      Type: bool
      Default: n
      Depends on: CONFIG_MCP_TRANSPORT_HTTP
      Selects: NET_MGMT, NET_MGMT_EVENT, NET_MGMT_EVENT_INFO

      When enabled, logs the HTTP server's accessible URL when:
      - Server starts (if IP already assigned)
      - IP address is assigned via DHCP or network management

      Example output (static):
        [00:00:00.123,456] <inf> mcp_http_transport: HTTP server configured
                                with static address: 192.168.1.100
        [00:00:00.123,789] <inf> mcp_http_transport: Accessible at:
                                http://192.168.1.100:8080/mcp

      Example output (dynamic):
        [00:00:00.123,456] <inf> mcp_http_transport: HTTP server listening
                                on 0.0.0.0:8080/mcp (no IP assigned yet)
        [00:00:05.678,901] <inf> mcp_http_transport: HTTP server accessible
                                at: http://192.168.1.150:8080/mcp

      This feature is helpful during development and debugging to quickly
      identify the server's network endpoint.
...
.. code-block:: cfg
   ...
   CONFIG_MCP_HTTP_ADDRESS_STATIC=y
   CONFIG_NET_CONFIG_MY_IPV4_ADDR="192.168.1.100"
   CONFIG_NET_CONFIG_MY_IPV4_NETMASK="255.255.255.0"
   CONFIG_NET_CONFIG_MY_IPV4_GW="192.168.1.1"

   # Log the server address on startup
   CONFIG_MCP_HTTP_LOG_ADDRESS=y

Dynamic IP configuration (DHCP):

.. code-block:: cfg

   # Use dynamic addressing (default)
   CONFIG_MCP_HTTP_ADDRESS_STATIC=n
   CONFIG_NET_DHCPV4=y

   # Log address when DHCP assigns it
   CONFIG_MCP_HTTP_LOG_ADDRESS=y
   CONFIG_NET_MGMT=y
   CONFIG_NET_MGMT_EVENT=y

.. note::
   When using static addressing, ensure CONFIG_NET_CONFIG_MY_IPV4_ADDR
   and related network configuration options are properly set in your
   project configuration.

.. warning::
   CONFIG_MCP_HTTP_LOG_ADDRESS adds dependencies on network management
   events. Disable this option in production builds if the additional
...
**Important:** The ``.data`` field in ``mcp_tool_message`` should contain
...
.. code-block:: c
                               ...
                               const char *execution_token)
   {
       ...
       struct mcp_tool_message response = {
           .type = MCP_USR_TOOL_RESPONSE,
           .data = result_text,              /* Plain text, not JSON */
...
Response text is limited by ``CONFIG_MCP_TOOL_RESULT_MAX_LEN`` (default: 256 bytes).

For larger responses, increase this configuration option:

.. code-block:: kconfig

   CONFIG_MCP_TOOL_RESULT_MAX_LEN=1024
