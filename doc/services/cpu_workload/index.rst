...
one complete activation. An activation starts when a blocked thread becomes
ready because of a wakeup event and ends when that thread blocks again. This
makes the profile describe event work instead of scheduler fragments.

When a thread has completed activations, the subsystem computes the burst
profile as:

.. code-block:: text

   burst profile = completed_activation_cycles / completed_activation_count

Each profile also has a confidence value from 0 to 100. Profiles with no samples
report 0 confidence. As a thread accumulates completed activations, the
