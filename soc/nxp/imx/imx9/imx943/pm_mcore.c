...
static void nvic_save()
{
	memcpy(nvic_context_t.ISER, (uint32_t *)NVIC->ISER, sizeof(NVIC->ISER));
	memcpy(nvic_context_t.ISPR, (uint32_t *)NVIC->ISPR, sizeof(NVIC->ISPR));
	memcpy(nvic_context_t.IPR, (uint8_t *)NVIC->IPR, sizeof(NVIC->IPR));
}

static void nvic_restore()
{
	memcpy((uint32_t *)NVIC->ISER, nvic_context_t.ISER, sizeof(NVIC->ISER));
	memcpy((uint32_t *)NVIC->ISPR, nvic_context_t.ISPR, sizeof(NVIC->ISPR));
	memcpy((uint8_t *)NVIC->IPR, nvic_context_t.IPR, sizeof(NVIC->IPR));
}

static void scb_save()
{
	...
	memcpy(scb_context_t.SHPR, (uint32_t *)SCB->SHPR, sizeof(SCB->SHPR));
...
static void scb_restore()
{
	...
	memcpy((uint32_t *)SCB->SHPR, scb_context_t.SHPR, sizeof(SCB->SHPR));
...
{
	int ret;
	struct scmi_cpu_info_get cpu_info;
