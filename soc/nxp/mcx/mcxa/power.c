...
static int mcxa_s2ram_retain_sram(void)
{
	...
	/* Selecting the arrays to retain is enough: the RAMA arrays stay on the
	 * SoC supply in active mode and the hardware hands them over to the VBAT
	 * LDO_RAM only while the CORE domain is gated in Deep Power Down, then
	 * back to the SoC supply automatically on wake. Do NOT call
	 * VBAT_SwitchSRAMPowerByLDOSRAM() here: that isolates and switches the
	 * arrays to the LDO_RAM in active mode too, and its mandatory counterpart
	 * VBAT_SwitchSRAMPowerBySocSupply() cannot run on resume because the boot
	 * ROM and the restored CPU context both use a stack that lives in the
	 * isolated RAMA - leaving the arrays isolated hangs the wake-up reset.
	 */
