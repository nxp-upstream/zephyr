 ...
 * Copyright 2025 NXP
 *
 * Based on NXP ke1xf soc.c, which is:
...
__weak void clock_init(void)
{
	...
	CLOCK_SetIpSrc(kCLOCK_Can0,
		       DT_CLOCKS_CELL(DT_NODELABEL(flexcan0), ip_source));
#endif
#if DT_NODE_HAS_STATUS_OKAY(DT_NODELABEL(flexcan1))
	CLOCK_SetIpSrc(kCLOCK_Can1,
		       DT_CLOCKS_CELL(DT_NODELABEL(flexcan1), ip_source));
#endif
#if DT_NODE_HAS_STATUS_OKAY(DT_NODELABEL(flexcan2))
	CLOCK_SetIpSrc(kCLOCK_Can2,
		       DT_CLOCKS_CELL(DT_NODELABEL(flexcan2), ip_source));
#endif
