...
static bool load_gdet_cfg(power_gdet_data_t *data)
{
	...
	if (retval) {
		...
		if ((MCUX_CSSL_FP_FUNCTION_CALLED(mcuxClEls_GlitchDetector_LoadConfig_Async)
				!= token) || (MCUXCLELS_STATUS_OK_WAIT != result)) {
			retval = false;
		}
	...
	if (retval) {
		...
		if ((MCUX_CSSL_FP_FUNCTION_CALLED(mcuxClEls_WaitForOperation)
				!= token) || (MCUXCLELS_STATUS_OK != result)) {
			retval = false;
		}
		MCUX_CSSL_FP_FUNCTION_CALL_END();
	}
...
static void config_svc_sensor(void)
{
	...
	if (status == kStatus_Success) {
		...
		status = (status == kStatus_Success) ?
			  OCOTP_OtpFuseRead((rev == 2U) ? 155 : 152, &gdetData.CFG[3]) :
			  status;
