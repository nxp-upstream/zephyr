 ...
 * must execute from RAM. AT_QUICKACCESS_SECTION_CODE places them in the
 * CodeQuickAccess section, which the linker collects into the .ramfunc region.
 */
AT_QUICKACCESS_SECTION_CODE(static void disable_xcache(XCACHE_Type *base))
{
...
AT_QUICKACCESS_SECTION_CODE(static void enable_xcache(XCACHE_Type *base))
{
 ...
 */
AT_QUICKACCESS_SECTION_CODE(static void enable_xspi_cache(CACHE64_CTRL_Type *cache))
{
...
AT_QUICKACCESS_SECTION_CODE(static void disable_xspi_cache(CACHE64_CTRL_Type *cache))
{
	/* Push (clean) cache */
 ...
 */
AT_QUICKACCESS_SECTION_CODE(static void init_xspi(XSPI_Type *base, CACHE64_CTRL_Type *cache))
{
...
AT_QUICKACCESS_SECTION_CODE(static void deinit_xspi(XSPI_Type *base, CACHE64_CTRL_Type *cache))
{
...
AT_QUICKACCESS_SECTION_CODE(static void deinit_xip(void))
{
...
AT_QUICKACCESS_SECTION_CODE(static void init_xip(void))
{
	if (POWER_IS_XIP_XSPI0()) {
		CLKCTL0->PSCCTL1_SET = CLKCTL0_PSCCTL1_SET_XSPI0_MASK;
 ...
 * Power control API
 */
AT_QUICKACCESS_SECTION_CODE(void power_xip_suspend(bool flush_xcache0, bool flush_xcache1))
{
	deinit_xip();
...
AT_QUICKACCESS_SECTION_CODE(void power_xip_resume(void))
{
	init_xip();
