...
#define IS_TX_POWER_VALID() \
	(DT_INST_FOREACH_PROP_ELEM_SEP(0, max_tx_power_support, \
		IS_EQUAL_TO_SUPPORTED, (||)))

/* Validation only if supported values exists */
BUILD_ASSERT(IS_TX_POWER_VALID(),
	"max-tx-power-dbm must be one of the supported values");
#endif
