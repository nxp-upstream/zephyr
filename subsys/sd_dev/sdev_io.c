 ...
 */
void sdev_rx_dispatch(const struct device *dev, sdev_pkt_t *pkt)
{
	...
	if (sdev_get_state(dev) != SDEV_DEVICE_READY) {
...
int sdio_read(struct sdio_dev_func *func, uint8_t *data)
{
	...
	if (sdev_get_state(func->sdio->card->dev) != SDEV_DEVICE_READY) {
