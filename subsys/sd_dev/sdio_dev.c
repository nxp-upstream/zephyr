...
static int sdio_dev_cfg_init(struct sdio_dev *sdio)
{
	int ret;
	int default_val;
	struct sdio_dev_cfg *cfg = sdio->cfg;

	if (sdio == NULL || cfg == NULL) {
		LOG_ERR("Invalid parameters: sdio=%p, cfg=%p", sdio, cfg);
		return -EINVAL;
	}

	/* Set CCCR version if specified */
	if (cfg->cccr_version > 0) {
		ret = sdio_dev_set_cccr_version(sdio, cfg->cccr_version);
		if (ret < 0) {
			if (ret == -EACCES) {
				default_val = sdio_dev_get_cccr_version(sdio);
				LOG_WRN("No permission to set CCCR version, using default: %d",
					default_val);
				cfg->cccr_version = default_val;
			} else {
				LOG_ERR("Failed to set CCCR version: %d", ret);
				return ret;
			}
		} else {
			LOG_DBG("CCCR version set to: %d", cfg->cccr_version);
		}
	}

	/* Set SDIO specification version if specified */
	if (cfg->sdio_version > 0) {
		ret = sdio_dev_set_sdio_version(sdio, cfg->sdio_version);
		if (ret < 0) {
			if (ret == -EACCES) {
				default_val = sdio_dev_get_sdio_version(sdio);
				LOG_WRN("No permission to set SDIO version, using default: %d",
					default_val);
				cfg->sdio_version = default_val;
			} else {
				LOG_ERR("Failed to set SDIO version: %d", ret);
				return ret;
			}
		} else {
			LOG_DBG("SDIO version set to: %d", cfg->sdio_version);
		}
	}

	/* Set SD specification version if specified */
	if (cfg->spec_version > 0) {
		ret = sdio_dev_set_spec_version(sdio, cfg->spec_version);
		if (ret < 0) {
			if (ret == -EACCES) {
				default_val = sdio_dev_get_spec_version(sdio);
				LOG_WRN("No permission to set SD spec version, using default: %d",
					default_val);
				cfg->spec_version = default_val;
			} else {
				LOG_ERR("Failed to set SD spec version: %d", ret);
				return ret;
			}
		} else {
			LOG_DBG("SD spec version set to: %d", cfg->spec_version);
		}
	}

	/* Set bus width if specified */
	if (cfg->bus_width > 0) {
		ret = sdio_dev_set_bus_width(sdio, cfg->bus_width);
		if (ret < 0) {
			if (ret == -EACCES) {
				default_val = sdio_dev_get_bus_width(sdio);
				LOG_WRN("No permission to set bus width, using default: %d",
					default_val);
				cfg->bus_width = default_val;
			} else {
				LOG_ERR("Failed to set bus width: %d", ret);
				return ret;
			}
		} else {
			LOG_DBG("Bus width set to: %d", cfg->bus_width);
		}
	}

	/* Set CSPI interrupt mode */
	ret = sdio_dev_set_cspi_int(sdio, cfg->cspi_int ? 1 : 0);
	if (ret < 0) {
		if (ret == -EACCES) {
			default_val = sdio_dev_get_cspi_int(sdio);
			LOG_WRN("No permission to set CSPI interrupt, using default: %d",
				default_val);
			cfg->cspi_int = (default_val != 0);
		} else {
			LOG_ERR("Failed to set CSPI interrupt: %d", ret);
			return ret;
		}
	} else {
		LOG_DBG("CSPI interrupt set to: %d", cfg->cspi_int);
	}

	/* Set multi-block support */
	ret = sdio_dev_set_multi_block(sdio, cfg->multi_block ? 1 : 0);
	if (ret < 0) {
		if (ret == -EACCES) {
			default_val = sdio_dev_get_multi_block(sdio);
			LOG_WRN("No permission to set multi-block, using default: %d",
				default_val);
			cfg->multi_block = (default_val != 0);
		} else {
			LOG_ERR("Failed to set multi-block: %d", ret);
			return ret;
		}
	} else {
		LOG_DBG("Multi-block set to: %d", cfg->multi_block);
	}

	/* Set low-speed capability */
	ret = sdio_dev_set_low_speed(sdio, cfg->low_speed ? 1 : 0);
	if (ret < 0) {
		if (ret == -EACCES) {
			default_val = sdio_dev_get_low_speed(sdio);
			LOG_WRN("No permission to set low-speed, using default: %d",
				default_val);
			cfg->low_speed = (default_val != 0);
		} else {
			LOG_ERR("Failed to set low-speed: %d", ret);
			return ret;
		}
	} else {
		LOG_DBG("Low-speed set to: %d", cfg->low_speed);
	}

	/* Set high-speed support if specified */
	if (cfg->high_speed > 0) {
		ret = sdio_dev_set_high_speed(sdio, cfg->high_speed);
		if (ret < 0) {
			if (ret == -EACCES) {
				default_val = sdio_dev_support_high_speed(sdio);
				LOG_WRN("No permission to set high-speed, using default: %d",
					default_val);
				cfg->high_speed = default_val;
			} else {
				LOG_ERR("Failed to set high-speed: %d", ret);
				return ret;
			}
		} else {
			LOG_DBG("High-speed set to: %d", cfg->high_speed);
		}
	}

	/* Set UHS mode if specified */
	if (cfg->uhs_mode > 0) {
		ret = sdio_dev_set_uhs(sdio, cfg->uhs_mode);
		if (ret < 0) {
			if (ret == -EACCES) {
				default_val = sdio_dev_get_uhs(sdio);
				LOG_WRN("No permission to set UHS mode, using default: %d",
					default_val);
				cfg->uhs_mode = default_val;
			} else {
				LOG_ERR("Failed to set UHS mode: %d", ret);
				return ret;
			}
		} else {
			LOG_DBG("UHS mode set to: %d", cfg->uhs_mode);
		}
	}

	/* Set drive strength if specified */
	if (cfg->strength > 0) {
		ret = sdio_dev_set_strength(sdio, cfg->strength);
		if (ret < 0) {
			if (ret == -EACCES) {
				default_val = sdio_dev_get_strength(sdio);
				LOG_WRN("No permission to set drive strength, using default: %d",
					default_val);
				cfg->strength = default_val;
			} else {
				LOG_ERR("Failed to set drive strength: %d", ret);
				return ret;
			}
		} else {
			LOG_DBG("Drive strength set to: %d", cfg->strength);
		}
	}

	/* Set asynchronous interrupt support */
	ret = sdio_dev_set_aync_intr(sdio, cfg->async_intr_support ? 1 : 0);
	if (ret < 0) {
		if (ret == -EACCES) {
			default_val = sdio_dev_get_aync_intr(sdio);
			LOG_WRN("No permission to set async interrupt, using default: %d",
				default_val);
			cfg->async_intr_support = (default_val != 0);
		} else {
			LOG_ERR("Failed to set async interrupt: %d", ret);
			return ret;
		}
	} else {
		LOG_DBG("Async interrupt set to: %d", cfg->async_intr_support);
	}
 ...
 * @retval negative errno code on failure
 */
int sdio_dev_func_cfg_init(struct sdio_dev_func *func,
			    struct sdio_func_cfg *func_cfg)
{
	int ret;
	...
	if (func_cfg->fn_code > 0) {
		...
		if (ret < 0) {
			if (ret == -EACCES) {
				...
				LOG_ERR("Failed to set function code for fn%d: %d",
					func_cfg->fn, ret);
				return ret;
			}
		} else {
			LOG_DBG("Function code for fn%d set to: 0x%02x",
				func_cfg->fn, func_cfg->fn_code);
		}
	}
	...
	if (func_cfg->block_size > 0) {
		...
		if (ret < 0) {
			if (ret == -EACCES) {
				...
				LOG_ERR("Failed to set block size for fn%d: %d",
					func_cfg->fn, ret);
				return ret;
			}
		} else {
			LOG_DBG("Block size for fn%d set to: %d",
				func_cfg->fn, func_cfg->block_size);
		}
	}

	LOG_INF("SDIO function %d configuration initialized successfully",
		func_cfg->fn);
	return 0;
}
		  ...
		  const struct sdio_dev_cfg *cfg)
{
	...
	sdio->num_funcs  = 0;
	sdio->cfg = sdev_heap_alloc(sizeof(struct sdio_dev_cfg));
...
int sdio_dev_deinit(struct sdio_dev *sdio)
{
	...
	for (int i = 0; i < CONFIG_SDIO_DEV_MAX_FUNCS; i++) {
		...
		if (ret) {
			LOG_WRN("func%d deinit failed (%d)",
				func->fn, ret);
...
int sdio_dev_get_cccr_version(struct sdio_dev *sdio)
{
	...
	ret = sdev_read_reg8(dev, cccr_addr + SDIO_CCCR_CCCR, &val);
	if (ret) {
		return ret;
	}
...
int sdio_dev_set_cccr_version(struct sdio_dev *sdio, uint8_t version)
{
	...
	ret = sdev_write_reg8(dev, cccr_addr + SDIO_CCCR_CCCR, val);
	if (ret) {
		return ret;
	}
...
int sdio_dev_set_spec_version(struct sdio_dev *sdio, uint8_t version)
{
	...
	ret = sdev_write_reg8(dev, addr, val);
	if (ret) {
		return ret;
	}
...
bool sdio_dev_master_intr_is_enable(const struct sdio_dev *sdio)
{
	...
	ret = sdev_read_reg8(dev, cccr_addr + SDIO_CCCR_INT_EN, &val);
	if (ret) {
		return false;
	}
...
int sdio_dev_set_bus_width(struct sdio_dev *sdio, uint8_t width)
{
	...
	ret = sdev_read_reg8(dev, cccr_addr + SDIO_CCCR_BUS_IF, &val);
	if (ret) {
		return ret;
	}
	...
	ret = sdev_write_reg8(dev, cccr_addr + SDIO_CCCR_BUS_IF, val);
	if (ret) {
		return ret;
	}
...
int sdio_dev_get_multi_block(struct sdio_dev *sdio)
{
	...
	ret = sdev_read_reg8(dev, cccr_addr + SDIO_CCCR_CAPS, &val);
	if (ret) {
		return ret;
	}
...
int sdio_dev_set_multi_block(struct sdio_dev *sdio, int multi_block)
{
	...
	ret = sdev_write_reg8(dev, cccr_addr + SDIO_CCCR_CAPS, val);
	if (ret) {
		return ret;
	}
...
int sdio_dev_support_high_speed(struct sdio_dev *sdio)
{
	...
	ret = sdev_read_reg8(dev, cccr_addr + SDIO_CCCR_SPEED, &val);
	if (ret) {
		return ret;
	}
...
int sdio_dev_set_high_speed(struct sdio_dev *sdio, uint8_t high_speed)
{
	...
	ret = sdev_write_reg8(dev, cccr_addr + SDIO_CCCR_SPEED, val);
	if (ret) {
		return ret;
	}
...
int sdio_dev_get_uhs(struct sdio_dev *sdio)
{
	...
	ret = sdev_read_reg8(dev, cccr_addr + SDIO_CCCR_UHS, &val);
	if (ret) {
		return ret;
	}
...
int sdio_dev_set_uhs(struct sdio_dev *sdio, uint8_t uhs_mode)
{
	...
	ret = sdev_write_reg8(dev, cccr_addr + SDIO_CCCR_UHS, val);
	if (ret) {
		return ret;
	}
...
int sdio_dev_get_strength(struct sdio_dev *sdio)
{
	...
	ret = sdev_read_reg8(dev, cccr_addr + SDIO_CCCR_DRIVE_STRENGTH, &val);
	if (ret) {
		return ret;
	}
...
int sdio_dev_set_strength(struct sdio_dev *sdio, uint8_t strength)
{
	...
	ret = sdev_write_reg8(dev, cccr_addr + SDIO_CCCR_DRIVE_STRENGTH, val);
	if (ret) {
		return ret;
	}
...
int sdio_dev_set_aync_intr(struct sdio_dev *sdio, int aync_int)
{
	...
	ret = sdev_read_reg8(dev, cccr_addr + SDIO_CCCR_INT_EXT, &val);
	if (ret) {
		return ret;
	}
	...
	ret = sdev_write_reg8(dev, cccr_addr + SDIO_CCCR_INT_EXT, val);
	if (ret) {
		return ret;
	}
...
int sdio_dev_func_intr_is_pending(struct sdio_dev_func *func)
{
	...
	ret = sdev_read_reg8(dev, cccr_addr + SDIO_CCCR_INT_P, &val);
	if (ret) {
		return ret;
	}
...
int sdio_dev_set_block_size(struct sdio_dev_func *func, uint16_t block_size)
{
	...
	/* Write LSB */
	ret = sdev_write_reg8(dev, addr, lsb);
	if (ret) {
		return ret;
	}

	/* Write MSB */
	ret = sdev_write_reg8(dev, addr + 1, msb);
	if (ret) {
		return ret;
	}
