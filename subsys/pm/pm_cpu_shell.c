...
static void pm_suspend_threads_cb(const struct k_thread *thread, void *user_data)
{
	...
	for (thread = _kernel.threads; thread != NULL; thread = thread->next_thread) {
		...
		if (strcmp(name, "shell_uart"))
		{
			...
			shell_thread = (struct k_thread *)thread;

			thread_node->thread = (struct k_thread *)thread;
			sys_slist_append(&suspended_threads, &thread_node->node);
		}
		else if(strcmp(name, "main") & strcmp(name, "idle"))
		{
			thread_node->thread = (struct k_thread *)thread;
			sys_slist_append(&suspended_threads, &thread_node->node);
...
void pm_resume_threads(void) {
	...
	while ((node = sys_slist_get(&suspended_threads)) != NULL) {
		...
		k_thread_resume(thread_node->thread);
		free(thread_node);
	}
...
static int cmd_cpu_lps(const struct shell *shell, size_t argc, char **argv)
{
	...
	if (!pm_state_supported(state)) {
		shell_error(shell, "The SoC does not support low power state: %s", state_str);
		return -EINVAL;
	}
...
SHELL_STATIC_SUBCMD_SET_CREATE(cpu_cmds,
    SHELL_CMD(lps_info, NULL, "List supported low power states info by
	    'cpu lps' command", cmd_cpu_lps_info),
    SHELL_CMD(lps, NULL, "Enter specific low power state by
	    'cpu lps specific_low_power_mode'", cmd_cpu_lps),
