				     ...
				     struct http_response_ctx *response_ctx)
{
	...
	if (is_initialize_request) {
		...
		/* get_client_by_uuid_str and increment ref count */
	...
	ret = mcp_server_handle_request(http_transport_state.server_core, &request_data,
					&request_type);
	if (ret < 0) {
		/* On error during initialization, release the persistent reference */
		if (is_initialize_request) {
			release_client(mcp_client);
		} else {
			/* Release transient reference from get_client_by_uuid_str */
	...
	/* The persistent reference from allocate_client stays until disconnect */
	if (!is_initialize_request) {
		client_ref_put(mcp_client);
	}
...
static void log_http_address(struct net_if *iface)
{
	...
	LOG_INF("Accessible at: http://%s:%d%s", CONFIG_NET_CONFIG_MY_IPV4_ADDR,
	...
	if (addr) {
		...
		LOG_INF("HTTP server accessible at: http://%s:%d%s", addr_str, CONFIG_MCP_HTTP_PORT,
...
static void net_addr_event_handler(struct net_mgmt_event_callback *cb, uint64_t mgmt_event,
				   struct net_if *iface)
{
...
int mcp_server_http_init(mcp_server_ctx_t server_ctx)
{
	...
	net_mgmt_init_event_callback(&net_addr_cb, net_addr_event_handler, NET_EVENT_IPV4_ADDR_ADD);
...
#if defined(CONFIG_MCP_HTTP_ADDRESS_STATIC)
	LOG_INF("HTTP transport running on %s:%d, endpoint: %s", CONFIG_NET_CONFIG_MY_IPV4_ADDR,
		CONFIG_MCP_HTTP_PORT, CONFIG_MCP_HTTP_ENDPOINT);
#else
