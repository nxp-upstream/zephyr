...
/* The MCP specification defines a default protocol version, which should be assumed when a client
 * doesn't include any version in its message during the initialization phase. As of 2025-11-25,
 * the default version is 2025-03-26.
 */
#define DEFAULT_PROTOCOL_VERSION "2025-03-26"
...
static int format_response(char *buffer, size_t buffer_size, const char *json_data)
{
	...
	if ((buffer == NULL) || (buffer_size == 0) || (json_data == NULL)) {
		return -EINVAL;
	}
