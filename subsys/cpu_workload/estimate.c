...
int cpu_workload_estimate_get(int cpu_id, struct cpu_workload_estimate *estimate)
{
	...
	struct cpu_load_window history;
	...
	/* Get which threads have just been woken up since the last query?
	 * How many cycles are these new arrivals expected to bring?
	 *
	 ...
	 * a hard error.
	 */
	ret = cpu_load_window_get(cpu_id, &history);
	if (ret == 0) {
