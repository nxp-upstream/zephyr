...
static bool str_has_prefix(struct at_client *at, const char *prefix)
{
	...
	if (strncmp((const char *)at->rsp_buf.data, prefix, len) != 0) {
		return false;
	}
			   ...
			   enum at_result *result)
{
	...
	if ((at->rsp_buf.len >= ok_len) && (strncmp((const char *)at->rsp_buf.data, ok, ok_len) == 0) &&
	    (at_check_byte(buf, '\n') == 0)) {
		*result = AT_RESULT_OK;
		return 0;
	}

	if ((at->rsp_buf.len >= error_len) && (strncmp((const char *)at->rsp_buf.data, error, error_len) == 0) &&
	    (at_check_byte(buf, '\n') == 0)) {
...
static bool is_vgm_or_vgs(struct at_client *at)
{
	...
	if (!strncmp((const char *)at->rsp_buf.data, "VGM", strlen("VGM"))) {
		return true;
	}

	if (!strncmp((const char *)at->rsp_buf.data, "VGS", strlen("VGS"))) {
		return true;
	}
...
static bool is_cmer(struct at_client *at)
{
	...
	if (strncmp((const char *)at->rsp_buf.data, cmer, len) == 0) {
		return true;
	}
...
static bool is_ring(struct at_client *at)
{
	...
	if (strncmp((const char *)at->rsp_buf.data, ring, len) == 0) {
		return true;
	}
