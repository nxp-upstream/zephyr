...
static void pce_abort(struct bt_obex_client *client, uint8_t rsp_code, struct net_buf *buf)
{
	...
	if (pbap_pce->cb != NULL && pbap_pce->cb->abort) {
				  ...
				  const uint8_t *pwd)
{
	...
	err = bt_pbap_calculate_rsp_digest(pwd, nonce, (uint8_t *)&result);
	if (err == 0) {
		return memcmp(result, (const uint8_t *)rsp_digest,
			    ...
			    const char *type)
{
	...
	if (atomic_get(&pbap_pse->_flags) == 0) {
