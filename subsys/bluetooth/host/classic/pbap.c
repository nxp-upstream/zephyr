...
static void pbap_pce_transport_connected(struct bt_conn *conn, struct bt_goep *goep)
{
...
static void pbap_pce_transport_disconnected(struct bt_goep *goep)
{
	struct bt_pbap *pbap = CONTAINER_OF(goep, struct bt_pbap, goep);
			...
			struct net_buf *buf)
{
	...
	err = pbap_check_target(buf);
	if (err != 0) {
		LOG_ERR("Failed to check target %d", err);
	}
