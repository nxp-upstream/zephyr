...
struct bt_avrcp_tg {
	...
	bool interim_sent[BT_AVRCP_EVT_VOLUME_CHANGED + 1];
};
...
static void avrcp_tx_buf_destroy(struct net_buf *buf)
{
	...
	if ((tx != NULL) && (tx->tg != NULL)) {
		(void)k_work_reschedule(&(tx->tg->vd_rsp_tx_work), K_NO_WAIT);
	}
...
#define AVRCP_STATUS_IS_REJECTED(status) (\
	((unsigned)(status) < BT_AVRCP_STATUS_ADDRESSED_PLAYER_CHANGED) &&    \
	((unsigned)(status) != BT_AVRCP_STATUS_OPERATION_COMPLETED)           \
)
...
static int bt_avrcp_rsp_to_status(uint8_t rsp, struct net_buf *buf, uint8_t *out_status)
{
	uint8_t status;

	if ((out_status == NULL) || (buf == NULL)) {
		return -EINVAL;
	}
	...
	case BT_AVRCP_RSP_REJECTED:
		if (buf->len < sizeof(status)) {
			...
			*out_status = BT_AVRCP_STATUS_INTERNAL_ERROR;
			return -EINVAL;
		}
...
static int add_fragment_data(struct bt_avrcp_ct *ct, const uint8_t *data, uint16_t data_len)
{
	...
	ctx = get_fragmentation_context(ct);
					       ...
					       uint8_t op_pdu_id, uint8_t target_pdu_id)
{
	...
	/* Add target PDU ID to payload */
	if (net_buf_tailroom(buf) < sizeof(target_pdu_id)) {
				 ...
				 struct net_buf *buf)
{
	...
	if ((err < 0) || (rsp_code == BT_AVRCP_RSP_REJECTED)) {
		avrcp_ct_cb->get_caps_rsp(get_avrcp_ct(avrcp), tid, status, NULL);
		return;
	}
					      ...
					      uint8_t rsp_code, struct net_buf *buf)
{
	...
	if ((avrcp_ct_cb == NULL) || (avrcp_ct_cb->notification_rsp == NULL)) {
		return;
	}
	...
	if (rsp_code == BT_AVRCP_RSP_REJECTED) {
		...
		status = net_buf_pull_u8(buf);
		avrcp_ct_cb->notification_rsp(get_avrcp_ct(avrcp), tid, status, 0, NULL);
		return;
	}
	...
	if (rsp_code == BT_AVRCP_RSP_INTERIM) {
		...
		avrcp_ct_cb->notification_rsp(get_avrcp_ct(avrcp), tid, BT_AVRCP_STATUS_SUCCESS,
					    ...
					    uint8_t rsp_code, struct net_buf *buf)
{
	...
	err = bt_avrcp_rsp_to_status(rsp_code, buf, &status);
	if ((err < 0) || (rsp_code == BT_AVRCP_RSP_REJECTED)) {
		avrcp_ct_cb->set_absolute_volume_rsp(get_avrcp_ct(avrcp), tid, status, 0);
		return;
	}
						      ...
						      uint8_t rsp_code, struct net_buf *buf)
{
	...
	err = bt_avrcp_rsp_to_status(rsp_code, buf, &status);
	if ((err < 0) || (rsp_code == BT_AVRCP_RSP_REJECTED)) {
		avrcp_ct_cb->list_player_app_setting_attrs_rsp(get_avrcp_ct(avrcp), tid, status,
							       NULL);
		return;
	}
						     ...
						     uint8_t rsp_code, struct net_buf *buf)
{
	...
	err = bt_avrcp_rsp_to_status(rsp_code, buf, &status);
	if ((err < 0) || (rsp_code == BT_AVRCP_RSP_REJECTED)) {
		avrcp_ct_cb->list_player_app_setting_vals_rsp(get_avrcp_ct(avrcp), tid, status,
							      NULL);
		return;
	}
							...
							uint8_t rsp_code, struct net_buf *buf)
{
	...
	err = bt_avrcp_rsp_to_status(rsp_code, buf, &status);
	if ((err < 0) || (rsp_code == BT_AVRCP_RSP_REJECTED)) {
		avrcp_ct_cb->get_curr_player_app_setting_val_rsp(get_avrcp_ct(avrcp), tid, status,
								 NULL);
		return;
	}
							 ...
							 uint8_t rsp_code, struct net_buf *buf)
{
	...
	err = bt_avrcp_rsp_to_status(rsp_code, buf, &status);
	if ((err < 0) || (rsp_code == BT_AVRCP_RSP_REJECTED)) {
		avrcp_ct_cb->get_player_app_setting_attr_text_rsp(get_avrcp_ct(avrcp), tid, status,
								  NULL);
		return;
	}
							...
							uint8_t rsp_code, struct net_buf *buf)
{
	...
	err = bt_avrcp_rsp_to_status(rsp_code, buf, &status);
	if ((err < 0) || (rsp_code == BT_AVRCP_RSP_REJECTED)) {
		avrcp_ct_cb->get_player_app_setting_val_text_rsp(get_avrcp_ct(avrcp), tid, status,
								 NULL);
		return;
	}
					  ...
					  uint8_t rsp_code, struct net_buf *buf)
{
	...
	err = bt_avrcp_rsp_to_status(rsp_code, buf, &status);
	if ((err < 0) || (rsp_code == BT_AVRCP_RSP_REJECTED)) {
		avrcp_ct_cb->get_element_attrs_rsp(get_avrcp_ct(avrcp), tid, status, NULL);
		return;
	}
					...
					uint8_t rsp_code, struct net_buf *buf)
{
	...
	err = bt_avrcp_rsp_to_status(rsp_code, buf, &status);
	if ((err < 0) || (rsp_code == BT_AVRCP_RSP_REJECTED)) {
		avrcp_ct_cb->get_play_status_rsp(get_avrcp_ct(avrcp), tid, status, NULL);
		return;
	}
...
static const struct avrcp_pdu_vendor_handler rsp_vendor_handlers[] = {
	{ BT_AVRCP_PDU_ID_GET_CAPS,            sizeof(uint8_t),
	  BT_AVRCP_CTYPE_STATUS,            process_get_caps_rsp },
	{ BT_AVRCP_PDU_ID_REGISTER_NOTIFICATION,    sizeof(uint8_t),
	  BT_AVRCP_CTYPE_NOTIFY,            process_register_notification_rsp },
	{ BT_AVRCP_PDU_ID_SET_ABSOLUTE_VOLUME,        sizeof(uint8_t),
	  BT_AVRCP_CTYPE_CONTROL,            process_set_absolute_volume_rsp },
	{ BT_AVRCP_PDU_ID_LIST_PLAYER_APP_SETTING_ATTRS, sizeof(uint8_t),
	...
	{ BT_AVRCP_PDU_ID_GET_CURR_PLAYER_APP_SETTING_VAL, sizeof(uint8_t),
	  BT_AVRCP_CTYPE_STATUS,            process_get_curr_player_app_setting_val_rsp },
	{ BT_AVRCP_PDU_ID_SET_PLAYER_APP_SETTING_VAL,    0,
	  BT_AVRCP_CTYPE_CONTROL,            process_set_player_app_setting_val_rsp },
	...
	{ BT_AVRCP_PDU_ID_INFORM_DISPLAYABLE_CHAR_SET,    0,
	  BT_AVRCP_CTYPE_CONTROL,            process_inform_displayable_char_set_rsp },
	{ BT_AVRCP_PDU_ID_INFORM_BATT_STATUS_OF_CT,    0,
	  BT_AVRCP_CTYPE_CONTROL,            process_inform_batt_status_of_ct_rsp },
	{ BT_AVRCP_PDU_ID_GET_ELEMENT_ATTRS,        sizeof(uint8_t),
	  BT_AVRCP_CTYPE_STATUS,            process_get_element_attrs_rsp },
	{ BT_AVRCP_PDU_ID_GET_PLAY_STATUS,        sizeof(uint8_t),
	  BT_AVRCP_CTYPE_STATUS,            process_get_play_status_rsp },
	{ BT_AVRCP_PDU_ID_SET_ADDRESSED_PLAYER,        sizeof(uint8_t),
	  BT_AVRCP_CTYPE_CONTROL,            process_set_addressed_player_rsp },
	{ BT_AVRCP_PDU_ID_PLAY_ITEM,            sizeof(uint8_t),
	  BT_AVRCP_CTYPE_CONTROL,            process_play_item_rsp },
	{ BT_AVRCP_PDU_ID_ADD_TO_NOW_PLAYING,        sizeof(uint8_t),
	  BT_AVRCP_CTYPE_CONTROL,            process_add_to_now_playing_rsp },
};
					       ...
					       struct net_buf *buf)
{
	...
	if (buf->len != sys_be16_to_cpu(pdu->param_len)) {
				...
				struct net_buf *buf)
{
	...
	if ((avrcp_tg_cb == NULL) || (avrcp_tg_cb->get_cap_req == NULL)) {
		error_code = BT_AVRCP_STATUS_NOT_IMPLEMENTED;
		goto err_rsp;
	}
...
err_rsp:
	err = bt_avrcp_tg_send_vendor_err_rsp(get_avrcp_tg(avrcp), tid,
					      BT_AVRCP_PDU_ID_REGISTER_NOTIFICATION, error_code);
	if (err < 0) {
		LOG_ERR("Failed to send register notification error response");
	}
}
						 ...
						 uint8_t ctype_or_rsp, struct net_buf *buf)
{
	...
	if (battery_status > BT_AVRCP_PLAYER_ATTR_SCAN) {
	...
	return avrcp_tg_cb->inform_batt_status_of_ct_req(get_avrcp_tg(avrcp), tid, battery_status);
...
static const struct avrcp_pdu_vendor_handler cmd_vendor_handlers[] = {
	{ BT_AVRCP_PDU_ID_REQ_CONTINUING_RSP,        sizeof(uint8_t),
	  BT_AVRCP_CTYPE_CONTROL,            handle_avrcp_continuing_rsp },
	{ BT_AVRCP_PDU_ID_ABORT_CONTINUING_RSP,        sizeof(uint8_t),
	  BT_AVRCP_CTYPE_CONTROL,            handle_avrcp_abort_continuing_rsp },
	{ BT_AVRCP_PDU_ID_GET_CAPS,            sizeof(uint8_t),
	  BT_AVRCP_CTYPE_STATUS,            process_get_caps_cmd },
	{ BT_AVRCP_PDU_ID_REGISTER_NOTIFICATION,
	  sizeof(struct bt_avrcp_register_notification_cmd),
	  BT_AVRCP_CTYPE_NOTIFY,            avrcp_register_notification_cmd_handler },
	{ BT_AVRCP_PDU_ID_SET_ABSOLUTE_VOLUME,        sizeof(uint8_t),
	  BT_AVRCP_CTYPE_CONTROL,            process_set_absolute_volume_cmd },
	{ BT_AVRCP_PDU_ID_LIST_PLAYER_APP_SETTING_ATTRS,    0,
	  BT_AVRCP_CTYPE_STATUS,            process_list_player_app_setting_attrs_cmd },
	{ BT_AVRCP_PDU_ID_LIST_PLAYER_APP_SETTING_VALS,
	...
	{ BT_AVRCP_PDU_ID_GET_CURR_PLAYER_APP_SETTING_VAL,
	  ...
	  BT_AVRCP_CTYPE_STATUS,            process_get_curr_player_app_setting_val_cmd },
	{ BT_AVRCP_PDU_ID_SET_PLAYER_APP_SETTING_VAL,
	  sizeof(struct bt_avrcp_set_player_app_setting_val_cmd),
	  BT_AVRCP_CTYPE_CONTROL,            process_set_player_app_setting_val_cmd },
	{ BT_AVRCP_PDU_ID_GET_PLAYER_APP_SETTING_ATTR_TEXT,
	...
	{ BT_AVRCP_PDU_ID_GET_PLAYER_APP_SETTING_VAL_TEXT,
	  ...
	  BT_AVRCP_CTYPE_STATUS,            process_get_player_app_setting_val_text_cmd },
	{ BT_AVRCP_PDU_ID_INFORM_DISPLAYABLE_CHAR_SET,
	  sizeof(struct bt_avrcp_inform_displayable_char_set_cmd),
	  BT_AVRCP_CTYPE_CONTROL,            process_inform_displayable_char_set_cmd },
	{ BT_AVRCP_PDU_ID_INFORM_BATT_STATUS_OF_CT,
	  sizeof(struct bt_avrcp_inform_batt_status_of_ct_cmd),
	  BT_AVRCP_CTYPE_CONTROL,            process_inform_batt_status_of_ct_cmd },
	{ BT_AVRCP_PDU_ID_GET_ELEMENT_ATTRS,
	  sizeof(struct bt_avrcp_get_element_attrs_cmd),
	  BT_AVRCP_CTYPE_STATUS,            process_get_element_attrs_cmd },
	{ BT_AVRCP_PDU_ID_GET_PLAY_STATUS,        0,
	  BT_AVRCP_CTYPE_STATUS,            process_get_play_status_cmd },
	{ BT_AVRCP_PDU_ID_SET_ADDRESSED_PLAYER,
	  sizeof(struct bt_avrcp_set_addressed_player_cmd),
	  BT_AVRCP_CTYPE_CONTROL,            process_set_addressed_player_cmd },
	{ BT_AVRCP_PDU_ID_PLAY_ITEM,
	  sizeof(struct bt_avrcp_play_item_cmd),
	  BT_AVRCP_CTYPE_CONTROL,            process_play_item_cmd },
	{ BT_AVRCP_PDU_ID_ADD_TO_NOW_PLAYING,
	  sizeof(struct bt_avrcp_add_to_now_playing_cmd),
...
static int avrcp_ct_handle_set_browsed_player(struct bt_avrcp *avrcp,
					      uint8_t tid, struct net_buf *buf)
{
	if ((avrcp_ct_cb == NULL) || (avrcp_ct_cb->set_browsed_player_rsp == NULL)) {
		return -EINVAL;
	}

	avrcp_ct_cb->set_browsed_player_rsp(get_avrcp_ct(avrcp), tid, buf);
...
static int avrcp_ct_handle_get_folder_items(struct bt_avrcp *avrcp,
					    uint8_t tid, struct net_buf *buf)
{
	if ((avrcp_ct_cb == NULL) || (avrcp_ct_cb->get_folder_items_rsp == NULL)) {
		return -EINVAL;
	}

	avrcp_ct_cb->get_folder_items_rsp(get_avrcp_ct(avrcp), tid, buf);
	return 0;
}

static int avrcp_ct_handle_change_path(struct bt_avrcp *avrcp,
				       uint8_t tid, struct net_buf *buf)
{
	uint8_t status;
	uint32_t num_items = 0;

	if ((avrcp_ct_cb == NULL) || (avrcp_ct_cb->change_path_rsp == NULL)) {
		return -EINVAL;
	}
	...
	avrcp_ct_cb->change_path_rsp(get_avrcp_ct(avrcp), tid, status, num_items);
	return 0;
}
...
static int avrcp_ct_handle_get_item_attrs(struct bt_avrcp *avrcp,
					  uint8_t tid, struct net_buf *buf)
{
	if ((avrcp_ct_cb == NULL) || (avrcp_ct_cb->get_item_attrs_rsp == NULL)) {
		return -EINVAL;
	}

	avrcp_ct_cb->get_item_attrs_rsp(get_avrcp_ct(avrcp), tid, buf);
	return 0;
}
...
static int avrcp_ct_handle_get_total_number_of_items(struct bt_avrcp *avrcp,
						     uint8_t tid, struct net_buf *buf)
{
	if ((avrcp_ct_cb == NULL) || (avrcp_ct_cb->get_total_number_of_items_rsp == NULL)) {
		return -EINVAL;
	}

	avrcp_ct_cb->get_total_number_of_items_rsp(get_avrcp_ct(avrcp), tid, buf);
	return 0;
}

static int avrcp_ct_handle_search(struct bt_avrcp *avrcp,
				  uint8_t tid, struct net_buf *buf)
{
	if ((avrcp_ct_cb == NULL) || (avrcp_ct_cb->search_rsp == NULL)) {
		return -EINVAL;
	}

	avrcp_ct_cb->search_rsp(get_avrcp_ct(avrcp), tid, buf);
	return 0;
}
						   ...
						   uint8_t tid, struct net_buf *buf)
{
	...
	if ((avrcp_ct_cb == NULL) || (avrcp_ct_cb->browsing_general_reject_rsp == NULL)) {
		return -EINVAL;
	}

	status = net_buf_pull_u8(buf);
	avrcp_ct_cb->browsing_general_reject_rsp(get_avrcp_ct(avrcp), tid, status);
	return 0;
}
...
static const struct avrcp_pdu_handler rsp_brow_handlers[] = {
	...
	{ BT_AVRCP_PDU_ID_GET_FOLDER_ITEMS, sizeof(struct bt_avrcp_get_folder_items_rsp),
	  avrcp_ct_handle_get_folder_items},
	{ BT_AVRCP_PDU_ID_CHANGE_PATH, sizeof(struct bt_avrcp_change_path_rsp),
	  avrcp_ct_handle_change_path},
	{ BT_AVRCP_PDU_ID_GET_ITEM_ATTRS, sizeof(struct bt_avrcp_get_item_attrs_rsp),
	  avrcp_ct_handle_get_item_attrs},
	{ BT_AVRCP_PDU_ID_GET_TOTAL_NUMBER_OF_ITEMS,
	  sizeof(struct bt_avrcp_get_total_number_of_items_rsp),
	  avrcp_ct_handle_get_total_number_of_items},
	{ BT_AVRCP_PDU_ID_SEARCH, sizeof(struct bt_avrcp_search_rsp), avrcp_ct_handle_search},
						...
						struct net_buf *buf)
{
	...
	if ((avrcp_tg_cb == NULL) || (avrcp_tg_cb->get_folder_items_req == NULL)) {
		goto error_rsp;
	}

	avrcp_tg_cb->get_folder_items_req(get_avrcp_tg(avrcp), tid, buf);
	return 0;
error_rsp:
	rsp_buf = bt_avrcp_create_pdu(NULL);
	__ASSERT(rsp_buf != NULL, "Failed to allocate response buffer");
	...
	err = bt_avrcp_tg_send_get_folder_items_rsp(get_avrcp_tg(avrcp), tid, rsp_buf);
	if (err < 0) {
...
static int avrcp_tg_handle_change_path_req(struct bt_avrcp *avrcp, uint8_t tid,
					   struct net_buf *buf)
{
	if ((avrcp_tg_cb == NULL) || (avrcp_tg_cb->change_path_req == NULL)) {
		return bt_avrcp_tg_send_change_path_rsp(get_avrcp_tg(avrcp), tid,
						BT_AVRCP_STATUS_INTERNAL_ERROR, 0U);
	}

	avrcp_tg_cb->change_path_req(get_avrcp_tg(avrcp), tid, buf);
	return 0;
}
					      ...
					      struct net_buf *buf)
{
	...
	if ((avrcp_tg_cb == NULL) || (avrcp_tg_cb->get_item_attrs_req == NULL)) {
		goto error_rsp;
	}

	avrcp_tg_cb->get_item_attrs_req(get_avrcp_tg(avrcp), tid, buf);
	return 0;
error_rsp:
	rsp_buf = bt_avrcp_create_pdu(NULL);
	__ASSERT(rsp_buf != NULL, "Failed to allocate response buffer");
	...
	err = bt_avrcp_tg_send_get_item_attrs_rsp(get_avrcp_tg(avrcp), tid, rsp_buf);
	if (err < 0) {
							 ...
							 uint8_t tid, struct net_buf *buf)
{
	...
	if ((avrcp_tg_cb == NULL) || (avrcp_tg_cb->get_total_number_of_items_req == NULL)) {
		goto error_rsp;
	}

	scope = net_buf_pull_u8(buf);

	avrcp_tg_cb->get_total_number_of_items_req(get_avrcp_tg(avrcp), tid, scope);
	return 0;
error_rsp:
	rsp_buf = bt_avrcp_create_pdu(NULL);
	__ASSERT(rsp_buf != NULL, "Failed to allocate response buffer");
	...
	err = bt_avrcp_tg_send_get_total_number_of_items_rsp(get_avrcp_tg(avrcp), tid, rsp_buf);
	if (err < 0) {
				      ...
				      struct net_buf *buf)
{
	...
	if ((avrcp_tg_cb == NULL) || (avrcp_tg_cb->search_req == NULL)) {
		goto error_rsp;
	}

	avrcp_tg_cb->search_req(get_avrcp_tg(avrcp), tid, buf);
	return 0;
error_rsp:
	rsp_buf = bt_avrcp_create_pdu(NULL);
	__ASSERT(rsp_buf != NULL, "Failed to allocate response buffer");
	...
	err = bt_avrcp_tg_send_search_rsp(get_avrcp_tg(avrcp), tid, rsp_buf);
	if (err < 0) {
				      ...
				      uint32_t interval, bt_avrcp_notification_cb_t cb)
{
	...
	if (avrcp_ct_cb->notification_rsp == NULL) {
...
int bt_avrcp_ct_get_cap(struct bt_avrcp_ct *ct, uint8_t tid, uint8_t cap_id)
{
	...
	if (avrcp_ct_cb->get_caps_rsp == NULL) {
		LOG_WRN("Rsp callback not registered");
		return -EOPNOTSUPP;
	}
...
int bt_avrcp_ct_set_absolute_volume(struct bt_avrcp_ct *ct, uint8_t tid, uint8_t absolute_volume)
{
	...
	if (avrcp_ct_cb->set_absolute_volume_rsp == NULL) {
		LOG_WRN("Rsp callback not registered");
		return -EOPNOTSUPP;
	}
...
int bt_avrcp_ct_list_player_app_setting_attrs(struct bt_avrcp_ct *ct, uint8_t tid)
{
	...
	if (avrcp_ct_cb->list_player_app_setting_attrs_rsp == NULL) {
		LOG_WRN("Rsp callback not registered");
		return -EOPNOTSUPP;
	}
...
int bt_avrcp_ct_list_player_app_setting_vals(struct bt_avrcp_ct *ct, uint8_t tid, uint8_t attr_id)
{
	...
	if (avrcp_ct_cb->list_player_app_setting_vals_rsp == NULL) {
		LOG_WRN("Rsp callback not registered");
		return -EOPNOTSUPP;
	}
						...
						struct net_buf *buf)
{
	...
	if (avrcp_ct_cb->get_curr_player_app_setting_val_rsp == NULL) {
		LOG_WRN("Rsp callback not registered");
		return -EOPNOTSUPP;
	}
						 ...
						 struct net_buf *buf)
{
	...
	if (avrcp_ct_cb->get_player_app_setting_attr_text_rsp == NULL) {
		LOG_WRN("Rsp callback not registered");
		return -EOPNOTSUPP;
	}
						...
						struct net_buf *buf)
{
	...
	if (avrcp_ct_cb->get_player_app_setting_val_text_rsp == NULL) {
		LOG_WRN("Rsp callback not registered");
		return -EOPNOTSUPP;
	}
					    ...
					    struct net_buf *buf)
{
	...
	if (avrcp_ct_cb->inform_displayable_char_set_rsp == NULL) {
		LOG_WRN("Rsp callback not registered");
		return -EOPNOTSUPP;
	}
					 ...
					 uint8_t battery_status)
{
	...
	if (avrcp_ct_cb->inform_batt_status_of_ct_rsp == NULL) {
		LOG_WRN("Rsp callback not registered");
		return -EOPNOTSUPP;
	}
	...
	if (net_buf_tailroom(buf) < sizeof(battery_status)) {
		LOG_WRN("Not enough headroom: for battery_status");
		net_buf_unref(buf);
		return -ENOMEM;
	}
				  ...
				  struct net_buf *buf)
{
	...
	if (avrcp_ct_cb->get_element_attrs_rsp == NULL) {
		LOG_WRN("Rsp callback not registered");
		return -EOPNOTSUPP;
	}
...
int bt_avrcp_ct_get_play_status(struct bt_avrcp_ct *ct, uint8_t tid)
{
	...
	if (avrcp_ct_cb->get_play_status_rsp == NULL) {
		LOG_WRN("Rsp callback not registered");
		return -EOPNOTSUPP;
	}
...
int bt_avrcp_ct_set_addressed_player(struct bt_avrcp_ct *ct, uint8_t tid, uint16_t player_id)
{
	...
	if (avrcp_ct_cb->set_addressed_player_rsp == NULL) {
		LOG_WRN("Rsp callback not registered");
		return -EOPNOTSUPP;
	}
...
int bt_avrcp_ct_play_item(struct bt_avrcp_ct *ct, uint8_t tid, struct net_buf *buf)
{
	...
	if (avrcp_ct_cb->play_item_rsp == NULL) {
		LOG_WRN("Rsp callback not registered");
		return -EOPNOTSUPP;
	}
...
int bt_avrcp_ct_add_to_now_playing(struct bt_avrcp_ct *ct, uint8_t tid, struct net_buf *buf)
{
	...
	if (avrcp_ct_cb->add_to_now_playing_rsp == NULL) {
		LOG_WRN("Rsp callback not registered");
		return -EOPNOTSUPP;
	}
...
int bt_avrcp_ct_set_browsed_player(struct bt_avrcp_ct *ct, uint8_t tid, uint16_t player_id)
{
	...
	if (avrcp_ct_cb->set_browsed_player_rsp == NULL) {
		LOG_WRN("Rsp callback not registered");
		return -EOPNOTSUPP;
	}
...
int bt_avrcp_ct_get_folder_items(struct bt_avrcp_ct *ct, uint8_t tid, struct net_buf *buf)
{
	...
	if (avrcp_ct_cb->get_folder_items_rsp == NULL) {
		LOG_WRN("Rsp callback not registered");
		return -EOPNOTSUPP;
	}
...
int bt_avrcp_ct_change_path(struct bt_avrcp_ct *ct, uint8_t tid, struct net_buf *buf)
{
	...
	if (avrcp_ct_cb->change_path_rsp == NULL) {
		LOG_WRN("Rsp callback not registered");
		return -EOPNOTSUPP;
	}
...
int bt_avrcp_ct_get_item_attrs(struct bt_avrcp_ct *ct, uint8_t tid, struct net_buf *buf)
{
	...
	if (avrcp_ct_cb->get_item_attrs_rsp == NULL) {
		LOG_WRN("Rsp callback not registered");
		return -EOPNOTSUPP;
	}
...
int bt_avrcp_ct_get_total_number_of_items(struct bt_avrcp_ct *ct, uint8_t tid, uint8_t scope)
{
	...
	if (avrcp_ct_cb->get_total_number_of_items_rsp == NULL) {
		LOG_WRN("Rsp callback not registered");
		return -EOPNOTSUPP;
	}
...
int bt_avrcp_ct_search(struct bt_avrcp_ct *ct, uint8_t tid, struct net_buf *buf)
{
	...
	if (avrcp_ct_cb->search_rsp == NULL) {
		LOG_WRN("Rsp callback not registered");
		return -EOPNOTSUPP;
	}
...
int bt_avrcp_tg_send_get_folder_items_rsp(struct bt_avrcp_tg *tg, uint8_t tid, struct net_buf *buf)
{
	int err;
...
int bt_avrcp_tg_send_change_path_rsp(struct bt_avrcp_tg *tg, uint8_t tid, uint8_t status,
				     uint32_t num_items)
{
	struct bt_avrcp_brow_pdu *hdr;
...
int bt_avrcp_tg_send_get_item_attrs_rsp(struct bt_avrcp_tg *tg, uint8_t tid, struct net_buf *buf)
{
	int err;
...
int bt_avrcp_tg_send_get_total_number_of_items_rsp(struct bt_avrcp_tg *tg, uint8_t tid,
						   struct net_buf *buf)
{
	int err;
...
int bt_avrcp_tg_send_search_rsp(struct bt_avrcp_tg *tg, uint8_t tid, struct net_buf *buf)
{
	int err;
...
int bt_avrcp_tg_send_browsing_general_reject_rsp(struct bt_avrcp_tg *tg, uint8_t tid,
						 uint8_t status)
{
	struct net_buf *buf;
	int err;
				      ...
				      uint8_t event_id, struct bt_avrcp_event_data *data)
{
	...
	switch (status) {
	case BT_AVRCP_STATUS_SUCCESS:
		if (tg->interim_sent[event_id] == false) {
			rsp_code = BT_AVRCP_RSP_INTERIM;
			tg->interim_sent[event_id] = true;
		} else {
			rsp_code = BT_AVRCP_RSP_CHANGED;
			tg->interim_sent[event_id] = false;
		}
		break;
						 ...
						 , uint8_t status, struct net_buf *buf)
{
	...
	if (rsp_code == BT_AVRCP_RSP_REJECTED) {
		status_buf = bt_avrcp_create_pdu(&avrcp_pool);
		if (status_buf == NULL) {
	...
	err = bt_avrcp_tg_send_vendor_rsp(tg, tid, pdu_id, rsp_code,
					 (rsp_code == BT_AVRCP_RSP_REJECTED) ? status_buf : buf);
	if (err < 0) {
		LOG_ERR("Failed to send vendor PDU (err: %d)", err);
	}
	return err;
							  ...
							  uint8_t status, struct net_buf *buf)
{
	...
	return bt_avrcp_tg_send_vendor_dependent_rsp(tg, tid, pud_id, status, buf);
}
