...
static void avrcp_ct_timeout(struct k_work *work)
{
	...
	LOG_WRN("Controller timeout: tid 0x%X, opc 0x%02X", avrcp->ct_req.tid, avrcp->ct_req.opcode);
}
...
static void avrcp_ct_unit_info_handler(struct bt_avrcp *avrcp, struct net_buf *buf, bt_avctp_cr_t cr)
{
...
static void avrcp_tg_unit_info_handler(struct bt_avrcp *avrcp, struct net_buf *buf, bt_avctp_cr_t cr)
{
	struct bt_avrcp_header *avrcp_hdr;
...
static struct net_buf *avrcp_create_unit_pdu(struct bt_avrcp *avrcp, bt_avctp_cr_t cr,
					bt_avrcp_ctype_t ctype, bt_avrcp_rsp_t rsp)
{
	struct net_buf *buf;
	...
	BT_AVRCP_HDR_SET_CTYPE_OR_RSP(&cmd->hdr, cr == BT_AVCTP_CMD ? ctype
								    : rsp);
...
static int bt_avrcp_send_unit_info_err_rsp(struct bt_avrcp *avrcp)
{
	...
	buf = avrcp_create_unit_pdu(avrcp, BT_AVCTP_RESPONSE, BT_AVRCP_CTYPE_STATUS, BT_AVRCP_RSP_REJECTED);
	if (!buf) {
		return -ENOMEM;
	}
...
int bt_avrcp_get_unit_info(struct bt_avrcp *avrcp)
{
	...
	buf = avrcp_create_unit_pdu(avrcp, BT_AVCTP_CMD, BT_AVRCP_CTYPE_STATUS, BT_AVRCP_RSP_STABLE);
	if (!buf) {
		return -ENOMEM;
	}
...
int bt_avrcp_send_unit_info_rsp(struct bt_avrcp *avrcp, struct bt_avrcp_unit_info_rsp *rsp)
{
	...
	buf = avrcp_create_unit_pdu(avrcp, BT_AVCTP_RESPONSE, BT_AVRCP_CTYPE_STATUS, BT_AVRCP_RSP_STABLE);
	if (!buf) {
		return -ENOMEM;
	}
