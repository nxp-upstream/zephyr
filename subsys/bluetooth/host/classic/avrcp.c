...
static atomic_t avctp_refcount = ATOMIC_INIT(0);
...
static int avctp_server_register(void)
{
	...
	/* Use atomic increment to register only once */
	if (atomic_inc(&avctp_refcount) > 0) {
		/* Already registered by another role */
		return 0;
	}
