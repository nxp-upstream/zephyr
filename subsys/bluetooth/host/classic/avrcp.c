					    ...
					    uint8_t rsp_code, struct net_buf *buf)
{
	...
	if (rsp_code != BT_AVRCP_RSP_ACCEPTED) {
		LOG_ERR("Invalid Set Absolute Volume response type: 0x%02x", rsp_code);
		return;
	}
	...
	avrcp_ct_cb->set_absolute_volume_rsp(get_avrcp_ct(avrcp), tid, rsp_code, absolute_volume);
}
						      ...
						      uint8_t rsp_code, struct net_buf *buf)
{
	...
	avrcp_ct_cb->list_player_app_setting_attrs_rsp(get_avrcp_ct(avrcp), tid, rsp_code, buf);
}
						     ...
						     uint8_t rsp_code, struct net_buf *buf)
{
	...
	avrcp_ct_cb->list_player_app_setting_vals_rsp(get_avrcp_ct(avrcp), tid, rsp_code, buf);
}
							...
							uint8_t rsp_code, struct net_buf *buf)
{
	...
	avrcp_ct_cb->get_curr_player_app_setting_val_rsp(get_avrcp_ct(avrcp), tid, rsp_code, buf);
}
						   ...
						   uint8_t rsp_code, struct net_buf *buf)
{
	...
	avrcp_ct_cb->set_player_app_setting_val_rsp(get_avrcp_ct(avrcp), tid, rsp_code);
}
							 ...
							 uint8_t rsp_code, struct net_buf *buf)
{
	...
	avrcp_ct_cb->get_player_app_setting_attr_text_rsp(get_avrcp_ct(avrcp), tid, rsp_code, buf);
}
							...
							uint8_t rsp_code, struct net_buf *buf)
{
	...
	avrcp_ct_cb->get_player_app_setting_val_text_rsp(get_avrcp_ct(avrcp), tid, rsp_code, buf);
}
						    ...
						    uint8_t rsp_code, struct net_buf *buf)
{
	...
	avrcp_ct_cb->inform_displayable_char_set_rsp(get_avrcp_ct(avrcp), tid, rsp_code);
}
						 ...
						 uint8_t rsp_code, struct net_buf *buf)
{
	...
	avrcp_ct_cb->inform_batt_status_of_ct_rsp(get_avrcp_ct(avrcp), tid, rsp_code);
}
					  ...
					  uint8_t rsp_code, struct net_buf *buf)
{
	...
	avrcp_ct_cb->get_element_attrs_rsp(get_avrcp_ct(avrcp), tid, rsp_code, buf);
}
					...
					uint8_t rsp_code, struct net_buf *buf)
{
	...
	avrcp_ct_cb->get_play_status_rsp(get_avrcp_ct(avrcp), tid, rsp_code, buf);
}
					     ...
					     uint8_t rsp_code, struct net_buf *buf)
{
	...
	avrcp_ct_cb->set_addressed_player_rsp(get_avrcp_ct(avrcp), tid, rsp_code, status);
}
				  ...
				  uint8_t rsp_code, struct net_buf *buf)
{
	...
	status = net_buf_pull_u8(buf);
	avrcp_ct_cb->play_item_rsp(get_avrcp_ct(avrcp), tid, rsp_code, status);
}
					   ...
					   uint8_t rsp_code, struct net_buf *buf)
{
	...
	status = net_buf_pull_u8(buf);
	avrcp_ct_cb->add_to_now_playing_rsp(get_avrcp_ct(avrcp), tid, rsp_code, status);
}
...
static const struct avrcp_pdu_vendor_handler rsp_vendor_handlers[] = {
	...
	{ BT_AVRCP_PDU_ID_LIST_PLAYER_APP_SETTING_VALS, 0,
						    ...
						    uint8_t ctype_or_rsp, struct net_buf *buf)
{
	...
	return avrcp_tg_cb->register_notification_req(get_avrcp_tg(avrcp), tid, event_id,
						      interval);
					  ...
					  uint8_t tid, struct net_buf *buf)
{
	...
	avrcp_ct_cb->get_item_attrs_rsp(get_avrcp_ct(avrcp), tid, status);
	return 0;
}
						     ...
						     uint8_t tid, struct net_buf *buf)
{
	...
	if (buf->len < 1U) {
		LOG_ERR("Invalid GET_TOTAL_NUMBER_OF_ITEMS rsp length");
		return -EMSGSIZE;
	}
...
static int avrcp_tg_handle_get_folder_items_req(struct bt_avrcp *avrcp,
                                                uint8_t tid, struct net_buf *buf)
{
	struct net_buf *rsp_buf;
	int err;
...
static int avrcp_tg_handle_change_path_req(struct bt_avrcp *avrcp,
                                           uint8_t tid, struct net_buf *buf)
{
...
static int avrcp_tg_handle_get_item_attrs_req(struct bt_avrcp *avrcp,
                                              uint8_t tid, struct net_buf *buf)
{
	if ((avrcp_tg_cb == NULL) || (avrcp_tg_cb->get_item_attrs_req == NULL)) {
		return bt_avrcp_tg_send_get_item_attrs_rsp(get_avrcp_tg(avrcp), tid,
							   BT_AVRCP_STATUS_INTERNAL_ERROR);
	}
                                                         ...
                                                         uint8_t tid, struct net_buf *buf)
{
	...
	uint8_t scope = net_buf_pull_u8(buf);
...
static int avrcp_tg_handle_search_req(struct bt_avrcp *avrcp,
                                      uint8_t tid, struct net_buf *buf)
{
	struct net_buf *rsp_buf;
	int err;
				  ...
				  const struct bt_avrcp_get_caps_rsp *rsp)
{
	...
	err = bt_avrcp_tg_send_vendor_rsp(tg, tid, BT_AVRCP_PDU_ID_GET_CAPS, BT_AVRCP_RSP_STABLE,
					  buf);
	if (err < 0) {
...
int bt_avrcp_tg_send_change_path_rsp(struct bt_avrcp_tg *tg, uint8_t tid,
                                     uint8_t status, uint32_t num_items)
{
	struct net_buf *buf;
	int err;
...
int bt_avrcp_tg_send_get_item_attrs_rsp(struct bt_avrcp_tg *tg, uint8_t tid, uint8_t status)
{
	struct net_buf *buf;
	int err;
	...
	if (net_buf_tailroom(buf) < sizeof(status)) {
		LOG_ERR("Not enough tailroom for GET_ITEM_ATTRIBUTES rsp");
		net_buf_unref(buf);
		return -ENOMEM;
	}
	net_buf_add_u8(buf, status);
...
int bt_avrcp_tg_send_notification_rsp(struct bt_avrcp_tg *tg, uint8_t tid, uint8_t type,
...
int bt_avrcp_tg_send_absolute_volume_rsp(struct bt_avrcp_tg *tg, uint8_t tid,  uint8_t rsp_code,
					 uint8_t absolute_volume)
{
	struct net_buf *buf;
	int err;
...
int bt_avrcp_tg_send_inform_displayable_char_set_rsp(struct bt_avrcp_tg *tg,
						    uint8_t tid, uint8_t rsp_code)
{
	struct net_buf *buf;
...
int bt_avrcp_tg_send_set_addressed_player_rsp(struct bt_avrcp_tg *tg, uint8_t tid,
					      uint8_t rsp_code, uint8_t status)
{
	struct net_buf *buf;
	int err;
...
int bt_avrcp_tg_send_play_item_rsp(struct bt_avrcp_tg *tg, uint8_t tid, uint8_t rsp_code,
				   uint8_t status)
{
	struct net_buf *buf;
	int err;
