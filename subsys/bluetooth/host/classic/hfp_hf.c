		   ...
		   enum at_cme cme_err)
{
	...
	if (result != AT_RESULT_OK) {
		LOG_WRN("It is ERROR response of AT command %d.", hf->cmd_indicator);
	}

	if (ARRAY_SIZE(cmd_init_list) > hf->cmd_indicator) {
		finish = cmd_init_list[hf->cmd_indicator].finish;
		if (finish) {
			(void)finish(hf_at, result, cme_err);
		}
	} else {
		LOG_ERR("Invalid indicator (%d>=%d)", hf->cmd_indicator,
...
static int at_cmd_init_start(struct bt_hfp_hf *hf)
{
	...
	while (ARRAY_SIZE(cmd_init_list) > hf->cmd_indicator)
	{
		LOG_DBG("Fetch AT command (%d)", hf->cmd_indicator);
		send = cmd_init_list[hf->cmd_indicator].send;
		if (send) {
		...
		if (!err) {
			...
			if (cmd_init_list[hf->cmd_indicator].disconnect) {
				hfp_hf_send_failed(hf);
				break;
			}
			/* Goto next AT command */
