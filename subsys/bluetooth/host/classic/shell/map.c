...
static struct mce_mas_instance *mce_mas_alloc(struct bt_conn *conn)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(mce_mas_instances[index]); i++) {
...
static struct mce_mas_instance *mce_mas_find(struct bt_map_mce_mas *mce_mas)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(mce_mas_instances); i++) {
		for (size_t j = 0; j < ARRAY_SIZE(mce_mas_instances[i]); j++) {
...
static struct mce_mns_instance *mce_mns_find(struct bt_map_mce_mns *mce_mns)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(mce_mns_instances); i++) {
...
static struct mse_mas_instance *mse_mas_alloc(struct bt_conn *conn)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(mse_mas_instances[index]); i++) {
...
static struct mse_mas_instance *mse_mas_find(struct bt_map_mse_mas *mse_mas)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(mse_mas_instances); i++) {
		for (size_t j = 0; j < ARRAY_SIZE(mse_mas_instances[i]); j++) {
...
#define MSE_MAS_ATTRS(idx, _) \
static struct bt_sdp_attribute UTIL_CAT(mse_mas_, UTIL_CAT(idx, _attrs))[] = { \
	BT_SDP_NEW_SERVICE, \
	BT_SDP_LIST( \
		BT_SDP_ATTR_SVCLASS_ID_LIST, \
		BT_SDP_TYPE_SIZE_VAR(BT_SDP_SEQ8, 3), \
		...
		BT_SDP_DATA_ELEM_LIST( \
			...
			{ \
				...
				BT_SDP_DATA_ELEM_LIST( \
					...
					{ \
						BT_SDP_TYPE_SIZE(BT_SDP_UINT8), \
						&mse_server[idx].rfcomm_server.server.rfcomm.channel \
					}, \
				) \
			}, \
			{ \
	...
	BT_SDP_SERVICE_NAME("MAP MAS " STRINGIFY(idx)), \
	{ \
		BT_SDP_ATTR_MAS_INSTANCE_ID, \
		{ \
			BT_SDP_TYPE_SIZE(BT_SDP_UINT8), \
			&mse_server[idx].instance_id \
		} \
	}, \
	{ \
		...
		{ \
			BT_SDP_TYPE_SIZE(BT_SDP_UINT8), \
			&mse_server[idx].supported_msg_type \
		} \
	}, \
	{ \
		BT_SDP_ATTR_GOEP_L2CAP_PSM, \
		{ \
			BT_SDP_TYPE_SIZE(BT_SDP_UINT16), \
			&mse_server[idx].l2cap_server.server.l2cap.psm \
		} \
	}, \
	{ \
		BT_SDP_ATTR_MAP_SUPPORTED_FEATURES, \
		{ \
			BT_SDP_TYPE_SIZE(BT_SDP_UINT32), \
			&mse_server[idx].supported_features \
		} \
	}, \
};
...
static int cmd_mse_mas_push_msg(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (inst->final) {
		const char *msg_handle = "0000000000000001";
...
static int cmd_mse_mas_get_owner_status(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (!bt_obex_has_header(buf, BT_OBEX_HEADER_ID_APP_PARAM)) {
		...
		struct bt_obex_tlv appl_params[] = {
			...
			{BT_MAP_APPL_PARAM_TAG_ID_PRESENCE_TEXT, sizeof(presence_text),
...
static int cmd_app_param_filter_priority(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (err == 0) {
		const char *priority[] = {"no_filtering", "high_priority", "non_high_priority"};
...
static int map_sdp_get_service_name(const struct net_buf *buf, char *name, size_t name_len)
{
	...
	size_t copy_len = MIN(value.text.len, name_len - 1);
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(clear, NULL, "Clear all application parameters",
		      cmd_clear_header_app_param, 1, 0),
