...
bt_pbap_tlv gpbap_appl_param[PBAP_APPL_PARAM_MAX_COUNT];
...
static int cmd_cmd_send(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (appl_param_count > 0U){
		err = bt_obex_add_header_app_param(pbap_pce_app.tx_buf, (size_t)appl_param_count, gpbap_appl_param);
		appl_param_count = 0U;
		if (err) {
...
static int cmd_add_appl_param(const struct shell *sh, size_t argc, char **argv)
{
	...
	if (!strcmp(tag, "ps")){
		gpbap_appl_param[appl_param_count].type = BT_PBAP_APPL_PARAM_TAG_ID_PROPERTY_SELECTOR;
		...
		gpbap_appl_param[appl_param_count].data = &gpbap_appl_param_data[appl_param_count][0];
		...
		sys_put_be16((uint16_t)strtoul(argv[1U], NULL, 10), &gpbap_appl_param_data[appl_param_count][0]);
		gpbap_appl_param[appl_param_count].data = &gpbap_appl_param_data[appl_param_count][0];
	}else if (!strcmp(tag, "lso")){
		gpbap_appl_param[appl_param_count].type = BT_PBAP_APPL_PARAM_TAG_ID_LIST_START_OFFSET;
		gpbap_appl_param[appl_param_count].data_len = 2U;
		sys_put_be16((uint16_t)strtoul(argv[1U], NULL, 10), &gpbap_appl_param_data[appl_param_count][0]);
		gpbap_appl_param[appl_param_count].data = &gpbap_appl_param_data[appl_param_count][0];
	}else if (!strcmp(tag, "rnmc")){
		gpbap_appl_param[appl_param_count].type = BT_PBAP_APPL_PARAM_TAG_ID_RESET_NEW_MISSED_CALLS;
		gpbap_appl_param[appl_param_count].data_len = 1U;
		gpbap_appl_param_data[appl_param_count][0] = strtoul(argv[1U], NULL, 10);
		gpbap_appl_param[appl_param_count].data = &gpbap_appl_param_data[appl_param_count][0];
		...
		gpbap_appl_param[appl_param_count].type = BT_PBAP_APPL_PARAM_TAG_ID_VCARD_SELECTOR_OPERATOR;
		gpbap_appl_param[appl_param_count].data_len = 1U;
		gpbap_appl_param_data[appl_param_count][0] = strtoul(argv[1U], NULL, 10);
		gpbap_appl_param[appl_param_count].data = &gpbap_appl_param_data[appl_param_count][0];
...
SHELL_STATIC_SUBCMD_SET_CREATE(pbap_add_appl_params,
...
SHELL_STATIC_SUBCMD_SET_CREATE(pbap_cmds,
	...
	SHELL_CMD_ARG(pull_vcardlisting_create, NULL, "<name>  <srmp>", cmd_pull_vcardlisting, 1, 2),
	...
	SHELL_CMD_ARG(add_appl_param, &pbap_add_appl_params, "Adding appl params", cmd_common, 1, 0),
	SHELL_SUBCMD_SET_END
);
