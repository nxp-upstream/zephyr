			      ...
			      const struct bt_sdp_discover_params *params)
{
	...
	if (result && result->resp_buf) {
		...
		bt_shell_print("A2SRC Server AVDTP version 0x%04x", param);
			      ...
			      const struct bt_sdp_discover_params *params)
{
	...
	if (result && result->resp_buf) {
		...
		bt_shell_print("A2SNK Server AVDTP version 0x%04x", param);
...
SHELL_STATIC_SUBCMD_SET_CREATE(br_cmds,
	...
	SHELL_CMD_ARG(sdp-find, NULL, "<HFPAG, HFPHF, A2SRC, A2SNK, PNP>", cmd_sdp_find_record, 2, 0),
