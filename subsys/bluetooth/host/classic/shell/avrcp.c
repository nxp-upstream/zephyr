...
static void avrcp_notification_cb_rsp(uint8_t event_id, struct bt_avrcp_event_data *data)
{
...
static void avrcp_get_folder_items_rsp(struct bt_avrcp_ct *ct, uint8_t tid, struct net_buf *buf)
{
	...
	if (buf->len < sys_be16_to_cpu(rsp->num_items)) {
		bt_shell_print("incompleted message");
		return;
	}

	items = net_buf_pull_mem(buf, sys_be16_to_cpu(rsp->num_items));
	for (int i = 0; i < sys_be16_to_cpu(rsp->num_items); i++) {
		bt_shell_print("items %d", items[i]);
	}
}
...
static void avrcp_get_total_number_of_items_rsp(struct bt_avrcp_ct *ct, uint8_t tid, struct net_buf *buf)
{
						    ...
						    uint8_t status, struct net_buf *buf)
{
	...
	bt_shell_print("listplayerappsettingattrs : status=0x%02x", status);
	if (buf == NULL) {
		return;
	}
						   ...
						   uint8_t status, struct net_buf *buf)
{
	...
	bt_shell_print("listplayerappsettingvals : status=0x%02x", status);
	if (buf == NULL) {
		return;
	}
						      ...
						      uint8_t status, struct net_buf *buf)
{
	...
	bt_shell_print("getcurrplayerappsettingval : status=0x%02x", status);
	if (buf == NULL) {
		return;
	}
...
static void avrcp_set_player_app_setting_val_rsp(struct bt_avrcp_ct *ct, uint8_t tid,
						 uint8_t status)
{
	bt_shell_print("SetPlayerAppSettingValue rsp: tid=0x%02x, status=%02x", tid, status);
}
						       ...
						       uint8_t status, struct net_buf *buf)
{
	...
	bt_shell_print("getplayerappsettingattrtext : status=0x%02x", status);
	if (buf == NULL) {
		return;
	}
						      ...
						      uint8_t status, struct net_buf *buf)
{
	...
	bt_shell_print("getplayerappsettingvaltext : status=0x%02x", status);
	if (buf == NULL) {
		return;
	}
...
{
	bt_shell_print("SetAddressedPlayer rsp: tid=0x%02x, status=%02x", tid, status);
}

static void avrcp_play_item_rsp(struct bt_avrcp_ct *ct, uint8_t tid, uint8_t status)
{
	bt_shell_print("Play item rsp: tid=0x%02x, status=%02x", tid, status);
}

static void avrcp_add_to_now_playing_rsp(struct bt_avrcp_ct *ct, uint8_t tid, uint8_t status)
{
	bt_shell_print("Add to now playing rsp: tid=0x%02x, status=%02x", tid, status);
}

static void avrcp_inform_batt_status_of_ct_rsp(struct bt_avrcp_ct *ct, uint8_t tid, uint8_t status)
{
	bt_shell_print("Inform battstatus of ct rsp: tid=0x%02x, status=%02x", tid, status);
}
...
static struct bt_avrcp_ct_cb app_avrcp_ct_cb = {
	...
	.get_folder_items_rsp = avrcp_get_folder_items_rsp,
	.change_path_rsp = avrcp_change_path_rsp,
	.get_item_attrs_rsp = avrcp_get_item_attrs_rsp,
	.get_total_number_of_items_rsp = avrcp_get_total_number_of_items_rsp,
	.search_rsp = avrcp_search_rsp,
	...
	.get_player_app_setting_val_text_rsp = avrcp_get_player_app_setting_val_text_rsp,
	.get_play_status_rsp = avrcp_get_play_status_rsp,
	.inform_batt_status_of_ct_rsp = avrcp_inform_batt_status_of_ct_rsp,
	.set_addressed_player_rsp = avrcp_set_addressed_player_rsp,
	.play_item_rsp = avrcp_play_item_rsp,
	.add_to_now_playing_rsp = avrcp_add_to_now_playing_rsp,
};
...
static struct bt_avrcp_tg_cb app_avrcp_tg_cb = {
	...
	.get_folder_items_req = avrcp_get_folder_items_req,
	.change_path_req = avrcp_change_path_req,
	.get_item_attrs_req = avrcp_get_item_attrs_req,
	.get_total_number_of_items_req = avrcp_get_total_number_of_items_req,
	.search_req = avrcp_search_req,
	...
	.get_player_app_setting_val_text_req = avrcp_get_player_app_setting_val_text_req,
	.inform_batt_status_of_ct_req = avrcp_inform_batt_status_of_ct_req,
	.get_play_status_req = avrcp_get_play_status_req,
	.set_addressed_player_req = avrcp_set_addressed_player_req,
	.play_item_req = avrcp_play_item_req,
	.add_to_now_playing_req = avrcp_add_to_now_playing_req,
};
...
static int cmd_send_get_caps_rsp(const struct shell *sh, int32_t argc, char *argv[])
{
	...
	err = bt_avrcp_tg_send_get_caps_rsp(default_tg, tg_tid, BT_AVRCP_STATUS_SUCCESS, buf);
	if (err < 0) {
...
static int cmd_send_get_element_attrs_rsp(const struct shell *sh, int32_t argc, char *argv[])
{
	...
	err = bt_avrcp_tg_send_get_element_attrs_rsp(default_tg, tg_tid, BT_AVRCP_STATUS_SUCCESS,
						     buf);
	if (err < 0) {
...
static int cmd_ct_register_notification(const struct shell *sh, int argc, char *argv[])
{
	...
	err = bt_avrcp_ct_register_notification(default_ct, get_next_tid(), event_id,
						interval, avrcp_notification_cb_rsp);
	if (err < 0) {
...
static int cmd_tg_send_notification_rsp(const struct shell *sh, int argc, char *argv[])
{
	...
	err = bt_avrcp_tg_send_notification_rsp(default_tg, tg_tid, status, event_id, &data);
	if (err < 0) {
...
static int cmd_tg_send_absolute_volume_rsp(const struct shell *sh, int32_t argc, char *argv[])
{
	...
	err = bt_avrcp_tg_send_absolute_volume_rsp(default_tg, tg_tid, BT_AVRCP_STATUS_SUCCESS,
						   absolute_volume);
	if (err < 0) {
							   ...
							   char *argv[])
{
	...
	err = bt_avrcp_tg_send_get_player_app_setting_val_text_rsp(default_tg, tg_tid,
								   BT_AVRCP_STATUS_SUCCESS, buf);
	if (err < 0) {
...
static int cmd_tg_send_inform_batt_status_of_ct_rsp(const struct shell *sh, int argc, char *argv[])
{
	...
	err = bt_avrcp_tg_send_inform_batt_status_of_ct_rsp(default_tg, tg_tid, status);
	if (err < 0) {
...
static int cmd_tg_send_get_play_status_rsp(const struct shell *sh, int argc, char *argv[])
{
	...
	err = bt_avrcp_tg_send_get_play_status_rsp(default_tg, tg_tid, BT_AVRCP_STATUS_SUCCESS, buf);
	if (err < 0) {
...
static int cmd_tg_send_set_addressed_player_rsp(const struct shell *sh, size_t argc, char **argv)
{
	...
	err = bt_avrcp_tg_send_set_addressed_player_rsp(default_tg, tg_tid, status);
	if (err < 0) {
...
static int cmd_tg_send_play_item_rsp(const struct shell *sh, size_t argc, char **argv)
{
	...
	err = bt_avrcp_tg_send_play_item_rsp(default_tg, tg_tid, status);
	if (err < 0) {
...
static int cmd_tg_send_add_to_now_playing_rsp(const struct shell *sh, size_t argc, char **argv)
{
	...
	err = bt_avrcp_tg_send_add_to_now_playing_rsp(default_tg, tg_tid, status);
	if (err < 0) {
...
static int cmd_tg_send_get_folder_items_rsp(const struct shell *sh, int argc, char *argv[])
{
	...
	err = bt_avrcp_tg_send_get_folder_items_rsp(default_tg, tg_tid, buf);
	if (err < 0) {
...
static int cmd_tg_send_change_path_rsp(const struct shell *sh, int argc, char *argv[])
{
	...
	err = bt_avrcp_tg_send_change_path_rsp(default_tg, tg_tid, BT_AVRCP_STATUS_SUCCESS, 10);
	if (err < 0) {
...
static int cmd_tg_send_get_item_attrs_rsp(const struct shell *sh, int argc, char *argv[])
{
	...
	err = bt_avrcp_tg_send_get_item_attrs_rsp(default_tg, tg_tid, buf);
	if (err < 0) {
...
static int cmd_tg_send_get_total_number_of_items_rsp(const struct shell *sh, int argc, char *argv[])
{
	...
	err = bt_avrcp_tg_send_get_total_number_of_items_rsp(default_tg, tg_tid, buf);
	if (err < 0) {
...
static int cmd_tg_send_search_rsp(const struct shell *sh, int argc, char *argv[])
{
	...
	err = bt_avrcp_tg_send_search_rsp(default_tg, tg_tid, buf);
	if (err < 0) {
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(register_notification, NULL, "register notify <event_id> [interval]",
	...
	SHELL_CMD_ARG(inform_batt, NULL, "Inform Battery Status Of CT <status>",
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(send_get_folder_items_rsp, NULL, HELP_NONE, cmd_tg_send_get_folder_items_rsp,
		      1, 0),
	SHELL_CMD_ARG(send_change_path_rsp, NULL, HELP_NONE, cmd_tg_send_change_path_rsp, 1, 0),
	...
	SHELL_CMD_ARG(send_inform_batt_status_of_ct_rsp, NULL, HELP_NONE,
		      cmd_tg_send_inform_batt_status_of_ct_rsp, 1, 0),
