...
static void handle_get_element_attrs_rsp(struct bt_avrcp_ct *ct, uint8_t tid, struct net_buf *buf)
{
...
static uint8_t test_vendor_dependent_payload[] = {
	0x01U, 0x02U, 0x03U, 0x04U, 0x05U, 0x06U, 0x07U, 0x08U
};

static void avrcp_vendor_dependent_rsp_cb(struct bt_avrcp_ct *ct, uint8_t tid, uint8_t pdu_id,
				     bt_avrcp_rsp_t rsp, struct net_buf *buf)
{
	bt_shell_print("vendor response received, tid=0x%02x, pdu_id=%u,  rsp=0x%02x, len=%u", tid, pdu_id, rsp, buf->len);
	if (BT_AVRCP_PDU_ID_GET_ELEMENT_ATTRS == pdu_id)
	{
		return handle_get_element_attrs_rsp(ct, tid, buf);
	}
...
static void avrcp_vendor_dependent_req_cb(struct bt_avrcp_tg *tg, uint8_t tid, uint8_t pdu_id, struct net_buf *buf)
{
...
static void avrcp_passthrough_cmd_req(struct bt_avrcp_tg *tg, uint8_t tid, struct net_buf *buf)
{
	...
	rsp = net_buf_pull_mem(buf, sizeof(*rsp));
	state = FIELD_GET(BIT(7), rsp->byte0);
	opid = FIELD_GET(GENMASK(6, 0), rsp->byte0);

	if (rsp->data_len > 0U) {
		if (buf->len < rsp->data_len) {
			bt_shell_print("Invalid passthrough data length: %d, buf length = %d",
				       rsp->data_len, buf->len);
		}
	...
	bt_shell_print("AVRCP passthrough command received: opid = 0x%02x (%s), tid=0x%02x, len=%u",
		       opid, state_str, tid, rsp->data_len);

	if (rsp->data_len > 0U && opvu != NULL) {
...
static int cmd_send_passthrough_rsp(const struct shell *sh, int32_t argc, char *argv[])
{
	...
	rsp->byte0 = FIELD_PREP(BIT(7), state) | FIELD_PREP(GENMASK(6, 0), opid);
	if (is_op_vu) {
...
static int cmd_get_element_attrs(const struct shell *sh, int32_t argc, char *argv[])
{
	...
	err = bt_avrcp_ct_vendor_dependent(default_ct, get_next_tid(),
					   BT_AVRCP_PDU_ID_GET_ELEMENT_ATTRS, buf);
	if (err) {
		shell_error(sh, "Failed to send vendor dependent command: %d", err);
		goto failed;
	} else {
		shell_print(sh, "AVRCP CT vendor dependent command sent");
		return 0;
	}
failed:
	net_buf_unref(buf);
...
static int cmd_send_get_element_attrs_rsp(const struct shell *sh, int32_t argc, char *argv[])
{
	...
	if (use_large_attrs) {
		...
		for (int i = 0; i < rsp->num_attrs; i++) {
			...
			net_buf_add_mem(buf, large_media_attrs[i].attr_val,
					large_media_attrs[i].attr_len);
		}
	...
	err = bt_avrcp_tg_send_vendor_dependent_rsp(default_tg, tg_tid,
...
static int cmd_vendor_dependent_ct(const struct shell *sh, int32_t argc, char *argv[])
{
	int err;
	struct net_buf *buf;
	uint8_t tid = get_next_tid();
	...
	net_buf_add_mem(buf, test_vendor_dependent_payload, sizeof(test_vendor_dependent_payload));

	err = bt_avrcp_ct_vendor_dependent(default_ct, tid, 0x00U, buf);
	if (err) {
		shell_error(sh, "Failed to send vendor dependent command: %d", err);
		net_buf_unref(buf);
	} else {
		shell_print(sh, "AVRCP CT vendor dependent command sent, tid=0x%02x", tid);
	}
	return err;
}

static int cmd_vendor_dependent_tg(const struct shell *sh, int32_t argc, char *argv[])
{
	int err;
	struct net_buf *buf;
	uint8_t tid = tg_tid;
	uint8_t pdu_id = 0x00U;
	bt_avrcp_rsp_t rsp = BT_AVRCP_RSP_ACCEPTED;
	...
	buf = bt_avrcp_create_pdu(&avrcp_tx_pool);
	if (buf == NULL) {
		shell_error(sh, "Failed to allocate vendor dependent response buffer");
		return -ENOMEM;
	}
	net_buf_add_mem(buf, test_vendor_dependent_payload, sizeof(test_vendor_dependent_payload));

	err = bt_avrcp_tg_send_vendor_dependent_rsp(default_tg, tid, pdu_id, rsp, buf);
	if (err) {
		shell_error(sh, "Failed to send vendor dependent response: %d", err);
		net_buf_unref(buf);
	} else {
		shell_print(sh, "AVRCP TG vendor dependent response sent, tid=0x%02x", tid);
	}
	return err;
}
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(send_passthrough_rsp, NULL, HELP_PASSTHROUGH_RSP, cmd_send_passthrough_rsp,
		      4, 0),
	SHELL_CMD_ARG(send_vendor_dependent_rsp, NULL, "send vendor dependent response", cmd_vendor_dependent_tg, 1, 0),
	SHELL_SUBCMD_SET_END);
