...
{
	bt_shell_print("AVRCP unit information request received");
}
...
static int cmd_set_unit_info_rsp(const struct shell *sh, int32_t argc, char *argv[])
{
	...
	if (default_avrcp != NULL) {
		err = bt_avrcp_set_unit_info_rsp(default_avrcp, &rsp);
		if (!err) {
			bt_shell_print("AVRCP set unit info response");
		} else {
			shell_error(sh, "Failed to set unit info response");
		}
	} else {
		shell_error(sh, "AVRCP is not connected");
	}
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(set_unit_rsp, NULL, "set unit info response", cmd_set_unit_info_rsp, 1, 0),
