...
static void pce_pull_cb(struct bt_pbap_pce *pbap_pce, uint8_t rsp_code, struct net_buf *buf, char *type)
{
	int err;
...
static int cmd_connect_rsp(const struct shell *sh, size_t argc, char **argv)
{
	...
	if (g_pbap_app->conn == NULL) {
        ...
        mopl = (uint8_t)strtoul(argv[1], NULL, 10);
	rsp = argv[2];
		    ...
		    BT_SDP_DATA_ELEM_LIST(
			    ...
			    {BT_SDP_TYPE_SIZE_VAR(BT_SDP_SEQ8, 5),
			     BT_SDP_DATA_ELEM_LIST(
				     ...
				     {BT_SDP_TYPE_SIZE(BT_SDP_UINT8), &rfcomm_channel}, )},
			    {BT_SDP_TYPE_SIZE_VAR(BT_SDP_SEQ8, 3),
			     BT_SDP_DATA_ELEM_LIST({BT_SDP_TYPE_SIZE(BT_SDP_UUID16),
						    BT_SDP_ARRAY_16(BT_SDP_PROTO_OBEX)}, )}, )),
	BT_SDP_LIST(
		BT_SDP_ATTR_PROFILE_DESC_LIST, BT_SDP_TYPE_SIZE_VAR(BT_SDP_SEQ8, 8),
		BT_SDP_DATA_ELEM_LIST({BT_SDP_TYPE_SIZE_VAR(BT_SDP_SEQ8, 6),
				       BT_SDP_DATA_ELEM_LIST({BT_SDP_TYPE_SIZE(BT_SDP_UUID16),
							      BT_SDP_ARRAY_16(BT_SDP_PBAP_SVCLASS)},
							     {BT_SDP_TYPE_SIZE(BT_SDP_UINT16),
							      BT_SDP_ARRAY_16(0x0102)}, )}, )),
	BT_SDP_SERVICE_NAME("Phonebook Access PSE"),
	{BT_SDP_ATTR_GOEP_L2CAP_PSM, {BT_SDP_TYPE_SIZE(BT_SDP_UINT16), &l2cap_psm}},
	{BT_SDP_ATTR_SUPPORTED_REPOSITORIES,
	 {BT_SDP_TYPE_SIZE(BT_SDP_UINT8), &supported_repositories}},
	{BT_SDP_ATTR_PBAP_SUPPORTED_FEATURES,
	 {BT_SDP_TYPE_SIZE(BT_SDP_UINT32), &supported_features}},
};
...
static int cmd_pse_set_supported_reposotories(const struct shell *sh, size_t argc, char *argv[])
{
	int err = 0;
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	sdp_cmds, SHELL_CMD_ARG(reg, NULL, "", cmd_pse_sdp_reg, 1, 0),
	SHELL_CMD_ARG(set_l2cap_psm, NULL, "<l2cap psm>", cmd_pse_set_l2cap_psm, 2, 0),
	SHELL_CMD_ARG(set_rfcomm_channel, NULL, "<rfcomm channle>", cmd_pse_set_rfcomm_channel, 2,
		      0),
	SHELL_CMD_ARG(set_supported_repositories, NULL, "<repositories>",
		      cmd_pse_set_supported_reposotories, 2, 0),
	SHELL_CMD_ARG(set_supported_features, NULL, "<features>", cmd_pse_supported_features, 2, 0),
...
static int cmd_add_auth_challenge(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (length > APP_PBAP_PWD_MAX_LENGTH) {
		bt_shell_error("the length is to long, %d > %d", length, APP_PBAP_PWD_MAX_LENGTH);
		return -EINVAL;
	}
	...
	if (err != 0) {
		bt_shell_error("generate auth_chanllenage nonce failed, err : %d", err);
		return -EINVAL;
	}
...
static int cmd_add_auth_response(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err = bt_pbap_generate_auth_response(g_pbap_app->pwd, g_pbap_app->peer_auth_challenge_nonce,
					     bt_auth_data);
	if (err != 0) {
		bt_shell_error("generate auth_chanllenage response failed, err : %d", err);
		return -EINVAL;
	}
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(connect_rsp, NULL, "<mopl> <rsp: unauth,success,error> [rsp_code]", cmd_connect_rsp,
		      3, 1),
...
static int cmd_add_list_start_offset(const struct shell *sh, size_t argc, char *argv[])
{
	...
	return add_header_appl_param(BT_PBAP_APPL_PARAM_TAG_ID_LIST_START_OFFSET, sizeof(data), data,
				    last);
}
...
static int cmd_add_property_selector(const struct shell *sh, size_t argc, char *argv[])
{
	...
	return add_header_appl_param(BT_PBAP_APPL_PARAM_TAG_ID_PROPERTY_SELECTOR, sizeof(data), data,
				    last);
}
...
static int cmd_add_reset_new_missed_calls(const struct shell *sh, size_t argc, char *argv[])
{
	...
	return add_header_appl_param(BT_PBAP_APPL_PARAM_TAG_ID_RESET_NEW_MISSED_CALLS, sizeof(reset),
				    &reset, last);
}
