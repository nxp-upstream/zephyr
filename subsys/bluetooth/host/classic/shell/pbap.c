			    ...
			    const struct bt_sdp_discover_params *params)
{
	...
	if (result == NULL || result->resp_buf == NULL) {
		bt_shell_error("No SDP PSE data from remote %s", addr);
...
static int pbap_pce_transport_connect(uint8_t channel, uint16_t psm)
{
	...
	if (err != 0) {
		bt_shell_error("Fail to send tranpsport connect %d", err);
...
{
	bt_shell_print("pbap_pse get pull_vcardlisting request");
	pbap_pse_pull_cb(pbap_pse, buf);
}

static void pbap_pse_pull_vcardentry_cb(struct bt_pbap_pse *pbap_pse, struct net_buf *buf)
{
	bt_shell_print("pbap_pse get pull_vcardentry request");
	pbap_pse_pull_cb(pbap_pse, buf);
}
...
static int pbap_pull_rsp_add_initial_headers(struct bt_pbap_app *pbap_app)
{
	...
	pbap_app->srm_state &= ~LOCAL_SRM_ENABLED;
	if (g_pbap_app->pbap_pse._goep._goep_v2) {
...
static int pbap_pull_rsp(const struct shell *sh, size_t argc, char *argv[], const char *type)
{
	...
	length = length - g_pbap_app->tx_buf->len - 1 - 2;
	...
	if (rsp_code == BT_OBEX_RSP_CODE_CONTINUE) {
		if (wait_to_send_cmd(g_pbap_app, g_pbap_app->pbap_pse._goep._goep_v2)) {
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	pse_cmds, SHELL_CMD_ARG(sdp_reg, NULL, "", cmd_pse_sdp_reg, 1, 0),
	SHELL_CMD_ARG(rfcomm_register, NULL, "", cmd_rfcomm_register, 1, 0),
	...
	SHELL_CMD_ARG(pull_phonebook_rsp, NULL,
		      ...
		      cmd_pull_phonebook_rsp, 3, 1),
	SHELL_CMD_ARG(pull_vcardlisting_rsp, NULL,
		      "<wait: 0,1> <rsp: continue, success, error> [rsp_code]",
		      cmd_pull_vcardlisting_rsp, 3, 1),
	SHELL_CMD_ARG(pull_vcardentry_rsp, NULL,
		      "<wait: 0,1> <rsp: continue, success, error> [rsp_code]",
		      cmd_pull_vcardentry_rsp, 3, 1),
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	add_appl_param, SHELL_CMD_ARG(Order, NULL, "<0,1,2> ", cmd_add_order, 2, 0),
	SHELL_CMD_ARG(SearchValue, NULL, "<text> ", cmd_add_search_value, 2, 0),
	SHELL_CMD_ARG(SearchAttribute, NULL, "<0x00-0x02> ", cmd_add_search_property, 2, 0),
	SHELL_CMD_ARG(MaxListCount, NULL, "<count_hex> ", cmd_add_max_list_count, 2, 0),
	SHELL_CMD_ARG(ListStartOffset, NULL, "<offset_hex> ", cmd_add_list_start_offset, 2, 0),
	SHELL_CMD_ARG(PropertySelector, NULL, "<selector_hex> ", cmd_add_property_selector, 2, 0),
	SHELL_CMD_ARG(Format, NULL, "<0x00|0x01> ", cmd_add_format, 2, 0),
	SHELL_CMD_ARG(PhonebookSize, NULL, "<size_hex> ", cmd_add_phonebook_size, 2, 0),
	SHELL_CMD_ARG(NewMissedCalls, NULL, "<count_hex> ", cmd_add_new_missed_calls, 2, 0),
	SHELL_CMD_ARG(PrimaryFolderVersion, NULL, "<32_char_hex> ", cmd_add_primary_folder_version,
		      2, 0),
	SHELL_CMD_ARG(SecondaryFolderVersion, NULL, "<32_char_hex> ",
		      cmd_add_secondary_folder_version, 2, 0),
	SHELL_CMD_ARG(vCardSelector, NULL, "<selector_hex> ", cmd_add_vcard_selector, 2, 0),
	SHELL_CMD_ARG(DatabaseIdentifier, NULL, "<32_char_hex> ", cmd_add_database_identifier, 2,
		      0),
	SHELL_CMD_ARG(vCardSelectorOperator, NULL, "<0x00|0x01> ", cmd_add_vcard_selector_operator,
		      2, 0),
	SHELL_CMD_ARG(ResetNewMissedCalls, NULL, "<0x01> ", cmd_add_reset_new_missed_calls, 2, 0),
	SHELL_CMD_ARG(PbapSupportedFeatures, NULL, "<features_hex> ",
		      cmd_add_pbap_supported_features, 2, 0),
	SHELL_SUBCMD_SET_END);
