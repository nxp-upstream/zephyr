			    ...
			    const struct bt_sdp_discover_params *params)
{
	...
	bt_shell_print("PSE feature param 0x%08x", param);
...
static int pbap_pull_rsp(const struct shell *sh, size_t argc, char *argv[], const char *type)
{
	...
	length = MIN(g_pbap_app->peer_mopl, g_pbap_app->pbap_pse._goep.obex.tx.mtu);
	length = MIN(length, strlen(value));

	length = length - g_pbap_app->tx_buf->len;
	if (length > 3) {
		err = pbap_pull_rsp_add_body_header(g_pbap_app, rsp_code, (uint16_t)length, value);
		if (err != 0) {
			goto failed;
		}
	}
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(disconnect_rsp, NULL, "<rsp: success,error> [rsp_code]", cmd_disconnect_rsp,
		      2, 1),
	SHELL_CMD_ARG(pull_phone_book_rsp, NULL,
		      "<wait: 0,1> <rsp: continue, success, error> [rsp_code]",
		      cmd_pull_phone_book_rsp, 3, 1),
	SHELL_CMD_ARG(pull_vcard_listing_rsp, NULL,
		      "<wait: 0,1> <rsp: continue, success, error> [rsp_code]",
		      cmd_pull_vcard_listing_rsp, 3, 1),
	SHELL_CMD_ARG(pull_vcard_entry_rsp, NULL,
		      "<wait: 0,1> <rsp: continue, success, error> [rsp_code]",
		      cmd_pull_vcard_entry_rsp, 3, 1),
...
{
	uint32_t features = BT_PBAP_PCE_SUPPORTED_FEATURES;
	uint8_t data[4];
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	add_appl_param,
	SHELL_CMD_ARG(Order, NULL, "<indexed:0x00,alphanumeric:0x01,phonetic:0x02> ", cmd_add_order,
		      2, 0),
	SHELL_CMD_ARG(SearchValue, NULL, "<text> ", cmd_add_search_value, 2, 0),
	SHELL_CMD_ARG(SearchAttribute, NULL, "<Name:0x00,Number:0x01,Sound:0x02> ",
		      cmd_add_search_property, 2, 0),
	...
	SHELL_CMD_ARG(Format, NULL, "<V2.1:0x00,V3.0:0x01> ", cmd_add_format, 2, 0),
	...
	SHELL_CMD_ARG(vCardSelectorOperator, NULL, "<OR:0x00,AND:0x01> ",
		      cmd_add_vcard_selector_operator, 2, 0),
	SHELL_CMD_ARG(ResetNewMissedCalls, NULL, "<Reset:0x01> ", cmd_add_reset_new_missed_calls, 2,
		      0),
	SHELL_CMD_ARG(PbapSupportedFeatures, NULL, "", cmd_add_pbap_supported_features, 1, 0),
	SHELL_SUBCMD_SET_END);
