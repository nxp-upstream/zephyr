...
static uint32_t calculate_transfers_per_second(const struct usb_ep_descriptor *ep_desc,
					       enum usb_device_speed speed)
{
	uint32_t interval = 1U << (ep_desc->bInterval - 1);
					 ...
					 uint8_t bytes_per_sample)
{
	...
	if (bytes_per_ms <= ep_mps) {
		...
		/* TODO: set endpoint interval as 0x04 */
		return 1000;
	}

	if ((bytes_per_ms / 2) <= ep_mps) {
		LOG_DBG("Optimizing to 0.5ms interval");
		/* TODO: set endpoint interval as 0x03 */
		return 2000;
	}
...
{
	const struct usb_ep_descriptor *const ep_desc = host_data->out_stream_iface.ep;
	...
	transfers_per_sec = calculate_transfers_per_second(ep_desc, speed);

	if (speed == USB_SPEED_SPEED_HS) {
		optimized_tps = try_optimize_hs_interval(host_data, ep_desc,
			       ...
			       struct uhc_transfer *const xfer)
{
	...
	if (!atomic_test_bit(&host_data->device_flags, UAC2_DEVICE_FLAG_STREAMING_IN)) {
		LOG_DBG("Device not streaming i, ignoring callback");
		goto cleanup;
	}
					 ...
					 enum usbh_uac2_stream_dir dir)
{
	...
	if (desc->clock_source != NULL) {
		...
		freq_info->range.dMAX = query.range.subranges[query.range.num_subranges - 1].umax32;
...
static int usbh_uac2_removed(struct usbh_class_data *const c_data)
{
	...
	memset(&host_data->in_descriptors, 0, sizeof(host_data->in_descriptors));
	memset(&host_data->out_descriptors, 0, sizeof(host_data->out_descriptors));
	memset(&host_data->in_stream_iface, 0, sizeof(host_data->in_stream_iface));
	memset(&host_data->out_stream_iface, 0, sizeof(host_data->out_stream_iface));

	host_data->current_ctrl_iface = NULL;

	memset(&host_data->current_in_format, 0, sizeof(host_data->current_in_format));
	memset(&host_data->current_out_format, 0, sizeof(host_data->current_out_format));

	memset(&host_data->in_volume_info, 0, sizeof(host_data->in_volume_info));
	memset(&host_data->out_volume_info, 0, sizeof(host_data->out_volume_info));
	memset(&host_data->in_frequency_info, 0, sizeof(host_data->in_frequency_info));
	memset(&host_data->out_frequency_info, 0, sizeof(host_data->out_frequency_info));
