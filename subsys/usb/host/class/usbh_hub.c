				   ...
				   struct usb_hub_port *const port_instance)
{
	...
	LOG_DBG("Port %d status: wPortStatus=0x%04x, wPortChange=0x%04x",
		port_num, port_sts.wPortStatus, port_sts.wPortChange);

	if ((port_sts.wPortChange & USB_HUB_PORT_CHANGE_CONNECTION) != 0) {
		ret = usbh_req_clear_hcfs_c_port_connection(hub_data->udev, port_num);
		if (ret != 0) {
	...
	if ((port_sts.wPortChange & USB_HUB_PORT_CHANGE_ENABLE) != 0) {
		ret = usbh_req_clear_hcfs_c_port_enable(hub_data->udev, port_num);
		if (ret != 0) {
	...
	if ((port_sts.wPortChange & USB_HUB_PORT_CHANGE_SUSPEND) != 0) {
		...
		ret = usbh_req_clear_hcfs_c_port_suspend(hub_data->udev, port_num);
		if (ret != 0) {
	...
	if ((port_sts.wPortChange & USB_HUB_PORT_CHANGE_OVER_CURRENT) != 0) {
		...
		ret = usbh_req_clear_hcfs_c_port_over_current(hub_data->udev, port_num);
		if (ret != 0) {
...
static void hub_port_process(struct usbh_hub_data *const hub_data,
			     struct usb_hub_port *const port_instance)
{
	struct usb_hub_port_status port_sts;
	struct usb_device *udev;
	...
	while (hub_data->connected) {
		...
		case PORT_STATE_RESETTING:
			...
			if (ret != 0) {
				if (port_instance->reset_count > 0) {
					LOG_WRN("Port %d enumeration failed, retrying (%d left)",
						port_num, port_instance->reset_count);
					port_instance->state = PORT_STATE_DISABLED;
					k_sleep(K_MSEC(CONFIG_USBH_HUB_ENUM_RETRY_DELAY_MS));
					continue;
...
static void hub_process_data(struct usbh_hub_data *const hub_data)
{
	...
	if (hub_changed) {
		...
		if (ret != 0) {
			...
			hub_data->status.wHubStatus = hub_status;
			hub_data->status.wHubChange = hub_change;
			LOG_DBG("Hub status: 0x%04x, change: 0x%04x",
				hub_data->status.wHubStatus,
				hub_data->status.wHubChange);
			clear_local_power =
				(hub_change & USB_HUB_CHANGE_LOCAL_POWER) != 0;
			clear_over_current =
	...
	for (port_index = 0; port_index < hub_data->port_count; ++port_index) {
		...
		hub_port_process(hub_data, port_instance);
	}
...
#define USBH_DEFINE_HUB_CLASS(i, _)						\
	static struct usbh_hub_data usbh_hub_data##i;				\
	USBH_DEFINE_CLASS(UTIL_CAT(usbh_hub_class_, i), &usbh_hub_class_api,	\
			  &usbh_hub_data##i, hub_filters)
