...
find_frame:
	...
	while (head != NULL) {
		const struct uvc_frame_common_descriptor *frame =
			(const struct uvc_frame_common_descriptor *)head;

		if (head->bDescriptorType == USB_DESC_CS_INTERFACE &&
		    frame->bDescriptorSubtype == frame_subtype &&
		    head->bLength >= sizeof(struct uvc_frame_common_descriptor)) {
					 ...
					 uint32_t *found_interval)
{
	...
	while (frames_found < format_header->bNumFrameDescriptors && desc_buf != NULL) {
		const struct uvc_frame_common_descriptor *frame_header = (const void *)desc_buf;
				...
				const struct uvc_frame_common_descriptor **frame, uint32_t *frmival)
{
	...
	for (uint8_t i = 0; i < uncompressed_info->num_uncompressed_formats; i++) {
		uncomp_format = uncompressed_info->uncompressed_format[i];
		...
		if (desc_pixelformat == fmt->pixelformat) {
			...
			if (uvc_host_find_frame_in_format((const void *)uncomp_format, fmt->width,
	...
	if (fmt->pixelformat == VIDEO_PIX_FMT_JPEG) {
		...
		for (uint8_t i = 0; i < mjpeg_info->num_mjpeg_formats; i++) {
			mjpeg_format = mjpeg_info->vs_mjpeg_format[i];
			...
			if (uvc_host_find_frame_in_format((const void *)mjpeg_format, fmt->width,
					       ...
					       uint32_t required_bandwidth)
{
	...
	for (uint8_t i = 0;
		...
		if_desc = host_data->stream_ifaces[i];
...
static int uvc_host_create_format_caps(struct uvc_host_data *const host_data)
{
	...
	for (uint8_t i = 0; i < uncompressed_info->num_uncompressed_formats; i++) {
		...
		if (uncomp_format) {
			total_caps += uncomp_format->bNumFrameDescriptors;
		}
	}
	...
	for (uint8_t i = 0; i < mjpeg_info->num_mjpeg_formats; i++) {
		...
		if (mjpeg_format) {
			total_caps += mjpeg_format->bNumFrameDescriptors;
		}
	}
