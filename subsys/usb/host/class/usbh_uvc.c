...
uvc_host_get_frame_default_intervals(const struct uvc_frame_common_descriptor *frame_desc)
{
	...
	if (frame_desc->bDescriptorSubtype == UVC_VS_FRAME_UNCOMPRESSED ||
		...
		const struct uvc_frame_uncomp_continuous_descriptor *std_desc =
			(const void *)frame_desc;
	...
	if (interval_type == 0) {
		...
		if (frame_desc->bDescriptorSubtype == UVC_VS_FRAME_UNCOMPRESSED ||
		    frame_desc->bDescriptorSubtype == UVC_VS_FRAME_MJPEG) {
			/* For both uncompressed and MJPEG, the structure layout is the same */
			const struct uvc_frame_uncomp_continuous_descriptor *continuous_desc =
				(const void *)frame_desc;
		...
		if (frame_desc->bDescriptorSubtype == UVC_VS_FRAME_FRAME_BASED) {
			const struct uvc_frame_framebased_discrete_descriptor *fb_desc =
				(const void *)frame_desc;
