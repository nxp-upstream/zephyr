...
static void uvc_host_flush_queue(const struct device *dev);
...
static int uvc_host_select_default_format(struct uvc_device *uvc_dev)
{
	...
	if (uncompressed_info->num_uncompressed_formats > 0 && 
		...
		uint32_t pixelformat = uvc_host_guid_to_pixelformat(format->guidFormat);
		if (pixelformat == 0) {
		...
		if (header->bDescriptorType ==  USB_DESC_CS_INTERFACE) {
			...
			case UVC_VC_HEADER: {
				struct uvc_vc_header_descriptor *header_desc = 
					(struct uvc_vc_header_descriptor *)desc_buf;
			...
			case UVC_VC_ENCODING_UNIT: {
				...
				if (uvc_dev->vc_encoding_uinit != NULL) {
			...
			case UVC_VC_EXTENSION_UNIT: {
				...
				if (uvc_dev->vc_Extension_ud != NULL) {
...
/**
 * @brief Parse frame intervals from descriptor
 *
 * Extracts frame interval information from frame descriptor and
 * returns the maximum supported interval value.
 *
 * @param desc_buf Pointer to frame descriptor buffer
 * @return Maximum frame interval in 100ns units
 */
static uint32_t uvc_host_parse_frame_intervals(uint8_t *desc_buf)
{
	uint32_t max_interval = 333333; /* Default 30fps */
	uint8_t interval_type = desc_buf[25]; /* bFrameIntervalType */
	uint8_t *interval_data = desc_buf + 26;
	
	if (interval_type == 0) {
		/* Continuous/stepwise frame intervals: take maximum value */
		max_interval = sys_le32_to_cpu(*(uint32_t*)(interval_data + 4));
	} else if (interval_type > 0) {
		/* Discrete frame intervals: take last (maximum) value */
		max_interval = sys_le32_to_cpu(*(uint32_t*)(interval_data + (interval_type - 1) * 4));
	}
			...
			if (frame_header->bLength >= sizeof(struct uvc_frame_header)) {
				...
				if (frame_width == target_width && frame_height == target_height) {
					...
					*found_interval = (frame_header->bLength >= 26) ? 
									 uvc_host_parse_frame_intervals(desc_buf) : 333333;
					return 0;
				}
				frames_found++;
			}
						  ...
						  uint32_t *frame_interval)
{
	...
	for (int i = 0; i < uncompressed_info->num_uncompressed_formats; i++) {
		...
		uint32_t desc_pixelformat = uvc_host_guid_to_pixelformat(format_desc->guidFormat);
...
{
	struct usb_if_descriptor *best_interface = NULL;
	struct usb_ep_descriptor *best_endpoint = NULL;
	uint32_t min_suitable_bandwidth = UINT32_MAX;
	bool found_suitable = false;
	enum usbh_speed device_speed = uvc_dev->udev->speed;
	
	LOG_DBG("Required bandwidth: %u bytes/sec (device speed: %s)", required_bandwidth,
			(device_speed == USBH_SPEED_HS) ? "High Speed" : "Full Speed");
	...
	for (int i = 0; i < UVC_STREAM_INTERFACES_MAX_ALT && uvc_dev->stream_ifaces[i]; i++) {
		...
		for (int ep = 0; ep < if_desc->bNumEndpoints; ep++) {
			...
			if (ep_desc->bDescriptorType == USB_DESC_ENDPOINT &&
				...
				if (device_speed == USBH_SPEED_HS) {
					uint8_t mult = ((sys_le16_to_cpu(ep_desc->wMaxPacketSize) >> 11) & 0x03) + 1;
					uint32_t interval_uframes = 1 << (ep_desc->bInterval - 1);
					uvc_dev->current_stream_iface_info.cur_ep_mps_mult = max_packet_size * mult;
					ep_bandwidth = (uvc_dev->current_stream_iface_info.cur_ep_mps_mult * 8000) / interval_uframes;
				} else {
					ep_bandwidth = (max_packet_size * 1000) / ep_desc->bInterval;
				}
				
				LOG_DBG("  Interface %u Alt %u EP[%d]: addr=0x%02x, maxpkt=%u, bandwidth=%u",
						if_desc->bInterfaceNumber, if_desc->bAlternateSetting, ep,
						ep_desc->bEndpointAddress, max_packet_size, ep_bandwidth);
				
				/* Check if endpoint satisfies bandwidth requirement and is optimal */
				if (ep_bandwidth >= required_bandwidth && ep_bandwidth < min_suitable_bandwidth) {
					min_suitable_bandwidth = ep_bandwidth;
					best_interface = if_desc;
					best_endpoint = ep_desc;
					found_suitable = true;
					
					LOG_DBG("Found better endpoint: interface %u alt %u EP 0x%02x, bandwidth=%u",
	...
	if (!found_suitable) {
		LOG_ERR("No endpoint can satisfy bandwidth requirement %u", required_bandwidth);
		return -ENOTSUP;
	}
	
	/* Update current streaming interface and endpoint */
	uvc_dev->current_stream_iface_info.current_stream_iface = best_interface;
	uvc_dev->current_stream_iface_info.current_stream_ep = best_endpoint;
	
	LOG_INF("Selected interface %u alternate %u endpoint 0x%02x with bandwidth %u for requirement %u",
			best_interface->bInterfaceNumber, best_interface->bAlternateSetting,
			best_endpoint->bEndpointAddress, min_suitable_bandwidth, required_bandwidth);
						  ...
						  uint32_t width, uint32_t height)
{
	...
	probe.bmHint = sys_cpu_to_le16(0x0001);			/* Frame interval hint */
	probe.bFormatIndex = format->bFormatIndex;		 /* Format index from descriptor */
	probe.bFrameIndex = frame->bFrameIndex;			/* Frame index from descriptor */
	probe.dwFrameInterval = sys_cpu_to_le32(frame_interval); /* Frame interval from descriptor */
	...
	ret = uvc_host_stream_interface_control(uvc_dev, UVC_SET_CUR, UVC_VS_PROBE_CONTROL, &probe, sizeof(probe));
	if (ret) {
	...
	memset(&probe, 0, sizeof(probe)); 
	ret = uvc_host_stream_interface_control(uvc_dev, UVC_GET_CUR, UVC_VS_PROBE_CONTROL, &probe, sizeof(probe));
	if (ret) {
	...
	ret = uvc_host_stream_interface_control(uvc_dev, UVC_SET_CUR, UVC_VS_COMMIT_CONTROL, &probe, sizeof(probe));
	if (ret) {
...
static int uvc_host_set_frame_rate(struct uvc_device *uvc_dev, uint32_t fps)
{
	...
	memset(&probe, 0, sizeof(probe));
	probe.bmHint = sys_cpu_to_le16(0x0001);
	probe.bFormatIndex = uvc_dev->current_format.format_index;
	probe.bFrameIndex = uvc_dev->current_format.frame_index;
	probe.dwFrameInterval = sys_cpu_to_le32(best_frame_interval);
...
{	
	...
	caps->min_vbuf_count = 2;			/* UVC typically needs 2 buffers */
...
static int uvc_host_remove_payload_header(struct net_buf *buf, struct video_buffer *vbuf)
{
	...
	if (payload_len > 0) {
		memmove(vbuf->buffer + vbuf->bytesused, buf->data + header_len, payload_len);
	}
...
{
	const struct device *dt_device = (const struct device *)xfer->priv;
	struct uvc_device *uvc_dev = dt_device->data;
	...
	if (buf->len > 0 && vbuf)
	{
		...
		if (uvc_dev->vbuf_offset + buf->len > vbuf->size) {
			LOG_ERR("Transfer would overflow buffer: offset=%u, len=%u, size=%u",
					uvc_dev->vbuf_offset, buf->len, vbuf->size);
			goto cleanup;
		}
		
		/* Update buffer offset tracking */
		uvc_dev->vbuf_offset += buf->len;
		...
		if (endOfFrame) {
			...
			k_poll_signal_raise(dt_device, VIDEO_BUF_DONE);
		}
	}
...
cleanup:
	...
	uvc_host_flush_queue(dt_device);
	return 0;
}
 ...
 */
static struct uhc_transfer *uvc_host_initiate_transfer(const struct device *dev,
						 struct video_buffer *const vbuf)
{
	...
	/* Validate input parameters */
	if (!dev || !vbuf || !uvc_dev || !uvc_dev->current_stream_iface_info.current_stream_ep) {
	...
	xfer = usbh_xfer_alloc(uvc_dev->udev, uvc_dev->current_stream_iface_info.current_stream_ep->bEndpointAddress,
					uvc_host_stream_iso_req_cb, dev);
	if (!xfer) {
	...
	buf = net_buf_alloc_with_data(&uvc_host_pool, vbuf->buffer, uvc_dev->current_stream_iface_info.cur_ep_mps_mult, K_NO_WAIT);
	if (!buf) {
 ...
 */
static struct net_buf *uvc_host_continue_transfer(const struct device *dev,
						 struct video_buffer *const vbuf)
{
	struct uvc_device *uvc_dev = dev->data;
	struct net_buf *buf;
	...
	/* Allocate buffer at offset position for continuation */
	 buf = net_buf_alloc_with_data(&uvc_host_pool, vbuf->buffer + (uvc_dev->transfer_count * uvc_dev->current_stream_iface_info.cur_ep_mps_mult), uvc_dev->current_stream_iface_info.cur_ep_mps_mult, K_NO_WAIT);
	if (buf == NULL) {
 ...
 * @return 0 on success, negative error code on failure
 */
static int uvc_host_flush_vbuf(const struct device *dev, struct video_buffer *const vbuf)
{
	struct uvc_device *uvc_dev = dev->data;
	struct net_buf *buf;
	struct uhc_transfer *xfer;
	...
	if (uvc_dev->transfer_count == 0) {
		xfer = uvc_host_initiate_transfer(dev, vbuf);
	} else {
		buf = uvc_host_continue_transfer(dev, vbuf);
		xfer->buf = buf;
	}
...
static void uvc_host_flush_queue(const struct device *dev)
{
	...
	while ((vbuf = k_fifo_peek_head(&uvc_dev->fifo_in)) != NULL) {
		ret = uvc_host_flush_vbuf(dev, vbuf);
		if (ret < 0) {
...
static int uvc_host_get_current_gain(struct uvc_device *uvc_dev, int32_t *gain_val)
{
	...
	ret = usbh_req_setup(uvc_dev->udev,
				 USB_REQTYPE_DIR_TO_HOST | USB_REQTYPE_TYPE_CLASS | 
...
static int uvc_host_init(struct usbh_contex *const uhs_ctx, struct usbh_class_data *cdata)
{
	const struct device *dev = cdata->priv;
	struct uvc_device *uvc_dev = (struct uvc_device *)dev->data;

	if (!uhs_ctx || !cdata || !dev || !uvc_dev) {
 ...
 * @return 0 on success, negative error code on failure
 */
static int uvc_host_connected(struct usb_device *udev, void *desc_start_addr, void *desc_end_addr, struct usbh_class_data *cdata)
{
	const struct device *dev = cdata->priv;
	struct uvc_device *uvc_dev = (struct uvc_device *)dev->data;
	int ret;
...
#ifdef CONFIG_POLL
	if (uvc_dev->sig) {
		k_poll_signal_raise(uvc_dev->sig, VIDEO_UVC_DEVICE_CONNECTED);
...
/** TODO */
static int uvc_host_removed(struct usbh_contex *const uhs_ctx, 
...
/** TODO */
static int uvc_host_suspended(struct usbh_contex *const uhs_ctx)
{
	return 0;
}

/** TODO */
static int uvc_host_resumed(struct usbh_contex *const uhs_ctx)
{
	return 0;
}

/** TODO */
static int uvc_host_rwup(struct usbh_contex *const uhs_ctx)
{
	return 0;
}
 ...
 *
 * @param dev Pointer to video device
 * @param enable Enbale or disable the video stream
