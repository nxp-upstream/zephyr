...
static int uvc_host_configure_device(struct uvc_host_data *const host_data)
{
	...
	const struct usb_if_descriptor *ctrl_iface = host_data->current_control_interface;
	const struct usb_if_descriptor *stream_iface = host_data->current_stream_iface_info.iface;
	int ret;

	if ((!ctrl_iface) || (!stream_iface)) {
				      ...
				      struct uvc_host_data *const host_data, uint8_t iface)
{
	...
	for (uint8_t i = 0; i < iad_desc->bInterfaceCount; i++) {
		...
		if (if_desc->bInterfaceClass == USB_BCC_VIDEO &&
			...
			if (host_data->current_control_interface) {
		...
		if (if_desc->bInterfaceClass == USB_BCC_VIDEO &&
			...
			if (host_data->current_stream_iface_info.iface) {
...
static uint32_t
uvc_host_get_frame_default_intervals(const struct uvc_frame_common_descriptor *frame_desc)
{
...
static int uvc_host_find_frame_in_format(const struct uvc_format_common_descriptor *format_header,
					 uint16_t target_width, uint16_t target_height,
					 uint8_t expected_frame_subtype,
					 const struct uvc_frame_common_descriptor **found_frame,
...
static int uvc_host_find_format(struct uvc_host_data *const host_data,
				...
				const struct uvc_frame_common_descriptor **frame, uint32_t *frmival)
{
	...
	for (uint8_t i = 0; i < host_data->num_uncompressed_formats; i++) {
		...
		if (desc_pixelformat == fmt->pixelformat) {
			...
			ret = uvc_host_find_frame_in_format((const void *)uncomp_format, fmt->width,
	...
	if (fmt->pixelformat == VIDEO_PIX_FMT_JPEG) {
		for (uint8_t i = 0; i < host_data->num_mjpeg_formats; i++) {
			...
			ret = uvc_host_find_frame_in_format((const void *)mjpeg_format, fmt->width,
			       ...
			       struct video_format *const fmt)
{
	...
	ret = uvc_host_select_streaming_alternate(host_data, byte_per_sec);
	if (ret != 0) {
...
static int uvc_host_create_format_caps(struct uvc_host_data *const host_data)
{
	...
	for (uint8_t i = 0; i < host_data->num_uncompressed_formats; i++) {
		...
		cap_index = uvc_host_parse_format_frames(
	...
	if (end_frame) {
		if (host_data->save_picture) {
			if (vbuf && vbuf->bytesused != 0) {
				...
				if (!atomic_test_bit(&host_data->device_flags,
						     UVC_DEVICE_FLAG_STREAMING)) {
					goto cleanup;
				}
...
cleanup:
	net_buf_unref(buf);
	if (atomic_test_bit(&host_data->device_flags, UVC_DEVICE_FLAG_STREAMING) && vbuf) {
...
static int uvc_host_enum_frame_intervals(const struct uvc_frame_common_descriptor *frame_ptr,
					 ...
					 uint32_t bmcontrol_bit)
{
	...
	if (!host_data->uvc_descriptors.vc_processing) {
		return false;
	}

	const struct uvc_processing_unit_descriptor *pud = host_data->uvc_descriptors.vc_processing;
					 ...
					 uint32_t bmcontrol_bit)
{
	...
	if (!host_data->uvc_descriptors.vc_camera) {
		return false;
	}
					...
					struct video_ctrl_range *range)
{
	...
	/* Query minimum value */
	ret = uvc_host_vc_get(host_data, UVC_GET_MIN, map->selector, entity_id, data, map->size);
	if (ret != 0) {
	...
	/* Query maximum value */
	ret = uvc_host_vc_get(host_data, UVC_GET_MAX, map->selector, entity_id, data, map->size);
	if (ret == -EPIPE) {
	...
	ret = uvc_host_vc_get(host_data, UVC_GET_RES, map->selector, entity_id, data, map->size);
	if (ret == -EPIPE) {
		LOG_WRN("Request 0x%02x not supported by device for entity %d, control selector "
			"0x%02x",
	...
	/* Query default value */
	ret = uvc_host_vc_get(host_data, UVC_GET_DEF, map->selector, entity_id, data, map->size);
	if (ret == -EPIPE) {
		LOG_WRN("Request 0x%02x not supported by device for entity %d, control selector "
			"0x%02x",
				       ...
				       int *initialized_count)
{
	...
	for (size_t i = 0; i < map_length; i++) {
		if (!(supported_controls & BIT(map[i].bit))) {
			LOG_DBG("Control bit %u not supported by device", map[i].bit);
			continue;
		}

		ret = uvc_host_query_control_range(host_data, entity_id, &map[i], &range);
		if (ret != 0) {
			continue;
		}

		ret = uvc_host_init_control_by_cid(dev, map[i].cid, &range);
		if (ret == -ENOTSUP) {
...
static int usb_host_camera_init_controls(const struct device *dev)
{
	...
	if (desc->vc_processing) {
		...
		control_bitmap = uvc_host_extract_control_bitmap(desc->vc_processing->bmControls,
	...
	if (desc->vc_camera) {
		...
		control_bitmap = uvc_host_extract_control_bitmap(desc->vc_camera->bmControls,
			  ...
			  uint8_t iface)
{
	...
	ret = uvc_host_parse_descriptors(c_data, host_data, target_iface);
	if (ret != 0) {
					 ...
					 const struct uvc_control_map **map)
{
	...
	if (host_data->uvc_descriptors.vc_selector) {
	...
	if (host_data->uvc_descriptors.vc_extension) {
...
static int usbh_uvc_set_ctrl(const struct device *dev, uint32_t id)
{
	...
	ret = uvc_host_vc_set(host_data, UVC_SET_CUR, map->selector, entity_id, data, map->size);
	if (ret != 0) {
				      ...
				      int32_t *value)
{
	...
	ret = uvc_host_vc_get(host_data, UVC_GET_CUR, map->selector, entity_id, data, map->size);
	if (ret != 0) {
...
#define USBH_VIDEO_DEVICE_DEFINE(n, _)                                                             \
	static struct uvc_host_data uvc_host_data##n;                                              \
                                                                                                   \
	DEVICE_DEFINE(usbh_uvc_##n, "usbh_uvc_" #n, NULL, NULL, &uvc_host_data##n, NULL,           \
		      POST_KERNEL, CONFIG_VIDEO_INIT_PRIORITY, &uvc_host_video_api);               \
                                                                                                   \
