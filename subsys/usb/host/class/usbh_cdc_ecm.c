...
static int usbh_cdc_ecm_get_mac_address(struct usbh_cdc_ecm_ctx *const ctx)
{
	...
	const struct usb_string_descriptor *mac_str_desc;
	char mac_str[NET_ETH_ADDR_LEN * 2 + 1];
	bool found_mac = false;
	int ret;

	ret = usbh_desc_get_supported_langs(ctx->udev, NULL, 0, &supported_langs);
	if (ret != 0) {
		goto cleanup;
	}
	...
	ret = usbh_desc_get_supported_langs(ctx->udev, langs, supported_langs, NULL);
	if (ret != 0) {
		goto cleanup;
	}

	mac_str_desc_buf = usbh_xfer_buf_alloc(ctx->udev, sizeof(uint8_t) * 26);
	if (mac_str_desc_buf == NULL) {
		ret = -ENOMEM;
		goto cleanup;
	}
	mac_str_desc = (struct usb_string_descriptor *)mac_str_desc_buf->data;

	for (unsigned int i = 0; i < supported_langs; i++) {
		net_buf_reset(mac_str_desc_buf);
		ret = usbh_req_desc_str(ctx->udev, ctx->mac_str_desc_idx, langs[i],
					mac_str_desc_buf);
		if (ret != 0) {
		...
		if (hex2bin(mac_str, strlen(mac_str), ctx->eth_mac.addr, NET_ETH_ADDR_LEN) !=
		    NET_ETH_ADDR_LEN) {
			continue;
		}
...
cleanup:
	...
	if (mac_str_desc_buf != NULL) {
		usbh_xfer_buf_free(ctx->udev, mac_str_desc_buf);
	}
