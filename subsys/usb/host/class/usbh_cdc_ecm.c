					  ...
					  const struct usb_if_descriptor *if_desc)
{
	...
	LOG_INF("the USB device information is summarized below\r\n"
		...
		ctx->max_segment_size, ctx->mac_str_desc_idx, ctx->mc_filter_supported_num,
		ctx->mc_filter_is_imperfect ? "imperfect" : "perfect", ctx->mc_filter_crc32_shift);
					...
					struct net_eth_addr *const eth_mac)
{
	...
	ret = usbh_desc_get_supported_langs(udev, NULL, 0, &supported_langs);
	if (ret != 0) {
		goto cleanup;
	}
	...
	langs = k_malloc(sizeof(uint16_t) * supported_langs);
	if (langs == NULL) {
		ret = -ENOMEM;
		goto cleanup;
	}

	ret = usbh_desc_get_supported_langs(udev, langs, supported_langs, NULL);
	if (ret != 0) {
		goto cleanup;
	}

	mac_str_desc_buf = usbh_xfer_buf_alloc(udev, sizeof(uint8_t) * 26);
	...
	for (unsigned int i = 0; i < supported_langs; i++) {
		...
		ret = usbh_req_desc_str(udev, mac_str_desc_idx, langs[i], mac_str_desc_buf);
		if (ret != 0) {
	...
	if (!found_mac) {
		...
		LOG_INF("device MAC address: %02x:%02x:%02x:%02x:%02x:%02x", eth_mac->addr[0],
			eth_mac->addr[1], eth_mac->addr[2], eth_mac->addr[3], eth_mac->addr[4],
			eth_mac->addr[5]);
	}
