...
otError infra_if_init(otInstance *aInstance, struct net_if *ail_iface)
{
	otError error = OT_ERROR_NONE;
	struct in6_addr addr = {0};
	int ret;
	...
	while (i + sizeof(struct net_icmpv6_nd_opt_hdr) <= aBufferLength) {
		...
		case NET_ICMPV6_ND_OPT_ROUTE:
			...
			const struct net_icmpv6_nd_opt_route_info *rio =
				(const struct net_icmpv6_nd_opt_route_info *)&aBuffer[i];
