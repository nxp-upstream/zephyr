...
static void remove_checksums_for_eth_offloading(uint8_t *buf, uint16_t len)
{
	...
	pkt_cursor = buf + hdr_len;

	if (config.chksum_support & NET_IF_CHECKSUM_IPV4_HEADER) {
		ipv4_hdr->chksum = 0;
	}

	switch(ipv4_hdr->proto) {
		case IPPROTO_ICMP:
			if (config.chksum_support & NET_IF_CHECKSUM_IPV4_ICMP) {
		...
		break;
		case IPPROTO_UDP:
			if (config.chksum_support & NET_IF_CHECKSUM_IPV4_UDP) {
		...
		break;
		case IPPROTO_TCP:
			if (config.chksum_support & NET_IF_CHECKSUM_IPV4_TCP) {
