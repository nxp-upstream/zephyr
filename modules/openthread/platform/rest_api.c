...
int rest_api_dataset_hex_handler(struct http_client_ctx *client, enum http_transaction_status status,
                                 const struct http_request_ctx *request_ctx,
                                 struct http_response_ctx *response_ctx, void *user_data)
{
    static char json_response[2048];
    
    LOG_DBG("Dataset HEX API called, status: %d", status);
    ...
    if (status == HTTP_SERVER_REQUEST_DATA_FINAL) {
        ...
        if (!instance) {
            ...
            if (error != OT_ERROR_NONE) {
                ...
                for (uint8_t i = 0; i < datasetTlvs.mLength; i++) {
                    snprintf(&hex_string[i * 2], 3, "%02x", datasetTlvs.mTlvs[i]);
                }
                hex_string[datasetTlvs.mLength * 2] = '\0';
                
                snprintf(json_response, sizeof(json_response),
                        "{\"status\":\"ok\",\"dataset_hex\":\"%s\",\"length\":%d}",
                        hex_string, datasetTlvs.mLength);
            }
        }
...
int rest_api_topology_handler(struct http_client_ctx *client, enum http_transaction_status status,
                              const struct http_request_ctx *request_ctx,
                              struct http_response_ctx *response_ctx, void *user_data)
{
    static char json_response[8192];
    
    LOG_DBG("Topology API called, status: %d", status);
    ...
    if (status == HTTP_SERVER_REQUEST_DATA_FINAL) {
        ...
        if (!instance) {
            ...
            // Format self node with extended address
            snprintf(nodes + strlen(nodes), sizeof(nodes) - strlen(nodes),
                    ...
                    rloc16, role_str, color, shape);
            first_node = false;
            
            // Get neighbor table (children and routers)
            openthread_mutex_lock();
            ...
            while (otThreadGetNextNeighborInfo(instance, &iterator, &neighborInfo) == OT_ERROR_NONE) {
                ...
                const char *neighbor_role = "unknown";
                const char *neighbor_color = "#cccccc";
                const char *neighbor_shape = "ellipse";
                ...
                snprintf(nodes + strlen(nodes), sizeof(nodes) - strlen(nodes),
                        ...
                        "\"shape\":\"%s\","
                        "\"size\":20}",
                        neighborInfo.mExtAddress.m8[0], neighborInfo.mExtAddress.m8[1],
                        neighborInfo.mExtAddress.m8[2], neighborInfo.mExtAddress.m8[3],
                        neighborInfo.mExtAddress.m8[4], neighborInfo.mExtAddress.m8[5],
                        neighborInfo.mExtAddress.m8[6], neighborInfo.mExtAddress.m8[7],
                        neighbor_role, neighborInfo.mRloc16, neighborInfo.mLinkQualityIn,
                        neighbor_color, neighbor_shape);
                first_node = false;
                
                // Add edge from neighbor to self (they are our neighbors)
                if (!first_edge) {
                    strncat(edges, ",", sizeof(edges) - strlen(edges) - 1);
                }
                
                snprintf(edges + strlen(edges), sizeof(edges) - strlen(edges),
            ...
            snprintf(json_response, sizeof(json_response),
                    "{\"status\":\"ok\",\"nodes\":%s,\"edges\":%s,\"role\":\"%s\"}",
                    nodes, edges, role_str);
        }
