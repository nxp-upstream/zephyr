...
static void dma_mcux_lpc_backup_reg(const struct device *dev)
{
	...
	for (uint32_t channel = 0; channel < config->num_of_channels; channel++) {
		if (dma_data->channel_index[channel] != -1) {
			channel_data = &dma_data->channel_data[virtual_channel];

			channel_data->backup_reg.xfercfg = dma_base->CHANNEL[channel].XFERCFG;
			channel_data->backup_reg.cfg     = dma_base->CHANNEL[channel].CFG;
			virtual_channel++;
		}
	}
}
...
static void dma_mcux_lpc_restore_reg(const struct device *dev)
{
	...
	for (uint32_t channel = 0; channel < config->num_of_channels; channel++) {
		if (dma_data->channel_index[channel] != -1) {
			...
			dma_base->CHANNEL[channel].XFERCFG = channel_data->backup_reg.xfercfg;
			dma_base->CHANNEL[channel].CFG     = channel_data->backup_reg.cfg;
			virtual_channel++;
		}
	}
}
