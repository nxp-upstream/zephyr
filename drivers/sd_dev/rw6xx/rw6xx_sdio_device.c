 ...
 */
static uint32_t sdio_to_rw612_addr(const struct device *dev,
				    uint32_t sdio_addr)
{
 ...
 */
static inline const struct sdev_cfg *rw6xx_sdio_get_config(
...
static int rw6xx_cis_tuple_configurate(const struct device *dev)
{
	...
	const struct sdev_cfg *sdev_cfg = &config->sdev_cfg;
	const struct sdio_dev_cfg *sdio_cfg = &sdev_cfg->sdio_cfg;
 ...
 */
static void rw6xx_sdio_set_state(const struct device *dev,
				  unsigned int state)
{
	struct rw6xx_sdio_data *data = dev->data;
 ...
 * @retval negative errno code on failure
 */
int rw6xx_send_data(const struct device *dev, sdev_pkt_t *pkt)
{
	int ret;
 ...
 */
static int rw6xx_register_rx_cb(const struct device *dev,
				sdev_recv_pkt cb)
{
	struct rw6xx_sdio_data *data = dev->data;
...
void rw6xx_rx_pkt_dispatch(void *data, size_t len)
{
	...
	sdev_pkt_t *pkt = sdev_pkt_alloc(SDEV_PKT_RX);
 ...
 */
static DEVICE_API(sdev, rw6xx_sdio_api) = {
...
static int sdio_hw_init(const struct device *dev)
{
	...
	if (cfg->pcfg != NULL) {
		...
		if (ret < 0) {
			LOG_ERR("RW6xx SDIO pinctrl init failed (%d)",
				ret);
			return ret;
		}
	}
	...
	SDU_InstallCallback(SDU_TYPE_FOR_WRITE_CMD,
			    rw6xx_rx_pkt_dispatch);
	SDU_InstallCallback(SDU_TYPE_FOR_WRITE_DATA,
			    rw6xx_rx_pkt_dispatch);
...
static int rw6xx_sdio_init(const struct device *dev)
{
	...
	sdev_set_state(dev, SDEV_DEVICE_INIT);
...
static int rw6xx_sdio_turn_off(const struct device *dev)
{
	...
	sdev_set_state(dev, SDEV_DEVICE_RESET);
