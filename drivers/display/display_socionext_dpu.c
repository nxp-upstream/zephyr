				...
				const enum display_pixel_format pixel_format)
{
	...
	data->fb_bytes =
		ROUND_UP((config->display_timing.width * data->pixel_bytes), DPU_FB_PITCH_ALIGN) *
		config->display_timing.height;
...
static int dpu_init(const struct device *dev)
{
	...
	if (data->pixel_format == PANEL_PIXEL_FORMAT_RGB_565) {
		dpu_set_pixel_format(dev, PIXEL_FORMAT_RGB_565);
	} else if (data->pixel_format == PANEL_PIXEL_FORMAT_RGB_888) {
		dpu_set_pixel_format(dev, PIXEL_FORMAT_RGB_888);
	} else if (data->pixel_format == PANEL_PIXEL_FORMAT_ARGB_8888) {
		dpu_set_pixel_format(dev, PIXEL_FORMAT_ARGB_8888);
	} else {
