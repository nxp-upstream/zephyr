...
{
	struct display_mipi_dbi_data *data = CONTAINER_OF(cb, struct display_mipi_dbi_data, te_gpio_cb);
			 ...
			 const void *buf)
{
	...
	mipi_dbi_command_write(config->mipi_dbi, NULL, MIPI_DCS_SET_COLUMN_ADDRESS, param, sizeof(param));
	...
	mipi_dbi_command_write(config->mipi_dbi, NULL, MIPI_DCS_SET_PAGE_ADDRESS, param, sizeof(param));
				    ...
				    const enum display_pixel_format pixel_format)
{
	...
	return mipi_dbi_command_write(config->mipi_dbi, NULL, MIPI_DCS_SET_PIXEL_FORMAT, &param, 1);
}
