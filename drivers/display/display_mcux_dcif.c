...
LOG_MODULE_REGISTER(display_mcux_dcif, CONFIG_DISPLAY_LOG_LEVEL);

/* This driver drives DCIF layer 0 as a single, full-panel framebuffer. */
#define MCUX_DCIF_LAYER 0U
/* Interrupts are routed to CPU domain 0 in a single-domain Zephyr system. */
#define MCUX_DCIF_DOMAIN 0U

struct mcux_dcif_config {
	DCIF_Type *base;
...
struct mcux_dcif_data {
	/* Pointer to active framebuffer */
	const uint8_t *active_fb;
	uint8_t *fb[CONFIG_MCUX_DCIF_FB_NUM];
...
static int mcux_dcif_write(const struct device *dev, const uint16_t x, const uint16_t y,
			   const struct display_buffer_descriptor *desc, const void *buf)
{
	const struct mcux_dcif_config *config = dev->config;
	struct mcux_dcif_data *data = dev->data;
	uint32_t h_idx;
	const uint8_t *src;
	uint8_t *dst;
...
#if defined(CONFIG_HAS_MCUX_CACHE) && defined(CONFIG_MCUX_DCIF_MAINTAIN_CACHE)
	...
	DCIF_SetLayerStride(config->base, MCUX_DCIF_LAYER, data->pitch_bytes);
	DCIF_SetLayerAddr(config->base, MCUX_DCIF_LAYER, (uint32_t)data->active_fb);
	DCIF_TriggerLayerShadowLoad(config->base, MCUX_DCIF_LAYER);

#if CONFIG_MCUX_DCIF_FB_NUM != 0
	/* Update index of active framebuffer */
	data->next_idx = (data->next_idx + 1) % CONFIG_MCUX_DCIF_FB_NUM;
#endif
...
static void mcux_dcif_get_capabilities(const struct device *dev,
...
static void *mcux_dcif_get_framebuffer(const struct device *dev)
{
	struct mcux_dcif_data *data = dev->data;
...
static int mcux_dcif_display_blanking_off(const struct device *dev)
{
	const struct mcux_dcif_config *config = dev->config;
...
static int mcux_dcif_display_blanking_on(const struct device *dev)
{
	const struct mcux_dcif_config *config = dev->config;
...
static int mcux_dcif_set_pixel_format(const struct device *dev,
				      const enum display_pixel_format pixel_format)
{
	...
	for (int i = 0; i < CONFIG_MCUX_DCIF_FB_NUM; i++) {
	...
	memset(config->fb_ptr, 0, data->fb_bytes * CONFIG_MCUX_DCIF_FB_NUM);

	return 0;
}

static void mcux_dcif_isr(const struct device *dev)
{
	const struct mcux_dcif_config *config = dev->config;
	struct mcux_dcif_data *data = dev->data;
	uint32_t status;

	status = DCIF_GetInterruptStatus(config->base, MCUX_DCIF_DOMAIN);
	DCIF_ClearInterruptStatus(config->base, MCUX_DCIF_DOMAIN, status);
...
static int mcux_dcif_init(const struct device *dev)
{
	const struct mcux_dcif_config *config = dev->config;
	struct mcux_dcif_data *data = dev->data;
	 ...
	 */
	ret = mcux_dcif_set_pixel_format(dev, config->init_pixel_format);
	if (ret) {
		return ret;
	}
	...
	ret = DCIF_SetLayerConfig(config->base, MCUX_DCIF_LAYER, &layer_config);
	if (ret != kStatus_Success) {
		LOG_ERR("Failed to configure DCIF layer (%d)", ret);
		return -EINVAL;
	}

	DCIF_EnableLayer(config->base, MCUX_DCIF_LAYER, true);

	DCIF_EnableInterrupts(config->base, MCUX_DCIF_DOMAIN, kDCIF_InterruptVsync);
...
static DEVICE_API(display, mcux_dcif_api) = {
	.blanking_on = mcux_dcif_display_blanking_on,
	.blanking_off = mcux_dcif_display_blanking_off,
	.set_pixel_format = mcux_dcif_set_pixel_format,
	.write = mcux_dcif_write,
	.get_capabilities = mcux_dcif_get_capabilities,
	.get_framebuffer = mcux_dcif_get_framebuffer,
};

/* Place the frame buffer in secondary RAM if specified, otherwise use default RAM */
#define MCUX_DCIF_FB_PLACEMENT(n)                                                                  \
...
#define MCUX_DCIF_FRAMEBUFFER_DECL(n)                                                              \
	MCUX_DCIF_FB_PLACEMENT(n) static uint8_t __aligned(DCIF_FB_ALIGN)                          \
		mcux_dcif_frame_buffer_##n[CONFIG_MCUX_DCIF_FB_NUM * DT_INST_PROP(n, height) *     \
					   ROUND_UP((DT_INST_PROP(n, width) * 4U), DCIF_FB_ALIGN)]

#define MCUX_DCIF_FRAMEBUFFER(n) mcux_dcif_frame_buffer_##n

#define MCUX_DCIF_DEVICE_INIT(n)                                                                   \
	static void mcux_dcif_config_func_##n(const struct device *dev)                            \
	{                                                                                          \
		IRQ_CONNECT(DT_INST_IRQN(n), DT_INST_IRQ(n, priority), mcux_dcif_isr,               \
			    DEVICE_DT_INST_GET(n), 0);                                             \
		irq_enable(DT_INST_IRQN(n));                                                        \
	}                                                                                          \
	MCUX_DCIF_FRAMEBUFFER_DECL(n);                                                             \
	struct mcux_dcif_data mcux_dcif_data_##n = {                                               \
		.next_idx = 0,                                                                     \
	};                                                                                         \
	struct mcux_dcif_config mcux_dcif_config_##n = {                                           \
		.base = (DCIF_Type *)DT_INST_REG_ADDR(n),                                          \
		.irq_config_func = mcux_dcif_config_func_##n,                                      \
		...
		.fb_ptr = MCUX_DCIF_FRAMEBUFFER(n),                                                \
	};                                                                                         \
	DEVICE_DT_INST_DEFINE(n, &mcux_dcif_init, NULL, &mcux_dcif_data_##n,                        \
			      &mcux_dcif_config_##n, POST_KERNEL, CONFIG_DISPLAY_INIT_PRIORITY,     \
			      &mcux_dcif_api);

DT_INST_FOREACH_STATUS_OKAY(MCUX_DCIF_DEVICE_INIT)
