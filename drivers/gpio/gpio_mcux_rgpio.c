...
/*
 * Open this port's clock gate when the devicetree gives it one.
 *
 * The RGPIO block is permanently clocked from its subsystem bus clock on some
 * SoCs and sits behind a gate software must open on others -- the i.MX RT266x
 * parts, for instance, come out of reset with the RGPIO gate closed, so the port
 * reads back nothing until it is opened. Gating it here rather than from board
 * code keeps that a property of the SoC devicetree. SoCs whose RGPIO nodes carry
 * no "clocks" property leave clock_dev NULL, and this is a no-op.
 */
