 ...
 *             -EIO if i2c error
 */
static int gpio_pca_series_pin_config_drive_strength(const struct device *dev, gpio_pin_t pin,
 ...
 */
static int gpio_pca_series_pin_config(const struct device *dev, gpio_pin_t pin, gpio_flags_t flags)
{
	const struct gpio_pca_series_config *cfg = dev->config;
	struct gpio_pca_series_data *data = dev->data;
	uint32_t reg_value;
	...
	if ((flags & GPIO_SINGLE_ENDED) != 0) {
		if (cfg->part_cfg->flags & PCA_HAS_OUT_CONFIG) {
			...
			ret = gpio_pca_series_reg_read_cache(dev, gpio_pca_series_1b_output_config,
								 (uint8_t *)&reg_value);
			reg_value |= (BIT(pin)); /* set bit to set open-drain */
			ret = gpio_pca_series_reg_write(dev, gpio_pca_series_1b_output_config,
							(uint8_t *)&reg_value);
		}
	} else {
		if (cfg->part_cfg->flags & PCA_HAS_OUT_CONFIG) {
	...
	if (((flags & GPIO_PULL_UP) != 0) || ((flags & GPIO_PULL_DOWN) != 0)) {
		if (cfg->part_cfg->flags & PCA_HAS_PULL) {
			...
			ret = gpio_pca_series_reg_read_cache(dev, gpio_pca_series_1b_pull_select,
								 (uint8_t *)&reg_value);
			...
			ret = gpio_pca_series_reg_write(dev, gpio_pca_series_1b_pull_select,
							(uint8_t *)&reg_value);

			/* configure gpio_pca_series_1b_pull_enable */
			ret = gpio_pca_series_reg_read_cache(dev, gpio_pca_series_1b_pull_enable,
								 (uint8_t *)&reg_value);
...
static int gpio_pca_series_port_read_standard(const struct device *dev, gpio_port_value_t *value)
{
	...
	/* Read Input Register */
	ret = gpio_pca_series_reg_read(dev, gpio_pca_series_1b_input_port, (uint8_t *)&input_data);
	if (ret) {
		goto out;
	}
					  ...
					  gpio_port_value_t value, gpio_port_value_t toggle)
{
	...
	LOG_DBG("dev %s write mask 0x%8.8lx value 0x%8.8lx toggle 0x%8.8lx", dev->name, mask, value,
		toggle);
	k_sem_take(&data->lock, K_FOREVER);
		...
		IF_ENABLED(CONFIG_GPIO_PCA_SERIES_INTERRUPT,                                       \
			   (.self = DEVICE_DT_INST_GET(inst),                                      \
				.int_work = Z_WORK_INITIALIZER(gpio_pca_series_interrupt_worker), ))}; \
																								   \
	DEVICE_DT_INST_DEFINE(inst, gpio_pca_series_init, NULL, &gpio_pca_series_##inst##_data,    \
