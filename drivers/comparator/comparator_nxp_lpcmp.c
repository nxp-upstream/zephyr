...
static int nxp_lpcmp_init(const struct device *dev)
{
	...
	/* Configure input source, mux selection, hysteresis, and power mode. */
	ccr2 = ((ccr2 & ~(LPCMP_CCR2_CMP_HPMD_MASK | LPCMP_CCR2_HYSTCTR_MASK |
			  LPCMP_CCR2_PSEL_MASK | LPCMP_CCR2_MSEL_MASK |
			  LPCMP_CCR2_INPSEL_MASK | LPCMP_CCR2_INMSEL_MASK)) |
			  LPCMP_CCR2_PSEL(config->positive_mux_input) |
			  LPCMP_CCR2_MSEL(config->negative_mux_input) |
			  LPCMP_CCR2_INPSEL(config->positive_mux_is_dac ? 0U : 1U) |
			  LPCMP_CCR2_INMSEL(config->negative_mux_is_dac ? 0U : 1U) |
			  LPCMP_CCR2_HYSTCTR(config->hysteresis_mode));

	if (config->power_mode == 1U) {
		ccr2 |= LPCMP_CCR2_CMP_HPMD_MASK;
	}
	base->CCR2 = ccr2;
