...
static inline int z_vrfy_mux_control_set_lock(const struct device *dev,
					      const struct mux_control *control,
					      bool lock)
{
	struct mux_control k_ctrl;
	...
	K_OOPS(K_SYSCALL_DRIVER_MUX_CONTROL(dev, set_lock));
	K_OOPS(mux_control_copy_from_user(&k_ctrl, k_cells, control));

	return z_impl_mux_control_set_lock(dev, &k_ctrl, lock);
}
#include <zephyr/syscalls/mux_control_set_lock_mrsh.c>
