...
static int icm42686_get_shift(enum sensor_channel channel, int accel_fs, int gyro_fs, int8_t *shift)
{
	switch (channel) {
	...
	case SENSOR_CHAN_ACCEL_Z:
		switch (accel_fs) {
		case ICM42686_DT_ACCEL_FS_2:
			*shift = 5;
			return 0;
		case ICM42686_DT_ACCEL_FS_4:
			*shift = 6;
			return 0;
		case ICM42686_DT_ACCEL_FS_8:
			*shift = 7;
			return 0;
		case ICM42686_DT_ACCEL_FS_16:
			*shift = 8;
			return 0;
		case ICM42686_DT_ACCEL_FS_32:
			*shift = 9;
			return 0;
		default:
			return -EINVAL;
	...
	case SENSOR_CHAN_GYRO_Z:
		switch (gyro_fs) {
		case ICM42686_DT_GYRO_FS_31_25:
			*shift = 0;
			return 0;
		case ICM42686_DT_GYRO_FS_62_5:
			*shift = 1;
			return 0;
		case ICM42686_DT_GYRO_FS_125:
			*shift = 2;
			return 0;
		case ICM42686_DT_GYRO_FS_250:
			*shift = 3;
			return 0;
		case ICM42686_DT_GYRO_FS_500:
			*shift = 4;
			return 0;
		case ICM42686_DT_GYRO_FS_1000:
			*shift = 5;
			return 0;
		case ICM42686_DT_GYRO_FS_2000:
			*shift = 6;
			return 0;
		case ICM42686_DT_GYRO_FS_4000:
			*shift = 7;
			return 0;
		default:
			return -EINVAL;
...
int icm42686_convert_raw_to_q31(struct icm4268x_cfg *cfg, enum sensor_channel chan, int32_t reading,
				q31_t *out)
{
	int32_t whole;
	int32_t fraction;
	...
	rc = icm42686_get_shift(chan, cfg->accel_fs, cfg->gyro_fs, &shift);
	if (rc != 0) {
		return rc;
	}
	...
	case SENSOR_CHAN_ACCEL_Y:
	case SENSOR_CHAN_ACCEL_Z:
		icm42686_accel_ms(cfg, reading, &whole, &fraction);
		break;
	case SENSOR_CHAN_GYRO_XYZ:
	case SENSOR_CHAN_GYRO_X:
	case SENSOR_CHAN_GYRO_Y:
	case SENSOR_CHAN_GYRO_Z:
		icm42686_gyro_rads(cfg, reading, &whole, &fraction);
		break;
	case SENSOR_CHAN_DIE_TEMP:
...
int icm42688_convert_raw_to_q31(struct icm4268x_cfg *cfg, enum sensor_channel chan, int32_t reading,
				q31_t *out)
{
	int32_t whole;
	int32_t fraction;
...
static uint32_t accel_period_ns[] = {
	[ICM4268X_DT_ACCEL_ODR_1_5625] = UINT32_C(10000000000000) / 15625,
	[ICM4268X_DT_ACCEL_ODR_3_125] = UINT32_C(10000000000000) / 31250,
	[ICM4268X_DT_ACCEL_ODR_6_25] = UINT32_C(10000000000000) / 62500,
	[ICM4268X_DT_ACCEL_ODR_12_5] = UINT32_C(10000000000000) / 12500,
	[ICM4268X_DT_ACCEL_ODR_25] = UINT32_C(1000000000) / 25,
	[ICM4268X_DT_ACCEL_ODR_50] = UINT32_C(1000000000) / 50,
	[ICM4268X_DT_ACCEL_ODR_100] = UINT32_C(1000000000) / 100,
	[ICM4268X_DT_ACCEL_ODR_200] = UINT32_C(1000000000) / 200,
	[ICM4268X_DT_ACCEL_ODR_500] = UINT32_C(1000000000) / 500,
	[ICM4268X_DT_ACCEL_ODR_1000] = UINT32_C(1000000),
	[ICM4268X_DT_ACCEL_ODR_2000] = UINT32_C(1000000) / 2,
	[ICM4268X_DT_ACCEL_ODR_4000] = UINT32_C(1000000) / 4,
	[ICM4268X_DT_ACCEL_ODR_8000] = UINT32_C(1000000) / 8,
	[ICM4268X_DT_ACCEL_ODR_16000] = UINT32_C(1000000) / 16,
	[ICM4268X_DT_ACCEL_ODR_32000] = UINT32_C(1000000) / 32,
};

static uint32_t gyro_period_ns[] = {
	[ICM4268X_DT_GYRO_ODR_12_5] = UINT32_C(10000000000000) / 12500,
	[ICM4268X_DT_GYRO_ODR_25] = UINT32_C(1000000000) / 25,
	[ICM4268X_DT_GYRO_ODR_50] = UINT32_C(1000000000) / 50,
	[ICM4268X_DT_GYRO_ODR_100] = UINT32_C(1000000000) / 100,
	[ICM4268X_DT_GYRO_ODR_200] = UINT32_C(1000000000) / 200,
	[ICM4268X_DT_GYRO_ODR_500] = UINT32_C(1000000000) / 500,
	[ICM4268X_DT_GYRO_ODR_1000] = UINT32_C(1000000),
	[ICM4268X_DT_GYRO_ODR_2000] = UINT32_C(1000000) / 2,
	[ICM4268X_DT_GYRO_ODR_4000] = UINT32_C(1000000) / 4,
	[ICM4268X_DT_GYRO_ODR_8000] = UINT32_C(1000000) / 8,
	[ICM4268X_DT_GYRO_ODR_16000] = UINT32_C(1000000) / 16,
	[ICM4268X_DT_GYRO_ODR_32000] = UINT32_C(1000000) / 32,
};

static int icm42686_fifo_decode(const uint8_t *buffer, struct sensor_chan_spec chan_spec,
				uint32_t *fit, uint16_t max_count, void *data_out)
{
	...
	while (count < max_count && buffer < buffer_end) {
		...
		if (chan_spec.chan_type == SENSOR_CHAN_DIE_TEMP) {
			...
			icm42686_get_shift(SENSOR_CHAN_ACCEL_XYZ, edata->header.accel_fs,
					   edata->header.gyro_fs, &data->shift);

			data->readings[count].timestamp_delta = (accel_frame_count - 1) * period_ns;
			rc = icm42686_read_imu_from_packet(buffer, true, edata->header.accel_fs, 0,
							   &data->readings[count].x);
			rc |= icm42686_read_imu_from_packet(buffer, true, edata->header.accel_fs, 1,
							    &data->readings[count].y);
			rc |= icm42686_read_imu_from_packet(buffer, true, edata->header.accel_fs, 2,
			...
			icm42686_get_shift(SENSOR_CHAN_GYRO_XYZ, edata->header.accel_fs,
					   edata->header.gyro_fs, &data->shift);

			data->readings[count].timestamp_delta = (gyro_frame_count - 1) * period_ns;
			rc = icm42686_read_imu_from_packet(buffer, false, edata->header.gyro_fs, 0,
							   &data->readings[count].x);
			rc |= icm42686_read_imu_from_packet(buffer, false, edata->header.gyro_fs, 1,
							    &data->readings[count].y);
			rc |= icm42686_read_imu_from_packet(buffer, false, edata->header.gyro_fs, 2,
							    &data->readings[count].z);
			if (rc != 0) {
				gyro_frame_count--;
				buffer = frame_end;
...
static int icm42686_one_shot_decode(const uint8_t *buffer, struct sensor_chan_spec chan_spec,
				    uint32_t *fit, uint16_t max_count, void *data_out)
{
	...
	case SENSOR_CHAN_ACCEL_Z:
	case SENSOR_CHAN_ACCEL_XYZ: {
		channel_request = icm4268x_encode_channel(SENSOR_CHAN_ACCEL_XYZ);
		...
		rc = icm42686_get_shift(SENSOR_CHAN_ACCEL_XYZ, header->accel_fs, header->gyro_fs,
					&out->shift);
		if (rc != 0) {
			return -EINVAL;
		}

		icm42686_convert_raw_to_q31(
			&cfg, SENSOR_CHAN_ACCEL_X,
			edata->readings[icm4268x_get_channel_position(SENSOR_CHAN_ACCEL_X)],
			&out->readings[0].x);
		icm42686_convert_raw_to_q31(
			&cfg, SENSOR_CHAN_ACCEL_Y,
			edata->readings[icm4268x_get_channel_position(SENSOR_CHAN_ACCEL_Y)],
			&out->readings[0].y);
		icm42686_convert_raw_to_q31(
			&cfg, SENSOR_CHAN_ACCEL_Z,
			edata->readings[icm4268x_get_channel_position(SENSOR_CHAN_ACCEL_Z)],
			&out->readings[0].z);
		*fit = 1;
		return 1;
	}
	...
	case SENSOR_CHAN_GYRO_Z:
	case SENSOR_CHAN_GYRO_XYZ: {
		channel_request = icm4268x_encode_channel(SENSOR_CHAN_GYRO_XYZ);
		if ((channel_request & edata->channels) != channel_request) {
			return -ENODATA;
		}
		...
		out->header.base_timestamp_ns = edata->header.timestamp;
		out->header.reading_count = 1;
		rc = icm42686_get_shift(SENSOR_CHAN_GYRO_XYZ, header->accel_fs, header->gyro_fs,
					&out->shift);
		if (rc != 0) {
			return -EINVAL;
		}
		...
		icm42686_convert_raw_to_q31(
			...
			edata->readings[icm4268x_get_channel_position(SENSOR_CHAN_GYRO_X)],
			&out->readings[0].x);
		icm42686_convert_raw_to_q31(
			&cfg, SENSOR_CHAN_GYRO_Y,
			edata->readings[icm4268x_get_channel_position(SENSOR_CHAN_GYRO_Y)],
			&out->readings[0].y);
		icm42686_convert_raw_to_q31(
	...
	case SENSOR_CHAN_DIE_TEMP: {
		channel_request = icm4268x_encode_channel(SENSOR_CHAN_DIE_TEMP);
		if ((channel_request & edata->channels) != channel_request) {
			return -ENODATA;
		}
		...
		rc = icm42686_get_shift(SENSOR_CHAN_DIE_TEMP, header->accel_fs, header->gyro_fs,
					&out->shift);
		if (rc != 0) {
			return -EINVAL;
		}
...
static int icm42686_decoder_decode(const uint8_t *buffer, struct sensor_chan_spec chan_spec,
				   uint32_t *fit, uint16_t max_count, void *data_out)
{
	...
	if (header->is_fifo) {
		return icm42686_fifo_decode(buffer, chan_spec, fit, max_count, data_out);
	}
	return icm42686_one_shot_decode(buffer, chan_spec, fit, max_count, data_out);
}
...
static bool icm2468x_decoder_has_trigger(const uint8_t *buffer, enum sensor_trigger_type trigger)
{
	const struct icm4268x_fifo_data *edata = (const struct icm4268x_fifo_data *)buffer;
...
SENSOR_DECODER_API_DT_DEFINE() = {
	...
	.decode = icm42686_decoder_decode,
	.has_trigger = icm2468x_decoder_has_trigger,
};

int icm42686_get_decoder(const struct device *dev, const struct sensor_decoder_api **decoder)
{
	ARG_UNUSED(dev);
...
void icm42688_get_decoder(const struct device *dev, const struct sensor_decoder_api **decoder)
{
	ARG_UNUSED(dev);
	*decoder = &SENSOR_DECODER_NAME();
...
int icm4268x_get_decoder(const struct device *dev, const struct sensor_decoder_api **decoder)
{
	...
	if (cfg.dev_id == WHO_AM_I_ICM42686)
		icm42686_get_decoder(decoder);
	else if (cfg.dev_id == WHO_AM_I_ICM42688)
		icm42688_get_decoder(decoder);
