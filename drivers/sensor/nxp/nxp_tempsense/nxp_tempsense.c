...
static int nxp_tempsense_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	 ...
	 */
	int64_t term0 = ((int64_t)data->tca_q4[0] * 1000) / 16;
	int64_t term1 = ((int64_t)data->tca_q4[1] * (int64_t)vets_mv) / 16;
	int64_t term2 = ((int64_t)data->tca_q4[2] * (int64_t)vets_mv *
			 (int64_t)vets_mv) / (16 * 1000);
	int64_t temp_mC = term0 + term1 + term2;
				    ...
				    struct sensor_value *val)
{
	...
	/* temperature stored in milli-degrees C */
	val->val1 = data->temperature_mdegc / 1000;
	val->val2 = (data->temperature_mdegc % 1000) * 1000; /* micro-degrees */
