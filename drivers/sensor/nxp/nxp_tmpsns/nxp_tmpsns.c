...
static int nxp_tmpsns_channel_get(const struct device *dev,
...
#define NXP_TMPSNS_INIT(inst)							\
	static struct nxp_tmpsns_data data_##inst = {				\
		.tmp_25c = 0,							\
		.tmp_measure = 0,						\
	};									\
										\
	static const struct nxp_tmpsns_config config_##inst = {			\
		.base = (TMPSNS_Type *)DT_INST_REG_ADDR(inst),			\
	};									\
										\
	SENSOR_DEVICE_DT_INST_DEFINE(inst, nxp_tmpsns_init, NULL,		\
				&data_##inst, &config_##inst, POST_KERNEL,	\
				CONFIG_SENSOR_INIT_PRIORITY, &nxp_tmpsns_api);
