...
#ifdef CONFIG_FXLS8974_MAG_VECM
static int fxls8974_handle_m_vecm_int(const struct device *dev)
{
	struct fxls8974_data *data = dev->data;
...
static void fxls8974_handle_int(const struct device *dev)
{
	...
	if (config->ops->byte_read(dev, FXLS8974_REG_M_INT_SRC,
				   &int_source)) {
	...
	if (int_source & FXLS8974_VECM_MASK) {
		fxls8974_handle_m_vecm_int(dev);
	}
#endif
...
#endif
#ifdef CONFIG_FXLS8974_MAG_VECM
	case FXLS8974_TRIG_M_VECM:
		mask = FXLS8974_VECM_MASK;
...
#ifdef CONFIG_FXLS8974_MOTION
static int fxls8974_motion_init(const struct device *dev)
{
	const struct fxls8974_config *config = dev->config;

	/* Set Mode 4, Motion detection with ELE = 1, OAE = 1 */
	if (config->ops->byte_write(dev,
				    FXLS8974_REG_FF_MT_CFG,
				    FXLS8974_FF_MT_CFG_ELE |
				    FXLS8974_FF_MT_CFG_OAE |
				    FXLS8974_FF_MT_CFG_ZEFE |
				    FXLS8974_FF_MT_CFG_YEFE |
				    FXLS8974_FF_MT_CFG_XEFE)) {
		return -EIO;
	}

	/* Set motion threshold to maximimum */
	if (config->ops->byte_write(dev, FXLS8974_REG_FF_MT_THS,
				    FXLS8974_REG_FF_MT_THS)) {
		return -EIO;
	}
...
#ifdef CONFIG_FXLS8974_MAG_VECM
static int fxls8974_m_vecm_init(const struct device *dev)
{
	const struct fxls8974_config *config = dev->config;
	uint8_t m_vecm_cfg = config->mag_vecm_cfg;

	/* Route the interrupt to INT1 pin */
#if CONFIG_FXLS8974_MAG_VECM_INT1
	m_vecm_cfg |= FXLS8974_MAG_VECM_INT1_MASK;
#endif

	/* Set magnetic vector-magnitude function */
	if (config->ops->byte_write(dev, FXLS8974_REG_M_VECM_CFG,
				    m_vecm_cfg)) {
	 ...
	 */
	if (config->ops->byte_write(dev, FXLS8974_REG_M_VECM_THS_MSB,
				    config->mag_vecm_ths[0])) {
		LOG_ERR("Could not set magnetic vector-magnitude function threshold MSB value");
		return -EIO;
	}

	if (config->ops->byte_write(dev, FXLS8974_REG_M_VECM_THS_LSB,
