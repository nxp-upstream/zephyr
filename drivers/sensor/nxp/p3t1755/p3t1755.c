...
static int p3t1755_read_reg(const struct device *dev, uint8_t reg, uint8_t *value, size_t data_size)
{
	...
	return i2c_burst_read_dt(config->p3t1755_bus_cfg.i2c, reg, value, data_size);
#else
...
static int p3t1755_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	p3t1755_read_reg(dev, P3T1755_TEMPERATURE_REG, &raw_temp, 2);
	 ...
	 */
	data->sample =
		(((uint16_t)data[0] << 8U) | (uint16_t)data[1]) >> P3T1755_TEMPERATURE_REG_SHIFT;
...
static int p3t1755_init(const struct device *dev)
{
...
#if DT_ANY_INST_ON_BUS_STATUS_OKAY(i2c)
	if (!device_is_ready(config->.p3t1755_bus_cfg.i2c.bus)) {
	...
	if (config->oneshot_mode) {
		p3t1755_read_reg(dev, P3T1755_CONFIG_REG, &data->config_reg, 1);
...
#define P3T1755_CONFIG_I3C(inst)								\
	{															\
		.p3t1755_bus_cfg =										\
			{													\
				.i3c = &p3t1755_data_##inst.i3c_dev,			\
			},													\
...
#define P3T1755_INIT(n)											\
	static const struct p3t1755_config p3t1755_config_##n = {	\
		COND_CODE_1(DT_INST_ON_BUS(n, i3c),						\
				...
				(P3T1755_CONFIG_I2C(n))) .oneshot_mode =		\
						     DT_INST_PROP(n, oneshot_mode),		\
	};															\
																\
