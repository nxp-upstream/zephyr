...
static void irqsteer_hw_init(const struct irqsteer_config *cfg)
{
	...
	IRQSTEER_WRITE32(base, CHAN_MINTDIS(reg_num), 0xFFFFFFFF);

	/* Clear all channel masks */
	for (; reg_index < reg_num; reg_index++) {
		IRQSTEER_WRITE32(base, CHAN_MASK(reg_index, reg_num), 0);
	}
}
...
static void irqsteer_enable_interrupt(const struct irqsteer_config *cfg, uint32_t irq)
{
	...
	IRQSTEER_WRITE32(base, CHAN_MASK(reg_index, reg_num), mask);
}

/**
 * @brief Disable specific interrupt in IRQ_STEER
 */
static void irqsteer_disable_interrupt(const struct irqsteer_config *cfg, uint32_t irq)
{
	int reg_num = cfg->reg_num;
	uint32_t base = cfg->regmap_phys;
	int reg_index = irqsteer_get_reg_index(cfg, irq);
					     ...
					     uint32_t master_index)
{
	...
	mintdis = IRQSTEER_READ32(base, CHAN_MINTDIS(reg_num));
	mintdis &= ~BIT(master_index);
	IRQSTEER_WRITE32(base, CHAN_MINTDIS(reg_num), mintdis);
}

/**
 * @brief Disable master interrupt output
 */
static void irqsteer_disable_master_interrupt(const struct irqsteer_config *cfg,
					      uint32_t master_index)
{
	uint32_t mintdis;
	int reg_num = cfg->reg_num;
						      ...
						      uint32_t master_index)
{
	...
	int reg_num = cfg->reg_num;
	uint32_t base = cfg->regmap_phys;
	int reg_index = reg_num - 1 -
			((reg_num % 2 == 0) ? (master_index * 2) : (master_index * 2 - 1));
	...
	for (i = 0; i < 2; i++) {
		...
		ch_status = IRQSTEER_READ32(base, CHAN_STATUS(reg_index, reg_num));
		ch_mask = IRQSTEER_READ32(base, CHAN_MASK(reg_index, reg_num));

		/* Only consider masked (enabled) interrupts */
		ch_status &= ch_mask;

		if (i == 0 || (i == 1 && reg_index == reg_num - 1)) {
			status = ch_status;
		} else {
			status |= ((uint64_t)ch_status << 32);
		}
		reg_index--;
	}
					 ...
					 bool enable)
{
	...
	if (enable) {
		...
		if (disp) {
			irqsteer_enable_master_interrupt(cfg, disp->master_index);
		}
	} else {
		if (disp) {
			irqsteer_disable_master_interrupt(cfg, disp->master_index);
		}
				       ...
				       uint32_t system_irq, bool enable)
{
	...
	if (enable) {
		irqsteer_enable_interrupt(cfg, system_irq);
	} else {
		irqsteer_disable_interrupt(cfg, system_irq);
	}
}
...
#define NXP_IRQSTEER_CONFIG_DEFINE(n) \
	...
	static struct irqsteer_config irqsteer_config_##n = { \
		...
		.num_masters = DT_INST_PROP(n, nxp_num_irqs) / 64, \
