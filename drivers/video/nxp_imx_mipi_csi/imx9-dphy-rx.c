				 ...
				 const struct nxp_imx_mipi_csi_config *cfg)
{
	...
	nxp_imx_dphy_write(data, cfg->dphy_drv_data, DPHY_RX_CFGCLKFREQRANGE, data->cfgclkfreqrange);
...
int nxp_imx_dphy_enable(struct nxp_imx_mipi_csi_data *data, const struct nxp_imx_mipi_csi_config *cfg)
{
	uint32_t val;
	int ret = 0;
...
int nxp_imx_dphy_disable(struct nxp_imx_mipi_csi_data *data, const struct nxp_imx_mipi_csi_config *cfg)
{
	int ret = 0;
