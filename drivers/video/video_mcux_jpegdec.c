...
static int mcux_jpegdec_set_fmt(const struct device *dev, struct video_format *fmt)
{
	 ...
	 * format cannot be changed since the decoder does not have CSC. Which means
	 * if the JPEG file is encoded in YUV420_2p(NV12) then the output can only be
	 * the same.
	 * The output format is obtained after parsing the first enqueued input
	 * frame buffer. If the first frame is not received yet, we cannot validate
	 * the output format.
	 * It is assumed that all the following JPEG frames in the stream have the
	...
	if (is_input) {
		...
		if (fmt->pixelformat != data->m2m.out.fmt.pixelformat) {
			LOG_ERR("JPEG decoder does not support CSC,
				the output format can not be changed.");
			return -EINVAL;
		}
...
static int mcux_jpegdec_enqueue(const struct device *dev, struct video_buffer *vbuf)
{
	 ...
	 */
	if (is_input) {
		if (((vbuf->size & 0x3FFU) != 0U) || (vbuf->size == 0U)) {
			LOG_ERR("Input buffer size must be non-zero and 1024-byte aligned");
			return -EINVAL;
		}
		...
		if (!data->first_frame_rcv) {
			...
			data->decoder_despt.config.jpegBufSize = vbuf->size;
			...
			data->format_idx =
				mcux_jpegdec_get_conf(data->decoder_despt.config.pixelFormat);
		...
		/*
		 * First input frame has been received, the output buffer can be verified
		 * and enqueued.
		 */
		if (data->first_frame_rcv) {
			/* Check if the output buffer size is sufficient */
			if (vbuf->size < data->m2m.out.fmt.size) {
				LOG_ERR("Output buffer size is insufficient");
				return -EINVAL;
			}
		} else {
			LOG_WRN("Output buffer cannot be enqueued before first input buffer is "
				"received");
			return -EAGAIN;
		}
	}
