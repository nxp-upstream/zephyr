...
static void mcux_jpegdec_decode_one_frame(const struct device *dev)
{
	...
	if (data->m2m.out.fmt.pixelformat == VIDEO_PIX_FMT_NV12) {
		data->decoder_despt.config.outBufAddr1 = (uint32_t)current_out->buffer
...
static int mcux_jpegdec_enqueue(const struct device *dev, struct video_buffer *vbuf)
{
	...
	if (is_input) {
		...
		if (!data->first_frame_rcv) {
			...
			data->m2m.out.fmt.pixelformat = pixel_map_confs[data->format_idx].vid_pixel_format;
			 ...
			 */
			data->m2m.out.fmt.pitch = pixel_map_confs[data->format_idx].bytes_per_pixel
...
static int mcux_jpegdec_init(const struct device *dev)
{
	...
	JPEGDEC_EnableInterrupts((JPEG_DECODER_Type *)&config->base, 0U, kJPEGDEC_DecodeCompleteFlag | kJPEGDEC_ErrorFlags);
...
static void mcux_jpegdec_isr(const struct device *dev)
{
	...
	/* Decode complete. */
    if ((JPEGDEC_GetStatusFlags((JPEG_DECODER_Type *)&config->base, 0) & kJPEGDEC_DecodeCompleteFlag) != 0U)
    {
        JPEGDEC_ClearStatusFlags((JPEG_DECODER_Type *)&config->base, 0, kJPEGDEC_DecodeCompleteFlag);
		current_out->bytesused = data->m2m.out.fmt.size;
    }
	
    /* Error occur, this output bufer is broken, no valid data can be used. */
    if ((JPEGDEC_GetStatusFlags((JPEG_DECODER_Type *)&config->base, 0) & kJPEGDEC_ErrorFlags) != 0U)
    {
		JPEGDEC_ClearStatusFlags((JPEG_DECODER_Type *)&config->base, 0, kJPEGDEC_ErrorFlags);
