...
struct nxp_video_sdma_data {
	...
	uint32_t frame_idx __aligned(4);
				...
				uint32_t channel, int status)
{
	...
	if (data->buf_reload_flag) {
		...
		if (data->queued_buf != NULL) {
			data->params.p_buffer = (uint32_t *)data->queued_buf->buffer;
		}
		data->stream_starved = (data->queued_buf == NULL);
	} else {
		if (data->stream_starved) {
			/* Signal any waiting threads. We will not write
			 * the current buffer to the output FIFO, as the
			 * SmartDMA will keep overwriting it with video data
			 * until we queue a new buffer
			 */
			k_sem_give(&data->stream_empty);
		} else {
			/* We have finished a frame, dequeue video buffer */
			data->active_buf->frame_idx = (data->frame_idx - 1) / 2;
...
static int nxp_video_sdma_stream_start(const struct device *dev)
{
	 ...
	 * (from 0-16) into this buffer
	 */
	data->params.p_buffer_ping_pong = &data->frame_idx;
				  ...
				  struct video_buffer *vbuf)
{
	...
	vbuf->bytesused = SDMA_VBUF_WIDTH * SDMA_LINE_COUNT;
