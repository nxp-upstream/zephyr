...
#if defined(CONFIG_VIDEO_AP1302_UPLOAD_FIRMWARE)
static const uint8_t ap1302_fw_blob[] = {
#include "ap1302_firmware_file.inc"
};
#endif
...
#define AP1302_FW_ADDR_SIZE	2U
#define AP1302_FW_WRITE_MAX	256U
...
static int ap1302_write_fw_window(const struct device *dev, const uint8_t *buf, uint32_t len,
				  unsigned int *win_pos)
{
	...
	while (pos < len) {
		uint32_t write_size;
		uint16_t write_addr;
		uint8_t i2c_buf[AP1302_FW_ADDR_SIZE + AP1302_FW_WRITE_MAX];

		write_size = MIN(len - pos, AP1302_FW_WINDOW_SIZE - *win_pos);
		write_size = MIN(write_size, (uint32_t)AP1302_FW_WRITE_MAX);
		write_addr = (uint16_t)(AP1302_FW_WINDOW_OFFSET + *win_pos);

		sys_put_be16(write_addr, &i2c_buf[0]);
		memcpy(&i2c_buf[AP1302_FW_ADDR_SIZE], &buf[pos], write_size);

		/* Write to device */
		ret = i2c_write_dt(&cfg->i2c, i2c_buf, write_size + AP1302_FW_ADDR_SIZE);
		if (ret < 0) {
...
static int ap1302_load_firmware(const struct device *dev)
{
...
#if defined(CONFIG_VIDEO_AP1302_UPLOAD_FIRMWARE)
	size_t fw_blob_size = sizeof(ap1302_fw_blob);
	...
	LOG_INF("Loading firmware: PLL init size %u, total size %u", fw_hdr->pll_init_size,
		fw_size);
...
static int ap1302_detect_chip(const struct device *dev)
{
	...
	if (chip_version != AP1302_CHIP_ID) {
		LOG_ERR("Invalid chip version: expected 0x%04x, got 0x%04x", AP1302_CHIP_ID,
			chip_version);
		return -ENODEV;
	}
	...
	LOG_INF("AP1302 detected: version 0x%04x, revision %u.%u.%u", chip_version,
		(chip_rev & 0xF000) >> 12, (chip_rev & 0x0F00) >> 8, chip_rev & 0x00FF);
...
static int ap1302_stall(const struct device *dev, bool stall)
{
	...
	if (stall) {
		...
		ret = video_write_cci_reg(&cfg->i2c, AP1302_SYS_START,
					  AP1302_SYS_START_PLL_LOCK | AP1302_SYS_START_STALL_EN |
						  AP1302_SYS_START_STALL_MODE_DISABLED);
		if (ret < 0) {
			return ret;
		}
		...
		ret = video_write_cci_reg(
			&cfg->i2c, AP1302_SYS_START,
			AP1302_SYS_START_PLL_LOCK | AP1302_SYS_START_STALL_STATUS |
				AP1302_SYS_START_STALL_EN | AP1302_SYS_START_STALL_MODE_DISABLED);
		if (ret < 0) {
			return ret;
		}
	}
...
static int ap1302_init_ctrls(const struct device *dev)
{
	...
	ret = video_init_int_menu_ctrl(&data->ctrls.linkfreq, dev, VIDEO_CID_LINK_FREQ, 0,
				       ap1302_link_frequencies,
...
static int ap1302_apply_pinctrl(const struct ap1302_config *cfg)
{
	int ret;
...
static int ap1302_enable_clock(const struct ap1302_config *cfg)
{
	int ret;
...
static void ap1302_disable_clock(const struct ap1302_config *cfg)
{
	if (cfg->clock_dev != NULL) {
...
static int ap1302_setup_isp_en_gpio(const struct ap1302_config *cfg)
{
	int ret;
...
static int ap1302_init(const struct device *dev)
{
	...
	ret = ap1302_apply_pinctrl(cfg);
	if (ret < 0) {
		return ret;
	}

	ret = ap1302_enable_clock(cfg);
	if (ret < 0) {
		return ret;
	}
	...
	ret = ap1302_setup_isp_en_gpio(cfg);
	if (ret < 0) {
		goto err_clock;
	}
...
#define AP1302_FW_PART_ADDR(n)									\
	COND_CODE_1(DT_INST_NODE_HAS_PROP(n, firmware_partition),				\
			(DT_REG_ADDR(DT_INST_PHANDLE(n, firmware_partition))), (0U))

#define AP1302_FW_PART_SIZE(n)									\
	COND_CODE_1(DT_INST_NODE_HAS_PROP(n, firmware_partition),				\
			(DT_REG_SIZE(DT_INST_PHANDLE(n, firmware_partition))), (0U))
...
#define AP1302_INIT(n)										\
	...
	static const struct ap1302_config ap1302_config_##n = {					\
		...
		.firmware_partition_addr = AP1302_FW_PART_ADDR(n),				\
		.firmware_partition_size = AP1302_FW_PART_SIZE(n),				\
	...
	DEVICE_DT_INST_DEFINE(n, ap1302_init, NULL, &ap1302_data_##n, &ap1302_config_##n,	\
			      POST_KERNEL, CONFIG_VIDEO_AP1302_INIT_PRIORITY,			\
			      &ap1302_driver_api);						\
