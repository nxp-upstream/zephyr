...
static const struct linear_range vref_trim = LINEAR_RANGE_INIT(1200000, 500U, 0x0U, 0x3FU);
...
struct nxp_vref_config {
	...
	VREF_Type *base;
	uint16_t buffer_start_delay;
	uint16_t bandgap_start_time;
...
#define REGULATOR_NXP_VREF_DEFINE(inst)						\
	static struct nxp_vref_data _CONCAT(data, inst);		\
										\
	static const struct nxp_vref_config _CONCAT(config, inst) = {	\
		.common = REGULATOR_DT_INST_COMMON_CONFIG_INIT(inst),		\
		.base = (VREF_Type *) DT_INST_REG_ADDR(inst),			\
		.buffer_start_delay = DT_INST_PROP(inst,			\
				buffer_start_delay),				\
		.bandgap_start_time = DT_INST_PROP(inst,			\
				bandgap_start_time),				\
		...
		.internal_voltage_regulator_en = DT_INST_PROP(inst,		\
				internal_voltage_regulator_en),			\
										...
										\
	DEVICE_DT_INST_DEFINE(inst, regulator_nxp_vref_init, NULL,		\
			&_CONCAT(data, inst), &_CONCAT(config, inst),		\
			POST_KERNEL, CONFIG_REGULATOR_NXP_VREF_INIT_PRIORITY,	\
			&nxp_vref_api);						\
