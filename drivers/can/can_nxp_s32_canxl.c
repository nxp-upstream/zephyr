...
#else /* not CONFIG_SOC_SERIES_S32ZE */
	data->rx_cbs[alloc].rx_info = (Canexcel_Ip_RxMdConfigType) {
					...
					struct can_frame *frame)
{
	...
	if (!!(msg_data[RX_ID] & CANXL_TX_HEADER_RTR_MASK)) {
		...
		memcpy(frame->data, &msg_data[RX_FD_DATA], can_dlc_to_bytes(frame->dlc));
	}

#ifdef CONFIG_CAN_RX_TIMESTAMP
	frame->timestamp = (uint64)msg_data[RX_XL_TIMESTAMPH] << 32 | msg_data[RX_XL_TIMESTAMPL];
#endif /* CAN_RX_TIMESTAMP */
}
#endif /* CONFIG_SOC_SERIES_S32ZE */
