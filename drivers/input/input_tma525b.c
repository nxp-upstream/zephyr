...
static int tma525b_process(const struct device *dev)
{
	...
	uint8_t reg_addr = TMA525B_TOUCH_DATA_SUBADDR;
	int ret;
	uint8_t i, j;
	int x, y;
	uint8_t touch_id;
	...
	ret = i2c_burst_read_dt(&config->bus, reg_addr, data->touch_buf,
	...
	ret = i2c_burst_read_dt(&config->bus, reg_addr, data->touch_buf, touch_data->length);
	if (ret < 0) {
	...
	for (i = 0; i < data->prev_touch_count; i++) {
		touch_found = false;
...
static int tma525b_chip_init(const struct device *dev)
{
	...
	while (retry < CONFIG_INPUT_TMA525B_RETRY_TIMES) {
		ret = i2c_burst_read_dt(&config->bus, reg_addr, read_buf, sizeof(read_buf));
		if (ret == 0) {
...
static int tma525b_init(const struct device *dev)
{
	...
	LOG_INF("TMA525B using interrupt mode");
#else
	...
	LOG_INF("TMA525B using polling mode");
#endif
	...
	LOG_INF("TMA525B initialized successfully");
	return 0;
}
