...
int syscon_lpc55sxx_pll0_notify(const struct clk *clk, const struct clk *parent,
...
const struct clock_driver_api nxp_syscon_pll0_api = {
	...
	.notify = syscon_lpc55sxx_pll0_notify,
};
...

{
	 ...
	 */
	input_clk = clock_get_rate(clk_data->parent);
	if ((input->reg_settings->CTRL & SYSCON_PLL1CTRL_BYPASSPREDIV_MASK) == 0) {
		/* Input passes through prediv */
...
int syscon_lpc55sxx_pll1_notify(const struct clk *clk, const struct clk *parent,
				uint32_t parent_rate)
{
	struct lpc55sxx_pll1_data *clk_data = clk->hw_data;
	/*
...
const struct clock_driver_api nxp_syscon_pll1_api = {
	...
	.notify = syscon_lpc55sxx_pll1_notify,
};
