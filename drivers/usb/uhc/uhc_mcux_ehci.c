	...
	if ((xfer->buf != NULL) && (transfer->transferBuffer != NULL)) {
		if (transfer->transferSofar > 0) {
			...
			if (USB_EP_DIR_IS_IN(xfer->ep)) {
				memcpy(net_buf_tail(xfer->buf), transfer->transferBuffer,
...
#if defined(CONFIG_NOCACHE_MEMORY)
	if (USB_EP_GET_IDX(xfer->ep) == 0 && mcux_xfer->setupPacket != NULL) {
		...
		memcpy(nocache_buf, mcux_xfer->setupPacket, 8u);
		mcux_xfer->setupPacket = nocache_buf;
	} else {
