...
/**
 * @brief Prepare endpoint queues for new control transfer
 *
 * Complete any pending data/status requests, mark endpoints as not busy.
 *
 * UDC driver must ensure that driver will not access any of queued buffers
 * before calling this function.
 *
 * @param[in] dev    Pointer to device struct of the driver instance
 *
 * @return pointer to buffer where USB stack wants to receive SETUP data.
 *         NULL if USB stack is not ready yet.
 */
struct net_buf *udc_setup_received(const struct device *dev);
