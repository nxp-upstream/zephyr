...
struct udc_stm32_data  {
	...
	/* Tracks if a SETUP packet was received and
	 * buffered but not acknowledged
	 * by the UDC stack yet
	 */
	bool pending_setup;
};
...
static int udc_stm32_submit_setup(struct udc_stm32_data *priv, struct net_buf *buf)
{
	/* HAL copies SETUP packet contents to pcd.Setup */
	net_buf_add_mem(buf, priv->pcd.Setup, sizeof(struct usb_setup_packet));

	return udc_submit_ep_event(priv->dev, buf, 0);
}
...
static void handle_msg_setup(struct udc_stm32_data *priv)
{
	...
	if (buf == NULL) {
		/*
		 * UDC stack not ready to process SETUP packet.
		 * Remember we have one pending such that we
		 * can provide it upon request in ep_enqueue().
		 */
		priv->pending_setup = true;
		return;
	} else {
		udc_stm32_submit_setup(priv, buf);
	}
}
	...
	if (ep_cfg->addr == USB_CONTROL_EP_OUT) {
		...
		if (bi->setup) {
			...
			if (priv->pending_setup) {
				ret = udc_stm32_submit_setup(priv, buf);
				goto fini;
			}
		}
	}
