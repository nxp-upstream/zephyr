...
extern int nxp_nbu_set_tx_power(int8_t level_dbm, uint8_t handle_type);
...
static void bt_nxp_send_vs_cmd_complete(const struct device *dev, uint16_t opcode, uint8_t status)
{
	 ...
	 * [5] Status
	 */
	payload = net_buf_add(buf, BT_HCI_EVT_HDR_SIZE + sizeof(struct bt_hci_evt_cmd_complete) + 1U);
	payload[0U] = BT_HCI_EVT_CMD_COMPLETE;
	payload[1U] = sizeof(struct bt_hci_evt_cmd_complete) + 1U;
	payload[2U] = 1U;
	sys_put_le16(opcode, &payload[3U]);
	payload[5U] = status;
...
static int bt_nxp_process_tx_power_cmd(const uint8_t *params, uint8_t params_len)
{
	...
	do
	{
		 ...
		 * - tx_power: 1 byte
		 */
		if ((params != NULL) && (params_len < sizeof(struct bt_hci_cp_vs_write_tx_power_level))) {
			status = -EINVAL;
			break;
		}
		...
		LOG_DBG("TX Power: level=%d dBm, type=%d, handle=0x%04x",
			level_dbm, handle_type, handle);
...
static bool bt_nxp_process_vs_command(const struct device *dev, struct net_buf *buf)
{
	...
	if (buf->len < (BT_HCI_CMD_HDR_SIZE + param_len + 1U)) {
		LOG_WRN("VS command buffer too short: len=%u, expected=%u",
			buf->len, BT_HCI_CMD_HDR_SIZE + param_len + 1U);
		return false;
	}
