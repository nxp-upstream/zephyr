...
/* mux-states entries are configured once at init; use generic MUX instance helpers */
#define UART_MCUX_FLEXCOMM_MUX_DEFINE(n) MUX_CONTROL_DT_INST_DEFINE(fc_uart, n)
...
#define UART_MCUX_FLEXCOMM_ASYNC_CFG(n)						\
	...
	.mux_dev = COND_CODE_1(DT_INST_NODE_HAS_PROP(n, mux_states),		\
			(MUX_CONTROL_DT_INST_DEV_GET(n)), (NULL)),		\
	.mux_controls = MUX_CONTROL_DT_INST_ARRAY_GET(fc_uart, n),		\
	.num_muxes = MUX_CONTROL_DT_INST_COUNT(n),
#else
#define UART_MCUX_FLEXCOMM_ASYNC_CFG(n)
#endif /* CONFIG_UART_ASYNC_API */
