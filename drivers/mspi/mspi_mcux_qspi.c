...
#define DT_DRV_COMPAT nxp_mcux_qspi
...
#include "fsl_qspi.h"
#if defined(CONFIG_SOC_MCXE247)
#include "fsl_qspi_soc.h"
#endif
#include "fsl_clock.h"

LOG_MODULE_REGISTER(mspi_mcux_qspi, CONFIG_MSPI_LOG_LEVEL);

#define MSPI_MCUX_QSPI_LUT_SEQ_IDX       0
#define MSPI_MCUX_QSPI_LUT_SEQ_UNIT      FSL_FEATURE_QSPI_LUT_SEQ_UNIT
#define MSPI_MCUX_QSPI_LUT_SEQ_INSTR_MAX (MSPI_MCUX_QSPI_LUT_SEQ_UNIT * 2)

struct mspi_mcux_qspi_config {
	QuadSPI_Type *base;
	uint32_t amba_base;
	uint32_t amba_size;
...
static uint8_t mspi_mcux_qspi_io_mode_to_pad(enum mspi_io_mode mode)
{
	switch (mode) {
...
static int mspi_mcux_qspi_setup_lut(QuadSPI_Type *base, int seq_idx,
				      const struct mspi_xfer *xfer,
				      const struct mspi_xfer_packet *pkt,
				      enum mspi_io_mode io_mode)
{
	uint32_t lut_base = seq_idx * MSPI_MCUX_QSPI_LUT_SEQ_UNIT;
	uint32_t lut[MSPI_MCUX_QSPI_LUT_SEQ_UNIT];
	uint8_t cmd[MSPI_MCUX_QSPI_LUT_SEQ_INSTR_MAX];
	uint8_t operand[MSPI_MCUX_QSPI_LUT_SEQ_INSTR_MAX];
	uint8_t pad[MSPI_MCUX_QSPI_LUT_SEQ_INSTR_MAX];
	uint16_t dummy;
	uint8_t n = 0;
	...
	if (dummy > 0) {
		cmd[n] = QSPI_DUMMY;
		pad[n] = mspi_mcux_qspi_io_mode_to_pad(io_mode);
	...
	if (n > MSPI_MCUX_QSPI_LUT_SEQ_INSTR_MAX) {
	...
	base->LCKCR = 0x2U;
	for (int i = 0; i < MSPI_MCUX_QSPI_LUT_SEQ_UNIT; i++) {
...
static int mspi_mcux_qspi_ip_read(QuadSPI_Type *base, uint32_t addr, uint8_t seq_idx,
...
/*
 * Bytes beyond len are padded with 0xFF (NOR flash erased value) so that partial
 * writes don't corrupt adjacent flash content. The QSPI FIFO expects big-endian
 * byte order within each word, a byte-swap is applied before writing to TBDR.
 */
static void mspi_mcux_qspi_tx_word(QuadSPI_Type *base, const uint8_t *src,
				    uint32_t off, uint32_t len)
{
	uint32_t word = 0xFFFFFFFFU;

	if (off < len) {
		uint32_t n = MIN(4U, len - off);

		memcpy(&word, src + off, n);
		if (n < 4U) {
			memset(((uint8_t *)&word) + n, 0xFF, 4U - n);
		}
	}
...
static int mspi_mcux_qspi_ip_write(QuadSPI_Type *base, uint32_t addr, uint8_t seq_idx,
			  const void *buf, uint32_t len)
{
	...
	for (uint32_t i = 0; i < prefill; i++) {
		mspi_mcux_qspi_tx_word(base, buf, i * 4U, len);
	}
...
static int mspi_mcux_qspi_ip_cmd(QuadSPI_Type *base, uint32_t addr, uint8_t seq_idx)
{
	QSPI_SetIPCommandAddress(base, addr);
	QSPI_ExecuteIPCommand(base, seq_idx);
	while (base->SR & QuadSPI_SR_BUSY_MASK) {
...
#if defined(CONFIG_SOC_MCXE247)
static void mspi_mcux_qspi_soc_configure(QuadSPI_Type *base,
					  uint32_t src_freq, uint32_t max_freq)
{
	qspi_soc_config_t soc_cfg = {0};
	uint8_t div = CLAMP((src_freq + max_freq - 1) / max_freq, 1, 8);

	soc_cfg.inputBufEnable = true;
	soc_cfg.divEnable = true;
	soc_cfg.internalClk = kQSPI_PllDiv1Clock;
	soc_cfg.clkMode = kQSPI_SysClock;
	soc_cfg.clkDiv = div;

	QSPI_SocConfigure(base, &soc_cfg);
}
#endif

static int mspi_mcux_qspi_config(const struct mspi_dt_spec *spec)
{
	const struct device *dev = spec->bus;
	const struct mspi_mcux_qspi_config *cfg = dev->config;
	...
	mspi_mcux_qspi_soc_configure(base, clock_rate, spec->config.max_freq);
...
static int mspi_mcux_qspi_dev_config(const struct device *dev,
				const struct mspi_dev_id *dev_id,
				const enum mspi_dev_cfg_mask param_mask,
				const struct mspi_dev_cfg *cfg)
{
	struct mspi_mcux_qspi_data *data = dev->data;
...
static int mspi_mcux_qspi_transceive(const struct device *dev,
				const struct mspi_dev_id *dev_id,
				const struct mspi_xfer *xfer)
{
	const struct mspi_mcux_qspi_config *cfg = dev->config;
	...
	for (uint32_t i = 0; i < xfer->num_packet; i++) {
		...
		ret = mspi_mcux_qspi_setup_lut(base, MSPI_MCUX_QSPI_LUT_SEQ_IDX,
		...
		if (pkt->num_bytes == 0) {
			ret = mspi_mcux_qspi_ip_cmd(base, addr,
					       MSPI_MCUX_QSPI_LUT_SEQ_IDX);
		} else if (pkt->dir == MSPI_RX) {
			ret = mspi_mcux_qspi_ip_read(base, addr,
					   MSPI_MCUX_QSPI_LUT_SEQ_IDX,
					   pkt->data_buf, pkt->num_bytes);
		} else {
			ret = mspi_mcux_qspi_ip_write(base, addr,
					    MSPI_MCUX_QSPI_LUT_SEQ_IDX,
					    pkt->data_buf, pkt->num_bytes);
		}
...
static int mspi_mcux_qspi_get_channel_status(const struct device *dev, uint8_t ch)
{
	struct mspi_mcux_qspi_data *data = dev->data;
...
static int mspi_mcux_qspi_init(const struct device *dev)
{
	const struct mspi_mcux_qspi_config *cfg = dev->config;
	struct mspi_mcux_qspi_data *data = dev->data;
	...
	return mspi_mcux_qspi_config(&spec);
}

static DEVICE_API(mspi, mspi_mcux_qspi_api) = {
	.config = mspi_mcux_qspi_config,
	.dev_config = mspi_mcux_qspi_dev_config,
	.get_channel_status = mspi_mcux_qspi_get_channel_status,
	.transceive = mspi_mcux_qspi_transceive,
};

#define MCUX_QSPI_INIT(n)                                                      \
	PINCTRL_DT_INST_DEFINE(n);                                             \
	static const struct mspi_mcux_qspi_config mspi_mcux_qspi_cfg_##n = {    \
	...
	static struct mspi_mcux_qspi_data mspi_mcux_qspi_data_##n;              \
	DEVICE_DT_INST_DEFINE(n,                                               \
			      mspi_mcux_qspi_init,                                   \
			      NULL,                                            \
			      &mspi_mcux_qspi_data_##n,                        \
			      &mspi_mcux_qspi_cfg_##n,                         \
			      POST_KERNEL,                                     \
