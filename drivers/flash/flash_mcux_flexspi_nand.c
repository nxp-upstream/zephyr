...
static int flash_flexspi_nand_xfer(const struct device *dev, uint8_t seq_index,
			 flexspi_command_type_t type, uint32_t addr, void *data, size_t data_len)
{
	struct flash_flexspi_nand_data *nand = dev->data;
...
restore:
	 ...
	 */
	(void)flash_flexspi_nand_write_reg(dev, SPI_NAND_REG_CONFIG, (cfg_restore |
		(cfg & SPI_NAND_CFG_ECC_EN)) & ~SPI_NAND_CFG_OTP_EN);
 ...
 *
 * Sequence: READ_CACHE (0x03) | CA[15:8]=0x08 | CA[7:0]=0x00 | Dummy(8) | Data
 */
static int flash_flexspi_nand_read_oob0(const struct device *dev, uint8_t *byte)
{
	const uint32_t lut_read_oob[MEMC_FLEXSPI_CMD_PER_SEQ] = {
		...
		FLEXSPI_LUT_SEQ(kFLEXSPI_Command_READ_SDR, kFLEXSPI_1PAD, 0x04U,
				kFLEXSPI_Command_STOP, kFLEXSPI_1PAD, 0),
		0,
	};
	uint32_t buf = 0;
...
static int flash_flexspi_nand_mark_oob0(const struct device *dev, off_t block_addr)
{
	...
	ret = flash_flexspi_nand_xfer(dev, PROG_EXEC, kFLEXSPI_Command,
...
static int flash_flexspi_nand_ex_op(const struct device *dev, uint16_t code,
				    const uintptr_t in, void *out)
{
	...
	case FLASH_EX_OP_RESET:
		return flash_flexspi_nand_reset(dev);
	...
	case FLASH_EX_OP_MARK_BAD_BLOCK: {
		...
		return flash_flexspi_nand_mark_oob0(dev, block_addr);
	}
...
#define FLASH_FLEXSPI_NAND(n)                                                       \
	...
	DEVICE_DT_INST_DEFINE(n, flash_flexspi_nand_init, NULL, &flash_flexspi_nand_data_##n, \
			      &flash_flexspi_nand_config_##n, POST_KERNEL,                            \
				  CONFIG_FLASH_MCUX_FLEXSPI_NAND_INIT_PRIORITY, &flash_flexspi_nand_api);
