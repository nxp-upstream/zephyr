...
/*
 * Unified LUT sequence layout shared by every supported chip. Each chip supplies its
 * own LUT array indexed by these constants, so the driver can reference operations
 * generically regardless of protocol (octal-OPI vs quad-SPI).
 *
 ...
 */
static const uint32_t flash_xspi_lut_mx25um[FLASH_CMD_SEQ_MAX][5] = {
	[FLASH_CMD_MEM_READ] = {
...
/* W25Q512NWEIQ — Winbond quad SPI, 64 MB, 4-byte addressing. */
static const uint32_t flash_xspi_lut_w25q512nw[FLASH_CMD_SEQ_MAX][5] = {
	[FLASH_CMD_MEM_READ] = {
