...
struct mcux_lpit_channel_data {
	...
	counter_alarm_callback_t alarm_callback; /* Alarm callback */
	void *alarm_user_data;                   /* User data for alarm callback */
};
...
static int mcux_lpit_set_top_value(const struct device *dev, const struct counter_top_cfg *cfg)
{
	...
	uint8_t channel_id = LPIT_CHANNEL_ID(dev);
	bool running = (config->base->CHANNEL[channel_id].TCTRL & LPIT_TCTRL_T_EN_MASK);

	if (cfg->ticks == 0U || cfg->ticks > config->info.max_top_value) {
		return -EINVAL;
	}
...
static int mcux_lpit_set_alarm(const struct device *dev, uint8_t chan_id,
			       const struct counter_alarm_cfg *alarm_cfg)
{
	const struct mcux_lpit_config *config = dev->config;
	struct mcux_lpit_channel_data *data = LPIT_CHANNEL_DATA(dev);
	uint8_t channel = LPIT_CHANNEL_ID(dev);
	uint32_t final_ticks = alarm_cfg->ticks;
	uint32_t now, max_ticks;

	/* Return error if device does not support requested channel */
	if (chan_id != channel) {
		return -ENOTSUP;
	}

	now = LPIT_GetCurrentTimerCount(config->base, channel);

	if (alarm_cfg->flags & COUNTER_ALARM_CFG_ABSOLUTE) {
		max_ticks = config->info.max_top_value;
	} else {
		max_ticks = config->info.max_top_value - now;
	}

	if (final_ticks > max_ticks) {
		LOG_ERR("Alarm ticks %d exceeds max top value %d", final_ticks, max_ticks);
		return -EINVAL;
	}
	...
	/* The underlying HAL driver function LPIT_SetTimerPeriod() assert(ticks > 2U) */
	if (final_ticks <= 2U) {
		return -EINVAL;
	}
	/* if top value has been set, the alarm ticks should not be bigger than top value*/
	if (data->top_callback && final_ticks > mcux_lpit_get_top_value(dev)) {
		return -EINVAL;
	}
	...
	/* To abort the current timer cycle and start a timer period with a new value,
	 * you must disable the timer channel and then enable it again.
	 */
	LPIT_StopTimer(config->base, channel);
	LPIT_SetTimerPeriod(config->base, channel, final_ticks);
	LPIT_EnableInterrupts(config->base, (1 << channel));
	LPIT_StartTimer(config->base, channel);

	return 0;
}

static int mcux_lpit_cancel_alarm(const struct device *dev, uint8_t chan_id)
{
	const struct mcux_lpit_config *config = dev->config;
	struct mcux_lpit_channel_data *data = LPIT_CHANNEL_DATA(dev);
	uint8_t channel = LPIT_CHANNEL_ID(dev);

	/* Return if device does not support requested channel*/
	if (chan_id != channel) {
		return -ENOTSUP;
	}
	LPIT_DisableInterrupts(config->base, (1 << channel));
	LPIT_StopTimer(config->base, channel);
...
static void mcux_lpit_isr(const struct device *dev)
{
	...
	for (int channel_index = 0; channel_index < config->num_channels; channel_index++) {
		...
		if (data->alarm_callback != NULL) {
			...
			counter_alarm_callback_t current_alarm_callback = data->alarm_callback;
			/* dsiable alarm callback for next alarm set up */
			data->alarm_callback = NULL;

			uint32_t current_ticks =
				LPIT_GetCurrentTimerCount(config->base, channel_index);
			/* Pass channel device as callback first parameter, because the
			 * application use channel api.
			 */
			current_alarm_callback(config->channels[channel_index], channel_index,
					       current_ticks, data->alarm_user_data);
		}
...
static DEVICE_API(counter, mcux_lpit_driver_api) = {
	...
	.set_alarm = mcux_lpit_set_alarm,
	.cancel_alarm = mcux_lpit_cancel_alarm,
};
	...
	static const struct mcux_lpit_config mcux_lpit_##n##_config = {                            \
		.info =                                                                            \
			{                                                                          \
				...
				.channels = DT_INST_FOREACH_CHILD_SEP_VARGS(                       \
					n, DT_NODE_HAS_COMPAT, (+), nxp_lpit_channel),             \
			},                                                                         \
