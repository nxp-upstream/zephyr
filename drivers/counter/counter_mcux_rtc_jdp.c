...
struct mcux_rtc_jdp_data {
	...
	/* software-pending mask: bit0=RTC_ALARM_CHANNEL, bit1=API_ALARM_CHANNEL */
...
static int mcux_rtc_jdp_start(const struct device *dev)
{
	...
	RTC_EnableInterrupts(config->base, (uint32_t)(kRTC_RTCInterruptEnable |
			        kRTC_APIInterruptEnable | kRTC_CounterRollOverInterruptEnable));
	return 0;
}
                  ...
                  const struct counter_alarm_cfg *alarm_cfg)
{
    ...
    const uint32_t top = info->max_top_value;
    const bool absolute = (alarm_cfg->flags & COUNTER_ALARM_CFG_ABSOLUTE) != 0U;
    uint32_t val = alarm_cfg->ticks;
    uint32_t now = RTC_GetCountValue(config->base);

    /* Determine late detection policy */
    uint32_t max_rel_val;
    bool irq_on_late;

    if (absolute) {
        /* Limit to detect too-late absolute setting. */
        ...
        /* Relative alarms: apply late-detection if short relative. */
        irq_on_late = (val < (top / 2U));
        max_rel_val = irq_on_late ? (top / 2U) : top;
        val = now + val; /* wraps naturally on 32-bit */
    }

    /* Program compare for the selected channel */
    ...
    /* Re-read counter and evaluate lateness */
    uint32_t cnt2 = RTC_GetCountValue(config->base);
    /* Detect "too late": diff = (val - 1) - cnt2 (wraps on 32-bit) */
    uint32_t diff = (val - 1U) - cnt2;
    int err = 0;
    ...
    if (diff > max_rel_val) {
        ...
        if (irq_on_late) {
            /* Trigger ISR path instead of calling callback directly */
            data->sw_pending_mask |= BIT(chan_id);
            irq_enable(config->irqn);
            NVIC_SetPendingIRQ(config->irqn);
        } else {
            /* Drop the alarm; no callback */
            data->alarm_callback[chan_id] = NULL;
            data->alarm_user_data[chan_id] = NULL;
        }
    } else {
        if (diff == 0U) {
            /* Edge case: fire immediately via ISR */
            data->sw_pending_mask |= BIT(chan_id);
            irq_enable(config->irqn);
            NVIC_SetPendingIRQ(config->irqn);
        } else {
            /* Normal case: ensure IRQ enabled for HW match */
            irq_enable(config->irqn);
        }
    }
...
static int mcux_rtc_jdp_set_top_value(const struct device *dev,
...
static uint32_t mcux_rtc_jdp_get_pending_int(const struct device *dev)
{
	...
	uint32_t flags = RTC_GetInterruptFlags(config->base) &
...
static int mcux_rtc_jdp_set_guard_period(const struct device *dev, uint32_t ticks,
					 uint32_t flags)
{
	struct mcux_rtc_jdp_data *data = dev->data;
...
static void mcux_rtc_jdp_isr(const struct device *dev)
{
    ...
    if ((status & kRTC_RTCInterruptFlag) != 0U || (data->sw_pending_mask & BIT(RTC_ALARM_CHANNEL))) {
    ...
    if ((status & kRTC_APIInterruptFlag) != 0U || (data->sw_pending_mask & BIT(API_ALARM_CHANNEL))) {
...
#define MCUX_RTC_JDP_INIT(n) \
	...
	static const struct mcux_rtc_jdp_config mcux_rtc_jdp_config_##n = { \
		...
		.info = { \
			...
			.freq = (DT_INST_PROP(n, clock_frequency) + (DT_INST_PROP(n, prescaler) / 2)) / DT_INST_PROP(n, prescaler), \
	...
	DEVICE_DT_INST_DEFINE(n, &mcux_rtc_jdp_init, NULL, \
			    &mcux_rtc_jdp_data_##n, &mcux_rtc_jdp_config_##n.info, \
			    POST_KERNEL, CONFIG_COUNTER_INIT_PRIORITY, \
			    &mcux_rtc_jdp_driver_api); \
	\
	static void mcux_rtc_jdp_irq_config_##n(const struct device *dev) \
	{ \
		IRQ_CONNECT(DT_INST_IRQN(n), \
			    DT_INST_IRQ(n, priority), \
			    mcux_rtc_jdp_isr, DEVICE_DT_INST_GET(n), 0); \
