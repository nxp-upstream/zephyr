				  ...
				  const struct counter_alarm_cfg *alarm_cfg)
{
	...
	absolute = ((alarm_cfg->flags & COUNTER_ALARM_CFG_ABSOLUTE) != 0U) ? true : false;
	val = alarm_cfg->ticks;
	...
	/* Program compare for the selected channel (store handlers first) */
	...
	cnt2 = RTC_GetCountValue(config->base);

	if (absolute != false) {
		/* Late if target time is now or in the past by <= guard ticks.
		 * Use unsigned "backward" distance to avoid signed wrap pitfalls:
		 * back = now - val; if back is small -> val <= now (within guard).
		 */
		back = cnt2 - val;
		...
		/* Relative late detection as before */
		diff = (val - 1U) - cnt2;
