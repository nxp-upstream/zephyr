				  ...
				  const struct counter_alarm_cfg *alarm_cfg)
{
	...
	/* Re-read counter and evaluate lateness/immediacy */
	cnt2 = RTC_GetCountValue(config->base);

	if (is_abs != false) {
		/* Late if target is now/past within guard: back = now - target */
		back = cnt2 - val;
		...
		/* Relative: diff = (target - 1) - now; handle late or immediate */
		diff = (val - 1U) - cnt2;
