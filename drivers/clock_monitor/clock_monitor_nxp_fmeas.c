...
/* Result counter is 31 bits; the HW formula subtracts a fixed +2 bias, so a
 * captured value below 2 means no target edges were counted (clock lost).
 */
...
static int nxp_fmeas_set_source(const struct device *dev, uint32_t reference, uint32_t target)
{
	...
	struct nxp_fmeas_data *data = dev->data;
	k_spinlock_key_t key;
	const struct nxp_fmeas_inputmux_entry *ref_entry;
	const struct nxp_fmeas_inputmux_entry *tar_entry;
