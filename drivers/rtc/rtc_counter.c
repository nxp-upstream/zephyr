				      ...
				      const struct rtc_time *timeptr)
{
	...
	ret = counter_rtc_time_to_ticks(timeptr, &desired_ticks);
	...
	alarm_cfg.callback = counter_rtc_alarm_callback;
 ...
 */
static inline bool rtc_compute_rearm_ticks(int64_t raw_alarm_ticks_64, uint32_t now_raw,
...
static void counter_rtc_reschedule_alarms(const struct device *dev)
{
	const struct counter_rtc_config *config = dev->config;
	struct counter_rtc_data *data = dev->data;
	...
	for (uint8_t id = 0U; id < data->num_alarm_chans; id++) {
		...
		if (counter_rtc_time_to_ticks(&configured_time, &alarm_abs_ticks) < 0) {
		...
		if (!rtc_compute_rearm_ticks(raw_alarm_ticks_64, now_raw, top, &alarm_ticks)) {
			continue;
		}
...
#ifdef CONFIG_RTC_ALARM
	counter_rtc_reschedule_alarms(dev);
#endif /* CONFIG_RTC_ALARM */
...
#define COUNTER_RTC_ALARMS_COUNT(n) DT_PROP_OR(DT_DRV_INST(n), alarms_count, 0)
#define COUNTER_RTC_ALARMS_SZ(n)	\
	(COUNTER_RTC_ALARMS_COUNT(n) > 0 ? COUNTER_RTC_ALARMS_COUNT(n) : 1)

#define COUNTER_RTC_DECLARE_ALARM_STORAGE(n) \
	/* Per-instance static storage for alarm context, sized from DT */          \
	IF_ENABLED(CONFIG_RTC_ALARM,                                                \
		(static rtc_alarm_callback                                              \
			counter_rtc_alarm_callback_arr_##n[COUNTER_RTC_ALARMS_SZ(n)];       \
		 static void                                                            \
			*counter_rtc_alarm_user_data_arr_##n[COUNTER_RTC_ALARMS_SZ(n)];     \
		 static uint16_t                                                        \
			counter_rtc_alarm_mask_arr_##n[COUNTER_RTC_ALARMS_SZ(n)];           \
		 static struct rtc_time                                                 \
			counter_rtc_alarm_time_arr_##n[COUNTER_RTC_ALARMS_SZ(n)];           \
		 static bool                                                            \
			counter_rtc_alarm_pending_arr_##n[COUNTER_RTC_ALARMS_SZ(n)];))

DT_INST_FOREACH_STATUS_OKAY(COUNTER_RTC_DECLARE_ALARM_STORAGE)
...
#define COUNTER_RTC_DEVICE_INIT(n) \
	...
	static struct counter_rtc_data counter_rtc_data_##n = {                     \
		IF_ENABLED(CONFIG_RTC_ALARM, (                                          \
			.alarm_callback = counter_rtc_alarm_callback_arr_##n,               \
			.alarm_user_data = counter_rtc_alarm_user_data_arr_##n,             \
			.alarm_mask = counter_rtc_alarm_mask_arr_##n,                       \
			.alarm_time = counter_rtc_alarm_time_arr_##n,                       \
			.alarm_pending = counter_rtc_alarm_pending_arr_##n,                 \
		))                                                                      \
	};                                                                          \
