...
static void nxp_srtc_convert_seconds_to_datetime(uint32_t seconds, struct rtc_time *timeptr)
{
	uint32_t x;
...
static int nxp_srtc_set_time(const struct device *dev, const struct rtc_time *timeptr)
{
	if (!timeptr || !rtc_utils_validate_rtc_time(timeptr, 0)) {
		return -EINVAL;
	}

	const struct nxp_srtc_config *config = dev->config;
	RTC_Type *rtc_reg = config->base;

	RTC_StopTimer(rtc_reg);
	rtc_reg->TSR = nxp_srtc_convert_datetime_to_seconds(timeptr);
	RTC_StartTimer(rtc_reg);

	return 0;
}

static int nxp_srtc_get_time(const struct device *dev, struct rtc_time *timeptr)
{
	if (!timeptr) {
		return -EINVAL;
	}
	...
	nxp_srtc_convert_seconds_to_datetime(seconds, timeptr);

	return 0;
}

#if defined(CONFIG_RTC_ALARM)
static int nxp_srtc_alarm_get_supported_fields(const struct device *dev, uint16_t id,
					       uint16_t *mask)
{
	ARG_UNUSED(dev);
...
static int nxp_srtc_alarm_set_time(const struct device *dev, uint16_t id, uint16_t mask,
				   const struct rtc_time *timeptr)
{
	...
	const struct nxp_srtc_config *config = dev->config;
	struct nxp_srtc_data *data = dev->data;
	RTC_Type *rtc_reg = config->base;
	...
	alarmSeconds = nxp_srtc_convert_datetime_to_seconds(timeptr);
...
static int nxp_srtc_alarm_get_time(const struct device *dev, uint16_t id, uint16_t *mask,
				   struct rtc_time *timeptr)
{
	...
	memset(timeptr, 0, sizeof(struct rtc_time));
	nxp_srtc_convert_seconds_to_datetime(alarmSeconds, timeptr);
...
static int nxp_srtc_alarm_is_pending(const struct device *dev, uint16_t id)
{
	if (id != 0) {
		return -EINVAL;
	}
...
static int nxp_srtc_alarm_set_callback(const struct device *dev, uint16_t id,
...
static int nxp_srtc_update_set_callback(const struct device *dev, rtc_update_callback callback,
					void *user_data)
{
	struct nxp_srtc_data *data = dev->data;
	uint32_t key = irq_lock();
...
static int nxp_srtc_set_calibration(const struct device *dev, int32_t calibration)
{
	const struct nxp_srtc_config *config = dev->config;
	RTC_Type *rtc_reg = config->base;
	...
	if (is_run_under_lpo) {
		/* LPO clock source calibration */
		/*
		 * For LPO mode:
		 * - LPO increments prescaler from bit TPR[5] (TPR[4:0] ignored)
		 * - Only TCR[7:5] are used for compensation (TCR[4:0] ignored)
		 * - Compensation range: 1020 to 1027 cycles (instead of 32768)
		 * - Formula: cycles = 1024 - (TCR[7:5] * 1) where TCR[7:5] is 3-bit value (0-7)
		 */

		/* Calculate TCR value for LPO mode, rounding to nearest integer */
		int64_t tcr_calc;

		if (calibration >= 0) {
			tcr_calc = (calibration * 1024LL + 500000000LL) / 1000000000LL;
		} else {
			tcr_calc = (calibration * 1024LL - 500000000LL) / 1000000000LL;
		}

		/* If TCR is within LPO range, use CIR = 0 (every second) */
		if (tcr_calc >= -4 && tcr_calc <= 3) {
			tcr_value = (int32_t)tcr_calc;
			cir_value = 0; /* Compensation every second */
		} else {
			/* TCR out of range, need to use larger compensation interval */
			/* Find the minimum CIR that brings TCR within LPO range */
			int32_t interval;

			for (interval = 2; interval <= 256; interval++) {
				int64_t adjusted_tcr;

				if (calibration >= 0) {
					adjusted_tcr =
						(calibration * 1024LL * interval + 500000000LL) /
						1000000000LL;
				} else {
					adjusted_tcr =
						(calibration * 1024LL * interval - 500000000LL) /
				...
				if (adjusted_tcr >= -4 && adjusted_tcr <= 3) {
					tcr_value = (int32_t)adjusted_tcr;
					cir_value = interval - 1; /* CIR = interval - 1 */
					break;
				}
			}

			/* If still out of range after trying all intervals */
			if (interval > 256) {
				return -EINVAL;
			}
		}

		/* For LPO mode, only TCR[7:5] are used, so shift the value left by 5 bits */
		/* Convert signed TCR to 3-bit representation in bits [7:5] */
		if (tcr_value >= 0) {
			tcr_reg_value = (tcr_value & 0x07) << 5;
		} else {
			/* For negative values, use 3-bit two's complement */
			tcr_reg_value = ((8 + tcr_value) & 0x07) << 5;
		}

		/* Construct the TCR register value: CIR[15:8] | TCR[7:5] | 0[4:0] */
		uint32_t tcr_register = (cir_value << 8) | tcr_reg_value;

		/* Write to the Time Compensation Register */
		rtc_reg->TCR = tcr_register;

	} else {
		/* Original 32.768 kHz clock source calibration code */
		/*
		 * Convert calibration from ppb to clock cycles adjustment
		 * Formula: tcr_value = (calibration * 32768 * interval) / 1000000000
		 *
		 * The TCR field adjusts cycles: 32768 - TCR (where TCR is sign-extended)
		 * Positive calibration (fast clock) requires positive TCR to subtract cycles
		 * Negative calibration (slow clock) requires negative TCR to add cycles
		 */

		/* Calculate initial TCR value for interval=1, rounding to nearest integer */
		int64_t tcr_calc;

		if (calibration >= 0) {
			tcr_calc = (calibration * 32768LL + 500000000LL) / 1000000000LL;
		} else {
			tcr_calc = (calibration * 32768LL - 500000000LL) / 1000000000LL;
		}

		/* If TCR is within range, use CIR = 0 (every second) */
		if (tcr_calc >= -128 && tcr_calc <= 127) {
			tcr_value = (int32_t)tcr_calc;
			cir_value = 0; /* Compensation every second */
		} else {
			/* TCR out of range, need to use larger compensation interval */
			/* Find the minimum CIR that brings TCR within range */
			int32_t interval;
			...
			for (interval = 2; interval <= 256; interval++) {
				...
				if (calibration >= 0) {
					adjusted_tcr =
						(calibration * 32768LL * interval + 500000000LL) /
						1000000000LL;
				} else {
					adjusted_tcr =
						(calibration * 32768LL * interval - 500000000LL) /
				...
				if (adjusted_tcr >= -128 && adjusted_tcr <= 127) {
					tcr_value = (int32_t)adjusted_tcr;
					cir_value = interval - 1; /* CIR = interval - 1 */
					break;
				}
		...
		/* Convert signed TCR to 8-bit two's complement representation */
		if (tcr_value >= 0) {
			tcr_reg_value = tcr_value & 0xFF;
		} else {
			tcr_reg_value = (256 + tcr_value) & 0xFF;
		}

		/* Construct the TCR register value: CIR[15:8] | TCR[7:0] */
		uint32_t tcr_register = (cir_value << 8) | tcr_reg_value;
...
static int nxp_srtc_get_calibration(const struct device *dev, int32_t *calibration)
{
	if (!calibration) {
		return -EINVAL;
	}
	...
	if (is_run_under_lpo) {
		...
		if (lpo_tcr_field & 0x04) {
			...
			tcr_value = (int32_t)(lpo_tcr_field | 0xFFFFFFF8);
		} else {
			/* Positive value */
		...
		if (tcr_field & 0x80) {
			...
			tcr_value = (int32_t)(tcr_field | 0xFFFFFF00);
		} else {
			/* Positive value */
...
static int nxp_srtc_init(const struct device *dev)
{
	const struct nxp_srtc_config *config = dev->config;
	RTC_Type *rtc_reg = config->base;
...
static void nxp_srtc_isr(const struct device *dev)
{
#if defined(CONFIG_RTC_ALARM) || defined(CONFIG_RTC_UPDATE)
	const struct nxp_srtc_config *config = dev->config;
	RTC_Type *rtc_reg = config->base;
...
#if DT_INST_IRQ_HAS_IDX(0, 0) && DT_INST_IRQ_HAS_IDX(0, 1)
static void nxp_srtc_alarm_isr(const struct device *dev)
{
#ifdef CONFIG_RTC_ALARM
	const struct nxp_srtc_config *config = dev->config;
	RTC_Type *rtc_reg = config->base;
...
static void nxp_srtc_second_isr(const struct device *dev)
{
#ifdef CONFIG_RTC_UPDATE
	struct nxp_srtc_data *data = dev->data;
	uint32_t key = irq_lock();
...
static DEVICE_API(rtc, rtc_nxp_srtc_driver_api) = {
	.set_time = nxp_srtc_set_time,
	.get_time = nxp_srtc_get_time,
#if defined(CONFIG_RTC_ALARM)
	.alarm_get_supported_fields = nxp_srtc_alarm_get_supported_fields,
	.alarm_set_time = nxp_srtc_alarm_set_time,
	.alarm_get_time = nxp_srtc_alarm_get_time,
	.alarm_is_pending = nxp_srtc_alarm_is_pending,
	.alarm_set_callback = nxp_srtc_alarm_set_callback,
#endif /* CONFIG_RTC_ALARM */
#if defined(CONFIG_RTC_UPDATE)
	.update_set_callback = nxp_srtc_update_set_callback,
#endif /* CONFIG_RTC_UPDATE */
#if defined(CONFIG_RTC_CALIBRATION)
	.set_calibration = nxp_srtc_set_calibration,
	.get_calibration = nxp_srtc_get_calibration,
#endif /* CONFIG_RTC_CALIBRATION */
};

/* IRQ connection macros */
#define RTC_NXP_SRTC_SINGLE_IRQ_CONNECT(n)                                                         \
	do {                                                                                       \
		IRQ_CONNECT(DT_INST_IRQN(n), DT_INST_IRQ(n, priority), nxp_srtc_isr,               \
			    DEVICE_DT_INST_GET(n), 0);                                             \
		irq_enable(DT_INST_IRQN(n));                                                       \
	} while (false)

#define RTC_NXP_SRTC_ALARM_IRQ_CONNECT(n)                                                          \
	do {                                                                                       \
		IRQ_CONNECT(DT_INST_IRQ_BY_IDX(n, 0, irq), DT_INST_IRQ_BY_IDX(n, 0, priority),     \
			    nxp_srtc_alarm_isr, DEVICE_DT_INST_GET(n), 0);                         \
		irq_enable(DT_INST_IRQ_BY_IDX(n, 0, irq));                                         \
	} while (false)

#define RTC_NXP_SRTC_SECOND_IRQ_CONNECT(n)                                                         \
	do {                                                                                       \
		IRQ_CONNECT(DT_INST_IRQ_BY_IDX(n, 1, irq), DT_INST_IRQ_BY_IDX(n, 1, priority),     \
			    nxp_srtc_second_isr, DEVICE_DT_INST_GET(n), 0);                        \
		irq_enable(DT_INST_IRQ_BY_IDX(n, 1, irq));                                         \
	} while (false)

#define NXP_SRTC_CONFIG_FUNC(n)                                                               \
	static void nxp_srtc_config_func_##n(const struct device *dev)                            \
	{                                                                                         \
		IF_ENABLED(DT_INST_IRQ_HAS_IDX(n, 1),                                              \
				(RTC_NXP_SRTC_ALARM_IRQ_CONNECT(n);   \
				RTC_NXP_SRTC_SECOND_IRQ_CONNECT(n);)) \
		IF_ENABLED(UTIL_NOT(DT_INST_IRQ_HAS_IDX(n, 1)),\
				(RTC_NXP_SRTC_SINGLE_IRQ_CONNECT(n);)) \
	}

#if DT_ANY_INST_HAS_PROP_STATUS_OKAY(clock_output)
#define NXP_SRTC_CLOCK_OUTPUT_ENABLE(n) .is_output_clock_enabled = DT_INST_PROP(n, clock_output),
#else
#define NXP_SRTC_CLOCK_OUTPUT_ENABLE(n)
#endif

#if DT_ANY_INST_HAS_PROP_STATUS_OKAY(time_seconds_frequency)
#define NXP_SRTC_TIME_SECONDS_FREQUENCY(n)                                                     \
	.time_seconds_frequency = DT_INST_PROP(n, time_seconds_frequency),
#else
#define NXP_SRTC_TIME_SECONDS_FREQUENCY(n)
#endif

#define RTC_NXP_SRTC_DEVICE_INIT(n)                                                            \
	NXP_SRTC_CONFIG_FUNC(n)                                                                    \
	static const struct nxp_srtc_config nxp_srtc_config_##n = {                                \
		.base = (RTC_Type *)DT_INST_REG_ADDR(n),                                           \
		.is_lpo_clock_source = (strcmp(DT_INST_PROP(n, clock_source), "LPO") == 0),        \
		NXP_SRTC_CLOCK_OUTPUT_ENABLE(n)                                                    \
		NXP_SRTC_TIME_SECONDS_FREQUENCY(n)                                                 \
		...
		.irq_config_func = nxp_srtc_config_func_##n,                                       \
	};                                                                                         \
                                                                                               \
	static struct nxp_srtc_data nxp_srtc_data_##n;                                             \
                                                                                               \
	DEVICE_DT_INST_DEFINE(n, nxp_srtc_init, NULL, &nxp_srtc_data_##n, &nxp_srtc_config_##n,    \
			      PRE_KERNEL_1, CONFIG_RTC_INIT_PRIORITY, &rtc_nxp_srtc_driver_api);

DT_INST_FOREACH_STATUS_OKAY(RTC_NXP_SRTC_DEVICE_INIT)
