...
def patch_generated_dts(*, t: Target, dts_text: str) -> str:
    """Post-process generated DTS text for build-only robustness."""

    out = dts_text

    # MCXC141/MCXC142 delete the USB node in their SoC DTSI, but the master
    # board DTS still contains an overlay referencing &usb.
    if t.soc in {"mcxc141", "mcxc142"}:
        out = _strip_dts_block(
            text=out,
            start_re=r"^\s*zephyr_udc0\s*:\s*&usb\s*\{\s*$",
            end_re=r"^\s*\};\s*$",
        )

    # MCXE245/MCXE246 delete lpi2c1; drop the master board overlay block.
    if t.soc in {"mcxe245", "mcxe246"}:
        out = _strip_dts_block(
            text=out,
            start_re=r"^\s*&lpi2c1\s*\{\s*$",
            end_re=r"^\s*\};\s*$",
        )
    ...
    #   can drop those overlays to avoid binding/property requirements.
    if t.soc in {"mcxe315", "mcxe316", "mcxe317"}:
        ...
        # Drop the whole LPUART5 overlay block (it may refer to a deleted node).
        out = _strip_dts_block(
            text=out,
            start_re=r"^\s*&lpuart_5\s*\{\s*$",
            end_re=r"^\s*\};\s*$",
        )

        # Drop EIM/EDAC enabling blocks copied from the master DTS.
        out = _strip_dts_block(
            text=out,
            start_re=r"^\s*&eim0\s*\{\s*$",
            end_re=r"^\s*\};\s*$",
        )
        out = _strip_dts_block(
            text=out,
            start_re=r"^\s*&eim0_channel(14|15)\s*\{\s*$",
            end_re=r"^\s*\};\s*$",
        )

        # Drop ERM0 enable block; not needed for hello_world.
        out = _strip_dts_block(
            text=out,
            start_re=r"^\s*&erm0\s*\{\s*$",
            end_re=r"^\s*\};\s*$",
        )
...
def render_kconfig_board(board_name: str, targets: list[Target]) -> str:
    ...
    for t in targets:
        ...
        lines.append(f"\tselect {soc_kconfig_symbol(t)}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    for t in targets:
        ...
        dts_text = patch_generated_dts(t=t, dts_text=dts_text)
