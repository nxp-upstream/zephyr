...
/*
 * The ITCM is RAM, not flash: applications relocate code into it
 * (CONFIG_CODE_DATA_RELOCATION with an ITCM_TEXT location) as well as
 * .rodata/.data/.bss, so the region has to be writable with the XN bit clear.
 * REGION_FLASH_ATTR would make it read-only and REGION_RAM_ATTR sets XN on an
 * XIP build, so neither of them fits.
 *
 * Only the access permission and the XN bit matter here. A TCM is tightly
 * coupled to the core, its accesses are routed to the TCM interface instead of
 * AXIM and are never cached whatever the MPU says, so the cacheability bits of
 * this region only describe the bank.
 */
