...
/*
 * GPU (VG-Lite) resource definitions for RT700.
 * Values are derived from NXP SDK_25_09 (MIMXRT700).
 */
#define CLOCK_ID_GPU                     kCLOCK_Gpu
#define IRQ_ID_GPU                       VGPU_IRQn
#define GPU_REG_BASE                     0x40486000u
#define RESET_ID_GPU                     1U /* Placeholder */
#define POWERGATE_GPU_PG_DEV             1U /* Placeholder */
#define SPI1_GPU_CTL                     1U   /* Placeholder */

/* XSPI0 flash address range */
#define XSPI0_BASE_ADDR   0x28000000U
#define XSPI0_END_ADDR    0x2c000000U  /* 64 MB */
...
static inline int buf_is_nor(void *addr)
{
	...
	return in_range(a, XSPI0_BASE_ADDR, XSPI0_END_ADDR);
}
...
/*
 * The SDRAM region on RT700 spans 6 MB starting at address 0x30080000
 * (0x3008_0000 – 0x3067_e000) and is treated as a single contiguous
 * memory block.
 *
...
#define CACHEABLE_START      0x30080000U
#define CACHEABLE_END        0x30080000U + 0x5fe000U
#define SPI1_UNCACHE_ADDR    0x3067e000U  /* dummy placeholder */

/* --------------------------------------------------------------------
 * SPI cache operation codes (RT700 mapping)
 * --------------------------------------------------------------------
 */
typedef enum {
...
/*
 * --------------------------------------------------------------------
 * Map SPI cache ops to global Cortex-M33 Cache maintenance on RT700
 * ------------------------------------------------------------------
 */
void spi1_cache_ops(spi_cache_op_t op, void *addr, size_t size)
{
	...
	case SPI_CACHE_FLUSH_INVALID_ALL:
		XCACHE_CleanInvalidateCache(XCACHE0);
		break;
