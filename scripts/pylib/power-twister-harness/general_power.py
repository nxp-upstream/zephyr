...
class ChannelReadings:
    ...
    def from_dict(cls, data: dict[str, Any]) -> 'ChannelReadings':
        return cls(
            ...
            readings=[
                Reading(raw=item['raw'], voltage_mv=item['voltage_mv'])
...
class ADCDevice:
    ...
    def from_dict(cls, data: dict[str, Any]) -> 'ADCDevice':
        return cls(
            channels={
                name: ChannelReadings.from_dict(channel_data)
    ...
    def connect(self, dft: DeviceAdapter):
        ...
        if configs:
            ...
            if self.probe_cap.channel_count:
                self.logger.info(
                    'General ADC power monitor connected successfully'
                )
                return
    ...
    def dump_power(self, filename: str | None = None, fake_run: bool = False):
        ...
        for route in common_routes:
            ...
            if count == 0:
                continue
            power_mw[route] = [
                (voltage_samples[i] * current_samples[i]) / 1000.0
    ...
    def _write_route_csv(
        ...
        with open(dump_path, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            headers = ['Sample_Index'] + [
                f'{route}_{suffix}' for route in route_names
            ]
        ...
        for sample in samples:
            for adc_name in sample.adcs:
                ...
                for channel_name in adc_device.channels:
                    ...
                    voltage_data[channel_id].append(
                        adc_readings.get_average_voltage()
                    )
        return voltage_data
    ...
    def _process_routes(
        ...
        for route in self.probe_settings.routes:
            ...
            for value_p, value_n in zip(
                ...
                currents.append(
                    route.voltage_to_current(abs(calibrated_p - calibrated_n))
                )
