...
def test_projectbuilder_sanitize_domains_file(mocked_jobserver):
    top_build_dir = os.path.abspath(
        os.path.join(os.sep, 'absolute', 'path', 'build_dir')
    )
    relative_domains = mock.Mock()
    default_domain = mock.Mock()
    default_domain.name = 'main'
    flash_main = mock.Mock()
    flash_main.name = 'main'
    flash_ipc = mock.Mock()
    flash_ipc.name = 'ipc_radio'
    domain_main = mock.Mock()
    domain_main.name = 'main'
    domain_main.build_dir = 'main'
    domain_ipc = mock.Mock()
    domain_ipc.name = 'ipc_radio'
    domain_ipc.build_dir = 'ipc_radio'
    relative_domains.get_default_domain.return_value = default_domain
    relative_domains.get_top_build_dir.return_value = '.'
    relative_domains.get_domains.side_effect = lambda default_flash_order=False: (
        [flash_main, flash_ipc]
        if default_flash_order else
        [domain_main, domain_ipc]
    )
    ...
    with (
        ...
        mock.patch('twisterlib.runner.Domains.from_file') as from_file,
        mock.patch('builtins.open', mock.mock_open()) as f,
    ):
        domains = mock.Mock()
        domains.get_domains_with_build_dirs_relative_to.return_value = (
            relative_domains
        )
        from_file.return_value = domains

        pb._sanitize_domains_file()

    from_file.assert_called_once_with(os.path.join(top_build_dir, 'domains.yaml'))
    domains.get_domains_with_build_dirs_relative_to.assert_called_once_with(
        top_build_dir
    )
